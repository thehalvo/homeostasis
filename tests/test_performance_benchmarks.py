    ...
    def test_language_detection_speed(self):
        ...
        for language, errors in self.test_errors.items():
            ...
            benchmark_result = self.run_benchmark(
                f"detect_{language}", detect_language, iterations=1000
            )
        ...
        primary_avg = statistics.mean(
            [results[lang]["mean"] for lang in primary_languages]
        )
        fringe_avg = statistics.mean(
            [results[lang]["mean"] for lang in fringe_languages]
        )
...
class TestErrorAnalysisPerformance(PerformanceBenchmark):
    ...
    def test_error_analysis_performance(self):
        ...
        for language, error in self.complex_errors.items():
            ...
            benchmark_result = self.run_benchmark(
                f"analyze_{language}", analyze_error, iterations=100
            )
            results[language] = benchmark_result
...
class TestPatchGenerationPerformance(PerformanceBenchmark):
    ...
    def test_patch_generation_performance(self):
        ...
        for language, scenario in self.patch_scenarios.items():
            ...
            def generate_patch():
                return plugin.generate_fix(scenario["analysis"], scenario["context"])

            benchmark_result = self.run_benchmark(
                f"patch_{language}", generate_patch, iterations=500
            )
            results[language] = benchmark_result
...
class TestMemoryUsageComparison(PerformanceBenchmark):
    ...
    def test_plugin_memory_footprint(self):
        ...
        fringe_avg = statistics.mean(
            [results[lang] for lang in ["zig", "nim", "crystal"] if lang in results]
        )
...
class TestRuleMatchingPerformance(PerformanceBenchmark):
    ...
    def _generate_rule_test_cases(self) -> Dict[str, List[Dict]]:
        ...
        return {
            "python": [
                ...
                {
                    "pattern": "TypeError.*takes.*positional",
                    "text": "TypeError: func() takes 2 positional arguments but 3 were given",
                },
            ]
            * 20,  # Multiply to test with many rules
            "javascript": [
                ...
                },
                {
                    ...
                    "text": "TypeError: callback is not a function",
                },
                {
            ...
            "java": [
                {
                    ...
                    "text": "java.lang.NullPointerException at com.example.Main",
                },
                {
                    "pattern": "ClassNotFoundException.*",
                    "text": "ClassNotFoundException: com.example.MissingClass",
                },
                {
                    "pattern": "ArrayIndexOutOfBoundsException",
                    "text": "ArrayIndexOutOfBoundsException: 10",
                },
            ]
            * 20,
        }
    ...
    def test_rule_matching_performance(self):
        ...
        for language, test_cases in self.rule_test_cases.items():
            ...
            benchmark_result = self.run_benchmark(
                f"rules_{language}", match_rules, iterations=100
            )
            results[language] = benchmark_result
