...
class TestBasicHealingScenarios:
    ...
    @pytest.mark.asyncio
    async def test_keyerror_healing(
        ...
        def trigger_keyerror():
            test_environment.inject_error(
                error_type="KeyError", file_path="app.py", error_code=""
            )
    ...
    @pytest.mark.asyncio
    async def test_attributeerror_healing(
        self, test_environment, scenario_runner, metrics_collector
    ):
        ...
        def trigger_attributeerror():
            test_environment.inject_error(
                error_type="AttributeError", file_path="app.py", error_code=""
            )
            test_environment.stop_service()
            test_environment.start_service()
            test_environment.trigger_error()
    ...
    @pytest.mark.asyncio
    async def test_typeerror_healing(
        self, test_environment, scenario_runner, metrics_collector
    ):
        ...
        def trigger_typeerror():
            test_environment.inject_error(
                error_type="TypeError", file_path="app.py", error_code=""
            )
            test_environment.stop_service()
            test_environment.start_service()
            test_environment.trigger_error()
...
"""
    ...
    @pytest.mark.asyncio
    async def test_multiple_error_healing(
        self, test_environment, scenario_runner, metrics_collector
    ):
...
"""
        ...
        metrics_collector.record_healing_duration(
            "Multiple Error Sequence", result1.duration + result2.duration
        )
        metrics_collector.record_success_rate(
            "Multiple Error Sequence", result1.success and result2.success
        )
    ...
    @pytest.mark.asyncio
    async def test_healing_performance_metrics(
        self, test_environment, scenario_runner, metrics_collector
    ):
        ...
        assert (
            total_time < scenario.timeout
        ), f"Scenario exceeded timeout: {total_time}s"
    ...
    def test_metrics_summary(self, metrics_collector):
        ...
        assert summary["scenario_success_rates"]["Test Scenario 1"] == pytest.approx(
            0.667, 0.01
        )
