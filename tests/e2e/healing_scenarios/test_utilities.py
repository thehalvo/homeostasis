...
"""
    ...
    def _create_test_config(self):
        ...
        config = {
            "general": {"project_name": "healing_test", "environment": "test"},
            "service": {
                "name": "test_service",
        ...
        raise"""
            ...
            if "logger.error" not in error_code:
                ...
                if lines and lines[0].startswith("@app.get"):
                    func_def_line = next(
                        (i for i, line in enumerate(lines) if "async def" in line), None
                    )
...
class HealingScenarioRunner:
    ...
    async def run_scenario(self, scenario: HealingScenario) -> HealingResult:
        ...
        try:
            ...
            healing_task = asyncio.create_task(self._monitor_healing_cycle(result))
            ...
            validation_passed = await self._validate_healing(scenario, result)

            result.success = (
                healing_success and validation_passed and result.deployment_successful
            )
    ...
    async def _validate_healing(
        self, scenario: HealingScenario, result: HealingResult
    ) -> bool:
        ...
        for check in scenario.validation_checks:
            try:
                ...
                all_checks_passed = False
                result.logs.append(
                    f"Validation check error: {check.__name__} - {str(e)}"
                )

        # Additional validation: verify error is fixed
        try:
            response = requests.get(
                f"http://localhost:{self.environment.port}/error", timeout=5
            )
            if response.status_code == 200:
                ...
                all_checks_passed = False
                result.logs.append(
                    f"Error endpoint still failing: {response.status_code}"
                )
        except Exception as e:
...
class MetricsCollector:
    ...
    def record_success_rate(self, scenario_name: str, success: bool):
        ...
        if scenario_name not in self.metrics["success_rates"]:
            self.metrics["success_rates"][scenario_name] = {"total": 0, "successful": 0}
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        if self.metrics["healing_durations"]:
            total_duration = sum(
                m["duration"] for m in self.metrics["healing_durations"]
            )
            summary["average_healing_duration"] = total_duration / len(
                self.metrics["healing_durations"]
            )
