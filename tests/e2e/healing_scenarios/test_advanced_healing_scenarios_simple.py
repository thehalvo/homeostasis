...
class TestAdvancedHealingScenariosSimple:
    ...
    async def test_framework_specific_healing(self):
        ...
        # Mock the components
        with patch(
            "modules.monitoring.extractor.extract_errors"
        ) as mock_extract_errors, patch(
            "orchestrator.orchestrator.Analyzer"
        ) as mock_analyzer_cls, patch(
            "orchestrator.orchestrator.PatchGenerator"
        ) as mock_patch_gen_cls:
    ...
    async def test_concurrent_healing_scenarios(self):
        ...
        async def run_healing_scenario(error_type, root_cause):
            ...
            with patch(
                ...
                # Mock error
                mock_extract_errors.return_value = [
                    {"exception_type": error_type, "message": f"{error_type} occurred"}
                ]

                # Mock analysis
                mock_analyzer.analyze_errors.return_value = [
                    {"error_type": error_type, "root_cause": root_cause}
                ]
        ...
        ), f"Expected at least 2 successful healings, got {successful_healings}"
