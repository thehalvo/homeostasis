    ...
    if USE_MOCK_INFRASTRUCTURE:
        ...
        def inject_error_adapter(error_type, file_path=None, error_code=None, **kwargs):
            ...
            if 'service' in kwargs:
                return original_inject_error(service=kwargs['service'], error_type=error_type)
            else:
        ...
        env.stop_service = lambda: env.services.get('test_service').stop() if 'test_service' in env.services else None
        ...
        def create_javascript_service():
            service = env.create_cross_language_service('test_service', 'javascript', 8000)
            service.start()
