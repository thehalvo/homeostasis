...
class E2ETestRunner:
    ...
    def _generate_recommendations(self, report: Dict[str, any]) -> List[str]:
        ...
        if report["summary"].get("total_tests", 0) > 0:
            success_rate = (
                report["summary"].get("passed", 0) / report["summary"]["total_tests"]
            )
...
def main():
    ...
    parser.add_argument(
        "--timeout", type=int, default=3600, help="Overall test timeout in seconds"
    )

    parser.add_argument(
        "--ci", action="store_true", help="CI mode - fail on any test failure"
    )
