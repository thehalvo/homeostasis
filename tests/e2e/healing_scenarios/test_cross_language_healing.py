...
"""
        ...
        self.orchestrator.config["service"]["start_command"] = (
            "cd " + str(self.service_path) + " && npm install && node app.js"
        )
...
"""
        ...
        self.orchestrator.config["service"][
            "start_command"
        ] = f"cd {self.service_path} && go mod download && go run main.go"
...
"""
        ...
        self.orchestrator.config["service"][
            "start_command"
        ] = f"cd {self.service_path} && javac SimpleService.java && java SimpleService"
...
class TestCrossLanguageHealing:
    ...
    @pytest.mark.parametrize(
        "language,setup_method,error_pattern",
        [
            (
                "javascript",
                "create_javascript_service",
    ...
    async def test_language_specific_healing(
        self, language: str, setup_method: str, error_pattern: str, metrics_collector
    ):
        ...
        if USE_MOCK_INFRASTRUCTURE:
            ...
            def create_js():
                svc = env.create_cross_language_service(
        ...
        try:
            ...
            metrics_collector.record_healing_duration(
                f"{language}_healing", result.duration
            )
            ...
            assert (
                result.patch_generated or result.patch_applied
            ), f"{language} patch should have been generated/applied"
    ...
    async def test_polyglot_microservice_healing(self, metrics_collector):
        ...
        # Modify port
        js_env.orchestrator.config["service"][
            "health_check_url"
...
async def trigger_error():
    ...
    try:
        response = requests.get("http://localhost:8001/data", timeout=5)
        data = response.json()
...
"""
        ...
        scenario = HealingScenario(
            ...
            validation_checks=[
                lambda: all(
                    check_service_healthy_on_port(port) for _, _, port in services
                ),
        ...
        try:
            runner = HealingScenarioRunner(
                services[0][1]
            )  # Use Python service for healing
            result = await runner.run_scenario(scenario)

            metrics_collector.record_healing_duration(
                "polyglot_healing", result.duration
            )
...
"""
        ...
        try:
            ...
            metrics_collector.record_healing_duration(
                "migration_healing", result.duration
            )
