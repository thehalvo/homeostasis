...
class TestDependencyVulnerabilityScanner:
    ...
    async def test_scan_npm_dependencies(self, scanner, temp_project):
        ...
        package_json = {
            "name": "test-app",
            "version": "1.0.0",
            "dependencies": {"lodash": "4.17.11"},  # Known vulnerable version
        }
...
"""
        ...
        with patch("asyncio.create_subprocess_exec") as mock_subprocess:
            ...
            vulnerabilities = await scanner._scan_python_code(
                temp_code / "vulnerable.py"
            )
...
"""
            ...
            with patch("asyncio.create_subprocess_exec") as mock_subprocess:
                ...
                mock_process.returncode = (
                    1  # Non-zero for safety when vulnerabilities found
                )
                mock_subprocess.return_value = mock_process
            ...
            assert any(
                cat in report["summary"]["by_category"]
