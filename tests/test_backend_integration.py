...
class TestBackendLanguageIntegration:
    ...
    def test_shared_error_schema(self):
        ...
        test_errors = {
            "python": {
                ...
                "traceback": ["Traceback (most recent call last):", "  File \"test.py\", line 1"]
            },
            "java": {
        ...
        for lang, error_data in test_errors.items():
            ...
            if lang == "python":
                assert denormalized.get("exception_type") == error_data.get("exception_type")
            elif lang == "java":
                assert denormalized.get("exception_class") == error_data.get("exception_class")
    ...
    def test_language_specific_analysis(self, test_runner, test_suite):
        ...
        for lang in ["python", "java", "javascript", "go"]:
            ...
            for case in cases:
                ...
                assert analysis, f"No analysis results for {lang} test case: {case.name}"
        ...
        for source_lang in languages:
            ...
            for target_lang in languages:
                ...
                for case in source_cases:
                    ...
                    assert normalized_roundtrip["error_type"] == normalized["error_type"], \
                        f"Error type changed: {normalized['error_type']} -> {normalized_roundtrip['error_type']}"
                    
                    assert normalized_roundtrip["message"] == normalized["message"], \
    ...
    def test_orchestrator_integration(self, test_suite):
        ...
        for case in test_suite.test_cases:
            try:
                ...
                # Check that analysis produced results
                assert analysis, f"No analysis results for {case.language} test case: {case.name}"
                
                # Check that language was automatically detected
                detected_analysis = orchestrator.analyze_error(case.error_data)
                assert detected_analysis, f"Language detection failed for test case: {case.name}"
                
                # Results should be similar between explicit and auto-detection
                assert detected_analysis.get("root_cause") == analysis.get("root_cause"), \
                    "Analysis results differ between explicit and auto-detected language"
            except Exception as e:
                pytest.fail(f"Error analyzing {case.language} test case {case.name}: {e}")
    ...
    def test_cross_language_capabilities(self, test_runner, test_suite):
        ...
        successful_conversions = sum(1 for r in results["conversion_tests"] if r["passed"])
        if len(results["conversion_tests"]) > 0:
            success_rate = (successful_conversions / len(results["conversion_tests"])) * 100
...
if __name__ == "__main__":
    ...
    if suite_path.exists():
        ...
        if failed > 0:
            ...
            for result in results:
                if not result.passed:
                    logging.warning(f"  {result.test_case.name} ({result.test_case.language})")
        ...
        logging.info(f"  Conversion tests: {len(cross_lang_results['conversion_tests'])}")
        successful = sum(1 for r in cross_lang_results['conversion_tests'] if r.get('passed'))
        logging.info(f"    Passed: {successful}")
        
        logging.info(f"  Pattern detection tests: {len(cross_lang_results['pattern_detection_tests'])}")
        with_similar = sum(1 for r in cross_lang_results['pattern_detection_tests'] 
                         if r.get('found_similar'))
        logging.info(f"    Found similar errors: {with_similar}")
        
        logging.info(f"  Cross suggestion tests: {len(cross_lang_results['cross_suggestion_tests'])}")
        with_suggestions = sum(1 for r in cross_lang_results['cross_suggestion_tests'] 
