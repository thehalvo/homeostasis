...
class ComprehensiveTestRunner:
    ...
    async def run_language_tests(self, languages: List[str] = None, 
        ...
        logger.info(f"Running tests for {len(target_languages)} languages: {sorted(target_languages)}")
        ...
        filtered_suites = {
            lang: cases for lang, cases in self.orchestrator.test_suites.items()
        ...
        for category, languages in LANGUAGE_CATEGORIES.items():
            ...
            if category_languages:
                ...
                category_stats[category] = {
                    ...
                    "pass_rate": sum(1 for r in category_results if r.passed) / len(category_results) * 100
                }
    ...
    def _collect_error_types(self, results: List[IntegrationTestResult]) -> Dict[str, int]:
    ...
    def _collect_fix_types(self, results: List[IntegrationTestResult]) -> Dict[str, int]:
    ...
    def _analyze_error_distribution(self, errors: List[Dict[str, Any]]) -> Dict[str, Any]:
        """Analyze the distribution of errors."""
        error_types = {}
        error_categories = {
            "null_safety": ["NullPointerException", "AttributeError", "TypeError", "nil"],
            "bounds_checking": ["IndexError", "IndexOutOfBounds", "ArrayIndexOutOfBoundsException"],
    ...
    def _analyze_fix_effectiveness(self) -> Dict[str, Any]:
            ...
            for result in results:
                ...
                for fix in result.generated_fixes:
                    ...
                    if fix_type not in fix_success_by_type:
                        fix_success_by_type[fix_type] = {"generated": 0, "successful": 0}
        ...
        return {
            ...
            "fix_generation_rate": (total_fixes_generated / total_errors * 100) if total_errors > 0 else 0,
            "fix_success_rate": (successful_fixes / total_fixes_applied * 100) if total_fixes_applied > 0 else 0,
    ...
    def print_summary(self, report: Dict[str, Any]):
        ...
        for language, stats in sorted(report["language_statistics"].items()):
            print(f"{language:15} - Total: {stats['total']:3d}, "
        ...
        if report["category_statistics"]:
            ...
            for category, stats in sorted(report["category_statistics"].items()):
                print(f"{category:15} - Languages: {len(stats['languages']):2d}, "
        ...
        print(f"Fixes Generated: {fix_stats['total_fixes_generated']} "
...
async def main():
    ...
    parser.add_argument(
        "--languages", "-l",
        nargs="+",
        help="Specific languages to test"
    )
    parser.add_argument(
        "--categories", "-c",
        nargs="+",
    ...
    parser.add_argument(
        "--output", "-o",
        type=Path,
        help="Output path for the test report"
    )
    parser.add_argument(
        "--test-dir", "-t",
        type=Path,
