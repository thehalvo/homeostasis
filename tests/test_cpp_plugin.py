...
class TestCPPExceptionHandler:
    ...
    def test_analyze_segmentation_fault(self):
        ...
        error_data = {
            ...
            "stack_trace": [{"function": "strcpy", "file": "string.cpp", "line": 10}],
        }
        ...
        assert any(
            keyword in analysis["suggestion"].lower()
            for keyword in ["null", "pointer", "buffer", "memory", "check"]
        )
    ...
    def test_analyze_stl_error(self):
        ...
        assert suggestion == "" or any(
            keyword in suggestion
            for keyword in ["bounds", "size", "check", "range", "index"]
        )
    ...
    def test_analyze_race_condition(self):
        ...
        assert any(
            keyword in analysis["suggestion"].lower()
            for keyword in ["mutex", "atomic", "sync", "lock", "thread", "check"]
        )
...
class TestCPPPatchGenerator:
    ...
    def test_generate_null_check_patch(self):
        ...
        context = {"code_snippet": "int len = strlen(str);", "variable_name": "str"}
        ...
        assert any(
            check in null_ptr_patch["suggestion_code"]
            for check in ["if (str != nullptr)", "if (str)", "if (str != NULL)"]
        )
    ...
    def test_generate_thread_safety_patch(self):
        ...
        analysis = {
            ...
            "error_data": {"error_type": "DataRace"},
            "suggestion": "Add synchronization",
            "confidence": "high",
        }

        context = {"code_snippet": "counter++;", "shared_variable": "counter"}
...
class TestCPPLanguagePlugin:
    ...
    def test_analyze_runtime_error(self, plugin):
        ...
        error_data = {
            "error_type": "SegmentationFault",
            "signal": "SIGSEGV",
            "stack_trace": [{"function": "main", "file": "test.cpp", "line": 20}],
        }
    ...
    def test_generate_fix(self, plugin):
        ...
        analysis = plugin.analyze_error(error_data)
        context = {"code_snippet": 'cout << "Hello World";', "includes": []}
        ...
        assert any(
            text in fix_content.lower()
            for text in ["include", "iostream", "std", "namespace", "fix", "check"]
        )
...
class TestCPPFrameworkErrors:
    ...
    def test_qt_signal_slot_error(self):
        ...
        assert suggestion == "" or any(
            keyword in suggestion
            for keyword in ["signal", "slot", "connect", "qt", "check", "null"]
        )
    ...
    def test_boost_exception(self):
        ...
        assert analysis.get("category") in [
            "runtime",
            "filesystem",
            "unknown",
            "memory",
        ...
        assert suggestion == "" or any(
            keyword in suggestion
            for keyword in ["file", "directory", "path", "check", "exist"]
        )
    ...
    def test_stl_exception(self):
        ...
        assert suggestion == "" or any(
            keyword in suggestion
            for keyword in ["range", "bounds", "size", "check", "index"]
        )
...
class TestCPPMemoryDebugging:
    ...
    def test_addresssanitizer_heap_use_after_free(self):
        ...
        error_data = {
            ...
            "stack_trace": [{"function": "getData", "file": "data.cpp", "line": 50}],
        }
...
class TestCPPIntegration:
    ...
    def test_end_to_end_memory_leak_fix(self):
        ...
        error_data = {
            ...
            "stack_trace": [
                {"function": "createBuffer", "file": "buffer.cpp", "line": 15}
            ],
        }
    ...
    def test_end_to_end_segfault_analysis(self):
        ...
        error_data = {
            ...
            "message": "Segmentation fault (core dumped)",
            "stack_trace": [
                {"function": "strlen", "file": "string.h", "line": 0},
                {"function": "processString", "file": "processor.cpp", "line": 25},
            ],
        }
    ...
    def test_framework_detection_in_errors(self):
        ...
        test_cases = [
            {"error_type": "QObject::connect", "expected_framework": "qt"},
            {"error_type": "boost::bad_lexical_cast", "expected_framework": "boost"},
            {"error_type": "std::bad_alloc", "expected_framework": "stl"},
        ]
