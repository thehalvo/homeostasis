...
def test_error_normalization():
    ...
    if "stack_trace" in standard_error and isinstance(
        ...
        if isinstance(standard_error["stack_trace"][0], dict):
            ...
            print(
                f"❌ Stack trace not normalized to structured frames: {standard_error['stack_trace']}"
            )
            success = False
    else:
        print(
            f"❌ Stack trace missing or not a list: {standard_error.get('stack_trace')}"
        )
        success = False
...
def test_error_denormalization():
    ...
    for key, expected in [
        ...
        if csharp_error.get(key) != expected:
            print(
                f"❌ Field {key}: Expected '{expected}', got '{csharp_error.get(key)}'"
            )
            success = False
        else:
            print(f"✅ Field {key}: '{expected}'")
    ...
    if "stack_trace" in csharp_error and isinstance(csharp_error["stack_trace"], str):
        ...
        print(
            f"❌ Stack trace missing or not a string: {csharp_error.get('stack_trace')}"
        )
        success = False
...
def test_stack_trace_parsing():
    ...
    for key, expected in [
        ...
        if first_frame.get(key) != expected:
            print(
                f"❌ First frame field {key}: Expected '{expected}', got '{first_frame.get(key)}'"
            )
            success = False
        else:
    ...
    if (
        ...
        print(
            f"✅ Lambda frame parsing succeeded: file={lambda_frame.get('file')}, line={lambda_frame.get('line')}"
        )
