...
class ChaosTestRunner:
    ...
    async def run(self, experiment_files: Optional[List[str]] = None):
        ...
        try:
            ...
            self.logger.info(f"Chaos tests completed. Run: {self.experiments_run}, "
                           f"Passed: {self.experiments_passed}, Failed: {self.experiments_failed}")
    
    async def load_experiments(self, experiment_files: Optional[List[str]] = None) -> List[ChaosExperiment]:
    ...
    def _generate_service_experiments(self, intensity: str) -> List[ChaosExperiment]:
        ...
        failure_params = {
            ...
            'medium': {'failure_probability': 0.3, 'affected_dependencies': ['cache', 'secondary-db']},
            'high': {'failure_probability': 0.5, 'affected_dependencies': ['cache', 'primary-db', 'auth']}
        }
    ...
    async def run_experiment(self, experiment: ChaosExperiment):
        ...
        if not await self.check_safety_conditions(baseline_metrics):
            self.logger.warning(f"Safety conditions not met, skipping experiment: {experiment.name}")
        ...
        try:
            ...
            if result.get('hypothesis_validated', False):
                ...
                self.logger.warning(f"Experiment failed: {experiment.name} - "
            ...
            self.logger.info(f"Experiment {experiment.name} completed in {duration:.2f} seconds")
        ...
        while not self.should_stop:
            try:
                ...
                if not await self.check_safety_conditions(metrics):
                    self.logger.error("Critical safety threshold breached during experiment!")
