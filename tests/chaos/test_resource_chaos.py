...
class TestResourceChaos:
    ...
    async def test_cpu_contention_patterns(self, resource_monitor):
        ...
        try:
            ...
            for i in range(5):
                ...
                await asyncio.sleep(
                    3
                )  # Extended low utilization period for CPU to settle
        ...
        assert (
            avg_high != avg_low
        ), "No variation detected between high and low CPU usage"
            ...
            def create_memory_churn(self, rate_mb_per_sec, duration_sec):
                ...
                while time.time() - start_time < duration_sec:
                    ...
                    if len(self.allocations) > 10 and random.random() > 0.5:
                        with self.allocation_lock:
                            self.allocations.pop(
                                random.randint(0, len(self.allocations) - 1)
                            )
                    ...
                    if churned_bytes > expected_bytes:
                        sleep_time = (churned_bytes - expected_bytes) / (
                            rate_mb_per_sec * 1024 * 1024
                        )
        ...
        class DiskIOSimulator:
            ...
            def sequential_write(self, file_size_mb, block_size_kb=64):
                ...
                return {
                    ...
                    "throughput_mbps": (
                        (bytes_written / 1024 / 1024) / sum(write_times)
                        if write_times
                        else 0
                    ),
                }
            ...
            def random_io(self, file_size_mb, io_size_kb=4, read_ratio=0.5):
                ...
                return {
                    ...
                    "avg_latency_ms": (
                        (sum(io_times) / len(io_times)) * 1000 if io_times else 0
                    ),
            ...
            def concurrent_io(self, num_threads=4, file_size_mb=10):
                ...
                def worker(thread_id):
                    file_path = os.path.join(
                        self.test_dir, f"concurrent_{thread_id}.dat"
                    )
                    operations = 0
                    start_time = time.time()
    ...
    async def test_resource_starvation(self, resource_monitor):
        ...
        process_results = {"greedy": [], "moderate": [], "minimal": []}
        ...
        async def greedy_process(process_id):
            ...
            while attempts < 10:
                acquired = starvation.request_resources(
                    process_id, cpu_tokens=80, memory_mb=800, io_mbps=80
                )
            ...
            return {"type": "greedy", "acquired": acquired, "attempts": attempts}
        ...
        async def moderate_process(process_id):
            ...
            while attempts < 20:
                acquired = starvation.request_resources(
                    process_id, cpu_tokens=30, memory_mb=256, io_mbps=20
                )
            ...
            return {"type": "moderate", "acquired": acquired, "attempts": attempts}
        ...
        async def minimal_process(process_id):
            ...
            while attempts < 30:
                acquired = starvation.request_resources(
                    process_id, cpu_tokens=10, memory_mb=64, io_mbps=5
                )
            ...
            return {"type": "minimal", "acquired": acquired, "attempts": attempts}
