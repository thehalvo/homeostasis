...
class TestPreflightChecks:
    ...
    def test_system_resources(self):
        ...
        if cpu_percent >= cpu_threshold:
            pytest.skip(
                f"CPU usage too high: {cpu_percent}% (threshold: {cpu_threshold}%)"
            )
        ...
        if memory.percent >= memory_threshold:
            pytest.skip(
                f"Memory usage too high: {memory.percent}% (threshold: {memory_threshold}%)"
            )
        if memory.available < min_memory_gb * 1024 * 1024 * 1024:
            pytest.skip(
                f"Less than {min_memory_gb}GB memory available: {memory.available / (1024**3):.1f}GB"
            )
        ...
        if disk.percent >= disk_threshold:
            pytest.skip(
                f"Disk usage too high: {disk.percent}% (threshold: {disk_threshold}%)"
            )
        if disk.free < min_disk_gb * 1024 * 1024 * 1024:
            pytest.skip(
                f"Less than {min_disk_gb}GB disk space available: {disk.free / (1024**3):.1f}GB"
            )
    ...
    def test_process_limits(self):
        ...
        print(
            f"DEBUG: Current file descriptor limits - soft: {soft_limit}, hard: {hard_limit}"
        )
        ...
        if soft_limit < 1024:
            ...
            try:
                ...
                soft_limit, hard_limit = resource.getrlimit(resource.RLIMIT_NOFILE)
                print(
                    f"DEBUG: Updated file descriptor limits - soft: {soft_limit}, hard: {hard_limit}"
                )
        ...
        assert (
            soft_limit >= 1024
        ), f"File descriptor limit too low: {soft_limit} (expected >= 1024)"
    ...
    def test_docker_availability(self, docker_client):
        ...
        available_images = {
            img.tags[0] for img in docker_client.images.list() if img.tags
        }
    ...
    async def test_baseline_performance(self):
        ...
        metrics = {"cpu_samples": [], "memory_samples": [], "io_samples": []}
        ...
        cpu_baseline_threshold = int(
            os.environ.get("CHAOS_CPU_BASELINE_THRESHOLD", "50")
        )
        memory_baseline_threshold = int(
            os.environ.get("CHAOS_MEMORY_BASELINE_THRESHOLD", "85")
        )

        # Verify system is relatively idle
        if avg_cpu >= cpu_baseline_threshold:
            pytest.skip(
                f"System CPU baseline too high: {avg_cpu:.1f}% (threshold: {cpu_baseline_threshold}%)"
            )
        if avg_memory >= memory_baseline_threshold:
            pytest.skip(
                f"System memory baseline too high: {avg_memory:.1f}% (threshold: {memory_baseline_threshold}%)"
            )
    ...
    def test_permissions(self):
        ...
        try:
            ...
            result = subprocess.run(
                ["capsh", "--print"], capture_output=True, text=True
            )
