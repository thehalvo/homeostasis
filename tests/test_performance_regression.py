...
class TestCoreComponentPerformance:
    ...
    def test_error_detection_performance(self):
        ...
        assert (
            result["duration"]["mean"] < threshold
...
class TestPatchGenerationPerformance:
    ...
    def setup_method(self):
        ...
        self.generator = AdvancedCodeGenerator(
            llm_manager=mock_llm_manager, context_manager=mock_context_manager
        )
        # Check if we're in mock mode
        self.use_mock_tests = (
            os.environ.get("USE_MOCK_TESTS", "false").lower() == "true"
        ...
        test_cases = [
            {
                ...
                "context": {"code": "print(x)", "line": 10, "variables": ["y", "z"]},
            },
            {
                "language": "javascript",
            ...
            for case in test_cases:
                ...
                simple_patch = {
                    ...
                    "fixed": case["context"]["code"].replace(
                        var_to_replace, case["context"]["variables"][0]
                    ),
                    "confidence": 0.8,
                }
