...
class TestRegulatedIndustriesSupport:
        ...
        with patch("modules.security.regulated_industries.get_compliance_reporting"):
            with patch("modules.security.regulated_industries.get_policy_engine"):
                with patch("modules.security.regulated_industries.get_rbac_manager"):
                    with patch(
                        "modules.security.regulated_industries.get_audit_logger"
                    ):
    ...
    def test_healthcare_configuration(self, regulated_support):
        ...
        result = regulated_support.configure_industry(
            RegulatedIndustry.HEALTHCARE, requirements, config
        )
        ...
        industry_config = regulated_support.industry_configs[
    ...
    def test_financial_services_configuration(self, regulated_support):
        ...
        config = {
            ...
            "approval_thresholds": {"timeout": 60},
            "retention_policies": {
        ...
        result = regulated_support.configure_industry(
            RegulatedIndustry.FINANCIAL_SERVICES, requirements, config
        )

        assert result is True
        assert (
            RegulatedIndustry.FINANCIAL_SERVICES in regulated_support.industry_configs
        )
    ...
    def test_financial_validation(self, regulated_support):
        ...
        action = {
            ...
            "approvals": {"change_advisory_board": True},
            "testing": {"user_acceptance_testing": True},
        ...
        sox_failures = [
            f for f in validation.failures if "SOX" in f.get("requirement", "")
        ]
    ...
    def test_government_validation(self, regulated_support):
        ...
        fedramp_failures = [
            f for f in validation.failures if "FEDRAMP" in f.get("requirement", "")
        ]
    ...
    def test_telecom_validation(self, regulated_support):
        ...
        # Should pass with proper controls
        nerc_failures = [
            f for f in validation.failures if "CIP" in f.get("requirement", "")
        ]
    ...
    def test_validation_workflow_creation(self, regulated_support):
        ...
        workflow_id = regulated_support.create_validation_workflow(
            RegulatedIndustry.HEALTHCARE, "phi_data_change", steps
        )
    ...
    def test_industry_dashboard(self, regulated_support):
        ...
        # Get dashboard
        dashboard = regulated_support.get_industry_dashboard(
            RegulatedIndustry.HEALTHCARE
        )
    ...
    def test_multi_industry_validation(self, regulated_support):
        ...
        hipaa_checked = any(
            r.value.startswith("hipaa") for r in validation.requirements_checked
        )
        pci_checked = any(
            r.value.startswith("pci") for r in validation.requirements_checked
        )
    ...
    def test_compliance_evidence_collection(self, regulated_support):
        ...
        regulated_support.audit_logger.search_events = Mock(
            return_value=[
        ...
        action = {
            ...
            "validation_docs": [{"id": "val_doc_001"}, {"id": "val_doc_002"}],
        }

        context = {"user": "validator", "data_types": ["pharmaceutical"]}
    ...
    def test_industry_specific_recommendations(self, regulated_support):
        ...
        context = {"user": "developer", "data_types": ["phi"]}
    ...
    def test_configuration_persistence(self, regulated_support, tmp_path):
        ...
        new_support = RegulatedIndustriesSupport(
            {"storage_path": str(tmp_path / "regulated")}
        )
