...
class TestSelfTrainingIntegration:
    ...
    def annotation_interface(self, temp_dir):
        ...
        return AnnotationInterface(
            storage_dir=temp_dir / "annotations", task_expiry_hours=1
        )
    ...
    def test_rule_extraction_from_fixes(self, temp_dir):
        ...
        rule_extractor = RuleExtractor(storage_dir=temp_dir / "rules", min_examples=2)
...
"""
        ...
        for fix in fixes:
            patterns = rule_extractor.analyze_successful_fix(
                fix["error_data"], fix["patch"], fix["metadata"]
            )
...
"""
        ...
        rule_extractor.analyze_successful_fix(
            similar_fix["error_data"], similar_fix["patch"], similar_fix["metadata"]
        )
    ...
    def test_adaptive_confidence_thresholds(self, temp_dir):
        ...
        calculator = ConfidenceCalculator(
            storage_dir=temp_dir / "confidence", base_threshold=0.7
        )
        ...
        assert (
            low_risk_result["adjusted_confidence"]
            > critical_context.historical_success_rate
        )
    ...
    def test_continuous_learning_pipeline(self, temp_dir, ml_feedback_loop):
        ...
        deployment_monitor = DeploymentMonitor(
            storage_dir=temp_dir / "deployments", monitoring_duration_hours=0.001
        )
    ...
    def test_review_trigger_logic(self, temp_dir):
        ...
        assert any(
            "critical" in r.lower() or "confidence" in r.lower() for r in reasons
        )

    def test_feedback_loop_with_annotations(
        self, ml_feedback_loop, annotation_interface
    ):
        ...
        annotation_interface.register_callback(
            AnnotationType.PATCH_REVIEW, annotation_callback
        )
