...
class LanguagePlugin:
    ...
    def get_language_version(self): pass
    def analyze_error(self, error_data): pass
    def normalize_error(self, error_data): pass
    def denormalize_error(self, standard_error): pass
    def generate_fix(self, analysis, context): pass
    ...
    def test_error_normalization(self):
        ...
        if "stack_trace" in standard_error and isinstance(standard_error["stack_trace"], list):
            if isinstance(standard_error["stack_trace"][0], dict):
                ...
                print(f"ℹ️ Stack trace normalized but not structured: {standard_error['stack_trace'][:2]}")
        else:
            success = False
            print(f"❌ Stack trace field missing or not a list: {standard_error.get('stack_trace')}")
        ...
        for key, expected in [
            ...
            if csharp_error.get(key) != expected:
                success = False
                print(f"❌ Round-trip field {key}: Expected '{expected}', got '{csharp_error.get(key)}'")
            else:
        ...
        if "stack_trace" in csharp_error and isinstance(csharp_error["stack_trace"], str):
            ...
            success = False
            print(f"❌ Round-trip stack trace field missing or not a string: {csharp_error.get('stack_trace')}")
