    ...
    return [
        {
            ...
            "auth": {"kubeconfig": "/path/to/kubeconfig", "context": "prod-1"},
            "region": "us-east-1",
            "service_mesh": "istio",
        ...
        },
        {
            ...
            "auth": {"kubeconfig": "/path/to/kubeconfig", "context": "prod-2"},
            "region": "us-west-2",
            "service_mesh": "istio",
            "namespaces": ["default", "production"],
        ...
        },
        {
            ...
            "auth": {"kubeconfig": "/path/to/eks-kubeconfig"},
            "region": "eu-west-1",
    ...
    return [
        {
            ...
            "traffic_split": {"k8s-prod-1": 0.5, "k8s-prod-2": 0.3, "eks-prod": 0.2},
...
async def test_kubernetes_connector():
    ...
    with patch(
        "modules.deployment.multi_environment.cross_cluster.KubernetesManager"
    ) as mock_k8s:
        ...
        # Test get health
        mock_manager.list_nodes = AsyncMock(
            return_value=[
                {"name": "node1", "status": "Ready"},
                {"name": "node2", "status": "Ready"},
            ]
        )
        mock_manager.get_cluster_metrics = AsyncMock(
            return_value={
                "cpu_usage": 45.0,
                "memory_usage": 55.0,
                "error_rate": 0.02,
...
def test_replica_distribution_balanced(orchestrator):
    ...
    distribution = orchestrator._calculate_replica_distribution(
        service, clusters, policy
    )
    ...
    assert abs(sum(distribution.values()) - sum(service.replicas.values())) <= len(
        clusters
    )
...
async def test_cross_cluster_healing(orchestrator):
    ...
    env = Environment(
        "env1", "Env 1", EnvironmentType.CLOUD_AWS, "us-east-1", {}, [], "healthy", {}
    )
    ...
    healing_context = HealingContext(
        ...
        constraints={
            "patch": {
                "spec": {"template": {"metadata": {"labels": {"fix": "applied"}}}}
            }
        },
        priority=1,
        timestamp=datetime.utcnow(),
    ...
    healing_plan = HealingPlan(
        ...
        steps=[
            HealingStep(
                "step1", env, "patch", {"service": "api-service"}, [], 60, False, None
            )
        ],
        rollback_steps=[],
    ...
    result = await orchestrator.perform_cross_cluster_healing(
...
async def test_service_mesh_controller_istio():
    ...
    result = await controller.configure_traffic_split(
        service, {"cluster-1": 0.6, "cluster-2": 0.4}
    )
    assert result is True

    # Test circuit breaking
    result = await controller.enable_circuit_breaking(
        service, {"consecutive_errors": 5, "interval_seconds": 30}
    )
    assert result is True
...
async def test_get_cluster_status(orchestrator):
    ...
    connector.get_service_status = AsyncMock(
        return_value={
            "exists": True,
            "ready": 3,
            "desired": 3,
...
async def test_get_cross_cluster_view(orchestrator):
    ...
    orchestrator.get_cluster_status = AsyncMock(
        return_value={
...
async def test_traffic_rebalancing(orchestrator):
    ...
    expected_split = {
        c: service.replicas.get(c, 0) / total_replicas for c in active_clusters
    }
