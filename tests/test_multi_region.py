...
async def test_region_health_monitor(mock_regions):
    ...
    monitor._check_environment_health = AsyncMock(
        return_value={
            "availability": 95.0,
            "latency_ms": 50.0,
            "error_rate": 0.02,
...
async def test_multi_region_healing_coordination(strategy):
    ...
    env2 = Environment(
        "env2", "Env 2", EnvironmentType.CLOUD_AWS, "us-west-2", {}, [], "healthy", {}
    )
    ...
    strategy._coordinate_synchronous_healing = AsyncMock(
        return_value={
            "status": "synchronous",
            "regions": {
                "us-east-1": {"status": "success"},
                "us-west-2": {"status": "success"},
            },
        }
    )
...
def test_optimal_region_selection(strategy):
    ...
    optimal = strategy.get_optimal_region_for_location(
        seattle_location, available_regions=["us-east-1", "eu-west-1"]
    )
...
async def test_resilience_status_generation(strategy):
    ...
    # Mock health monitor
    strategy.health_monitor.check_region_health = AsyncMock(
...
def test_recommendation_generation(strategy):
    ...
    status = {
        ...
        "health_trends": {"us-east-1": {"trend": "degrading"}},
    }
...
async def test_auto_failback():
    ...
    with patch("modules.deployment.multi_environment.multi_region.SecurityAuditor"):
        ...
        orchestrator.execute_failover = AsyncMock(
            return_value=FailoverEvent(
                event_id="failback-1",
                timestamp=datetime.utcnow(),
