...
def mock_config():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        return {
            ...
            "providers": [{"name": "local", "type": "local", "path": tmpdir}],
            "templates": [
                {
                    "id": "web-service",
...
def config_manager(mock_config):
    ...
    with patch(
        "modules.deployment.multi_environment.config_manager.AuditLogger",
        return_value=mock_audit_logger,
    ):
        with patch(
            "modules.deployment.multi_environment.config_manager.DistributedMonitor"
        ):
...
def test_secret_manager():
    ...
    encrypted_values = [old_manager.encrypt("secret1"), old_manager.encrypt("secret2")]
...
async def test_set_config_with_validation(config_manager):
    ...
    validation_rules = {"type": "number", "range": {"min": 1, "max": 100}}
    ...
    with pytest.raises(ValueError, match="Validation failed"):
        await config_manager.set_config(
            "app.threads", 200, "production", validation_rules=validation_rules
        )
...
async def test_secret_encryption(config_manager):
    ...
    encrypted = await config_manager.get_config(
        "database.password", "test", decrypt_secrets=False
    )
    ...
    decrypted = await config_manager.get_config(
        "database.password", "test", decrypt_secrets=True
    )
...
async def test_config_references(config_manager):
    ...
    await config_manager.set_config(
        "app.port", {"_type": "reference", "_ref": "defaults.port"}, "staging"
    )
...
async def test_detect_drift(config_manager):
    ...
    config_manager._get_expected_configs = AsyncMock(
        return_value={"app.version": "1.0.0", "app.replicas": 3, "app.timeout": 30}
    )

    config_manager._get_actual_configs = AsyncMock(
        return_value={
...
async def test_promote_config(config_manager):
    ...
    result = await config_manager.promote_config(
        "development", "staging", keys=["app.version", "app.features.newFeature"]
    )
...
async def test_promote_config_dry_run(config_manager):
    ...
    result = await config_manager.promote_config(
        "development", "production", dry_run=True
    )
...
async def test_rollback_config(config_manager):
        ...
        side_effect=[
            ...
            [
                ...
                },
                {
                    "environment": "staging",
                    "value": "1.0.0",
                    "modified": (timestamp - timedelta(seconds=1)).isoformat(),
                },
            ],
        ]
    )
    ...
    assert any(
        c["key"] == "app.newkey" and c["action"] == "removed" for c in result["changes"]
    )
    # Version should be rolled back
    assert any(
        c["key"] == "app.version" and c["action"] == "rollback"
...
async def test_pending_changes_approval(config_manager):
    ...
    change = await config_manager.set_config(
        "critical.setting", "value", "production", reason="Critical production change"
    )
    ...
    dev_change = await config_manager.set_config(
        "test.setting", "value", "development", reason="Dev change"
    )

    assert dev_change.change_id not in [
        c.change_id for c in config_manager.pending_changes
    ]
...
async def test_get_config_summary(config_manager):
    ...
    await config_manager.set_config(
        "database.password", "secret", "staging", encrypt_secret=True
    )
...
def test_template_rendering(config_manager):
    ...
    variables = {"host": "localhost", "port": 8080, "replicas": 3}
...
def test_drift_severity_calculation(config_manager):
    ...
    severity = config_manager._calculate_drift_severity(
        [{"key": "database.password", "type": "mismatch"}], "staging"
    )
    assert severity == "critical"

    # High - production environment
    severity = config_manager._calculate_drift_severity(
        [{"key": "app.version", "type": "mismatch"}], "production"
    )
    assert severity == "high"

    # Medium - many changes
    severity = config_manager._calculate_drift_severity(
        [{"key": f"app.setting{i}", "type": "mismatch"} for i in range(15)], "staging"
    )
    assert severity == "medium"

    # Low - few non-critical changes
    severity = config_manager._calculate_drift_severity(
        [{"key": "app.timeout", "type": "mismatch"}], "development"
    )
    assert severity == "low"
