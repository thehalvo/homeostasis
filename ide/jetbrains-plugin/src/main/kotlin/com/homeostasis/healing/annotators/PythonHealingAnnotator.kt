...
/**
 * External annotator for Python code healing
 */
class PythonHealingAnnotator : ExternalAnnotator<PsiFile, List<HealingSuggestion>>() {
    
    companion object {
        private val LOG = Logger.getInstance(PythonHealingAnnotator::class.java)
    }
    ...
    override fun collectInformation(file: PsiFile, editor: Editor, hasErrors: Boolean): PsiFile? {
        ...
        if (!file.language.id.equals("Python", ignoreCase = true)) {
            return null
        }
    ...
    override fun doAnnotate(collectedInfo: PsiFile?): List<HealingSuggestion>? {
        ...
        return try {
            ...
            LOG.error("Error analyzing Python file: ${collectedInfo.name}", e)
            null
        }
    }
    ...
    override fun apply(
        ...
        for (suggestion in annotationResult) {
            try {
                ...
                LOG.error("Error creating annotation for Python suggestion: ${suggestion.id}", e)
            }
        }
    }
}