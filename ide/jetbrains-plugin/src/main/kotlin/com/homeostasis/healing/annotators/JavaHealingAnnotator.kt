...
/**
 * External annotator for Java code healing
 */
class JavaHealingAnnotator : ExternalAnnotator<PsiFile, List<HealingSuggestion>>() {
    
    companion object {
        private val LOG = Logger.getInstance(JavaHealingAnnotator::class.java)
    }
    ...
    override fun collectInformation(file: PsiFile, editor: Editor, hasErrors: Boolean): PsiFile? {
        ...
        if (!file.language.id.equals("JAVA", ignoreCase = true)) {
            return null
        }
    ...
    override fun doAnnotate(collectedInfo: PsiFile?): List<HealingSuggestion>? {
        ...
        return try {
            ...
            LOG.error("Error analyzing Java file: ${collectedInfo.name}", e)
            null
        }
    }
    ...
    override fun apply(
        ...
        for (suggestion in annotationResult) {
            try {
                ...
                LOG.error("Error creating annotation for Java suggestion: ${suggestion.id}", e)
            }
        }
    }
}