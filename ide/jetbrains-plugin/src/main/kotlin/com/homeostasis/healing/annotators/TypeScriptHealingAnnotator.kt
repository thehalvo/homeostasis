...
/**
 * External annotator for TypeScript code healing
 */
class TypeScriptHealingAnnotator : ExternalAnnotator<PsiFile, List<HealingSuggestion>>() {
    
    companion object {
        private val LOG = Logger.getInstance(TypeScriptHealingAnnotator::class.java)
    }
    ...
    override fun collectInformation(file: PsiFile, editor: Editor, hasErrors: Boolean): PsiFile? {
        ...
        if (!file.language.id.equals("TypeScript", ignoreCase = true)) {
            return null
        }
    ...
    override fun doAnnotate(collectedInfo: PsiFile?): List<HealingSuggestion>? {
        ...
        return try {
            ...
            LOG.error("Error analyzing TypeScript file: ${collectedInfo.name}", e)
            null
        }
    }
    ...
    override fun apply(
        ...
        for (suggestion in annotationResult) {
            try {
                ...
                LOG.error("Error creating annotation for TypeScript suggestion: ${suggestion.id}", e)
            }
        }
    }
}