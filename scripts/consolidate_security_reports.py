...
"""
        ...
        for vuln in sorted(filtered_vulns, key=lambda x: ['critical', 'high', 'medium', 'low', 'info'].index(x.get('severity', 'medium').lower())):
            severity = vuln.get('severity', 'medium').lower()
            location = ''
            if vuln.get('file'):
                ...
                location = f"<span class='file-path'>{html.escape(vuln['package'])}</span>"
...
def main():
    ...
    parser.add_argument('--format', '-f', choices=['html', 'json'], default='json', help='Output format')
    parser.add_argument('--severity-threshold', '-s', default='low', 
                       choices=['info', 'low', 'medium', 'high', 'critical'],
                       help='Minimum severity to include in report')
    parser.add_argument('--report-pattern', '-p', default='*-report.json',
    ...
    if summary['by_severity']['critical'] > 0:
        print(f"❌ Critical vulnerabilities found: {summary['by_severity']['critical']}")
        exit(1)
