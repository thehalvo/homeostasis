...
def generate_comparison_report(baseline_db: str, current_db: str) -> str:
    ...
    for test_name in sorted(set(list(baseline_data.keys()) + list(current_metrics.keys()))):
        ...
        # Calculate changes
        duration_change, duration_dir = calculate_change(baseline['mean_duration'], avg_duration)
        memory_change, memory_dir = calculate_change(baseline['mean_memory'], avg_memory)
        ...
        # Format row
        duration_str = f"{avg_duration*1000:.1f}ms {duration_dir} {duration_change:+.1f}%"
        memory_str = f"{avg_memory:.1f}MB {memory_dir} {memory_change:+.1f}%"
        cpu_str = f"{avg_cpu:.1f}% {cpu_dir} {cpu_change:+.1f}%"
        
        report += f"| {test_name} | {duration_str} | {memory_str} | {cpu_str} | {status} |\n"
    ...
    if improvements:
        ...
        for imp in improvements:
            report += f"- **{imp['test']}**: Duration {imp['duration_change']:.1f}% faster\n"
...
def generate_json_report(baseline_db: str, current_db: str, output_path: str):
    ...
    for test_name in sorted(set(list(baseline_data.keys()) + list(current_metrics.keys()))):
        ...
        if baseline:
            ...
            # Calculate changes
            duration_change = ((test_data["current"]["duration"] - baseline['mean_duration']) 
                              / baseline['mean_duration'] * 100)
            
            test_data["changes"] = {
                "duration": duration_change,
                "memory": ((test_data["current"]["memory"] - baseline['mean_memory']) 
                          / (baseline['mean_memory'] + 0.001) * 100),
                "cpu": ((test_data["current"]["cpu"] - baseline['mean_cpu']) 
                       / (baseline['mean_cpu'] + 0.001) * 100)
            }
            ...
            if duration_change > 20:
                report["regressions"].append({
                    ...
                    "regression_factor": test_data["current"]["duration"] / baseline['mean_duration']
                })
            elif duration_change < -10:
                report["improvements"].append({
                    "test_name": test_name,
                    "improvement_percent": -duration_change
                })
...
def main():
    ...
    parser.add_argument('--output', default='performance_comparison.md', 
    ...
    if args.format in ['markdown', 'both']:
        ...
        if args.format == 'both':
            output_path = args.output.replace('.md', '.md') if args.output.endswith('.md') else args.output + '.md'
