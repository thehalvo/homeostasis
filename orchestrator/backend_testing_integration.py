...
from modules.analysis.enhanced_cross_language_orchestrator import EnhancedCrossLanguageOrchestrator
from modules.analysis.shared_rule_system import shared_rule_registry, initialize_shared_rules
...
class BackendTestingManager:
    ...
    def __init__(self, orchestrator: Orchestrator, 
        ...
        if test_dir is None:
            test_dir = Path(os.path.dirname(os.path.abspath(__file__))) / ".." / "tests" / "test_cases"
    ...
    def initialize(self) -> None:
        ...
        logger.info(f"Backend testing initialized with {len(supported_languages)} languages")
    
    def run_backend_tests(self, suite_name: Optional[str] = None,
        ...
        for name, suite in suites_to_run.items():
            ...
            if language:
                ...
                if not language_cases:
                    logger.warning(f"No test cases for language {language} in suite {name}")
                    continue
            ...
            results["suites"][name] = {
                "total": suite_total,
                "passed": suite_passed,
                "pass_percentage": (suite_passed / suite_total * 100) if suite_total > 0 else 0,
    ...
    def run_cross_language_tests(self, suite_name: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        for name, suite in suites_to_run.items():
            ...
            suite_successful_conv = sum(1 for r in cross_results["conversion_tests"] if r.get("passed"))
            ...
            suite_successful_cf = sum(1 for r in cross_results["cross_suggestion_tests"] 
            ...
            self.metrics["cross_language"]["conversions"]["successful"] += suite_successful_conv
            ...
            results["suites"][name] = {
                "conversions": {
                    ...
                    "success_percentage": (suite_successful_conv / suite_conversions * 100) 
                                        if suite_conversions > 0 else 0,
        ...
        results["summary"] = {
            "conversions": {
                ...
                "success_percentage": (successful_conversions / total_conversions * 100) 
    ...
    def validate_fix(self, error_data: Dict[str, Any], 
                    fix_data: Dict[str, Any],
                    language: str) -> Tuple[bool, Dict[str, Any]]:
        """
    ...
    def generate_regression_test(self, error_data: Dict[str, Any],
                               fix_data: Dict[str, Any],
                               language: str,
                               test_name: Optional[str] = None) -> LanguageTestCase:
        """
        ...
        if not test_name:
            ...
            test_name = f"{language.capitalize()}_{sanitized_type}_Test_{int(time.time())}"
    ...
    def add_regression_test(self, test_case: LanguageTestCase,
                          suite_name: str = "regression") -> None:
        """
        ...
        if suite_name not in self.test_suites:
            self.test_suites[suite_name] = LanguageTestSuite(suite_name, "Regression tests")
    ...
    def track_fix_attempt(self, error_data: Dict[str, Any],
                        fix_data: Dict[str, Any],
                        language: str,
                        success: bool) -> None:
        """
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        metrics["fixes"]["success_rate"] = (
            ...
            if metrics["fixes"]["total_attempted"] > 0 else 0
        )
        
        metrics["tests"]["pass_rate"] = (
            (metrics["tests"]["passed"] / metrics["tests"]["total_run"]) * 100
            if metrics["tests"]["total_run"] > 0 else 0
        )
        
        metrics["cross_language"]["conversions"]["success_rate"] = (
            (metrics["cross_language"]["conversions"]["successful"] / 
             metrics["cross_language"]["conversions"]["total"]) * 100
            if metrics["cross_language"]["conversions"]["total"] > 0 else 0
        )
        
        metrics["cross_language"]["fixes"]["success_rate"] = (
            (metrics["cross_language"]["fixes"]["successful"] / 
             metrics["cross_language"]["fixes"]["total"]) * 100
            if metrics["cross_language"]["fixes"]["total"] > 0 else 0
        )
        ...
        for file_path in self.test_dir.glob("*.json"):
            try:
                ...
                if suite.test_cases:
                    ...
                    logger.info(f"Loaded test suite: {file_path.stem} with {len(suite.test_cases)} test cases")
    ...
    def _check_suggestion_relevance(self, suggestion: str, 
                                  error_type: str, 
                                  language: str) -> bool:
        """
        ...
        if language == "python":
            ...
            elif "attributeerror" in error_type_lower and "nonetype" in error_type_lower:
...
if __name__ == "__main__":
    ...
    logger.info(f"  Conversion success rate: {metrics['cross_language']['conversions']['success_rate']:.1f}%")
    logger.info(f"  Cross-fix success rate: {metrics['cross_language']['fixes']['success_rate']:.1f}%")
