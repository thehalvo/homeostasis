...
class HomeostasisClient:
    ...
    def get_canary(
        self, service_name: Optional[str] = None, fix_id: Optional[str] = None
    ) -> Dict:
    ...
    def approve_fix(self, approval_id: str, comment: Optional[str] = None) -> bool:
        ...
        try:
            response = self._post_api_resource(
                f"/api/approvals/{approval_id}/approve", data
            )
    ...
    def reject_fix(self, approval_id: str, reason: Optional[str] = None) -> bool:
        ...
        try:
            response = self._post_api_resource(
                f"/api/approvals/{approval_id}/reject", data
            )
            return response.get("success", False)
        except APIError:
            return False
