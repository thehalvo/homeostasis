...
from .microservice_healer import ServiceInfo, ServiceCommunicationProtocol
    ...
    async def inject_fault(
        ...
        if fault_type == "delay":
            fault_config = {
                "delay": {
                    "percentage": {
                        "value": parameters.get('percentage', 10)
                    },
...
class LinkerdAdapter(ServiceMeshAdapter):
    ...
    async def enable_canary_deployment(
        ...
        traffic_split = {
            ...
            "spec": {
                ...
                "backends": [
                    {
                        "service": f"{service_name}-stable",
                        "weight": 100 - percentage
                    },
                    {
                        "service": f"{service_name}-canary",
                        "weight": percentage
                    }
                ]
            }
    ...
    async def inject_fault(
        ...
        self.logger.warning(
            "Fault injection not natively supported in Linkerd"
        )
        return False
...
class ServiceMeshIntegration:
    ...
    async def analyze_distributed_trace(
        self,
        trace_id: str
    ) -> Dict[str, Any]:
        ...
        analysis = {
            "trace_id": trace_id,
            "total_duration": sum(span.get("duration", 0) for span in trace_data.get("spans", [])),
            "services_involved": [span.get("service") for span in trace_data.get("spans", [])],
            "errors": [
                span for span in trace_data.get("spans", [])
