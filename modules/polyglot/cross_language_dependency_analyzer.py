...
class CrossLanguageDependencyAnalyzer:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.dependency_graph = DependencyGraph(
            nodes={}, edges=[], api_contracts={}, shared_structures={}
        )
    ...
    async def analyze_project(self, project_root: Path) -> DependencyGraph:
        ...
        """
        self.logger.info(
            f"Starting cross-language dependency analysis for: {project_root}"
        )
        ...
        for path in project_root.rglob("*"):
            if path.is_file():
                for language, marker_files in markers.items():
                    if any(
                        path.name == marker or path.match(marker)
                ...
                for js_file in component_path.rglob("*.js"):
                    ...
                    for method, route in express_routes:
                        self._add_api_endpoint(
                            component_id, route, method.upper(), "javascript"
                        )
    ...
    async def _analyze_csharp_dependencies(self, component: Dict[str, Any]) -> None:
        ...
        for cs_file in component_path.rglob("*Controller.cs"):
            ...
            for method, endpoint in aspnet_endpoints:
                self._add_api_endpoint(
                    component_id, endpoint, method.replace("Http", "").upper(), "csharp"
                )
            ...
            if "HttpClient" in content:
                ...
                for method, url in http_calls:
                    self._add_outgoing_api_call(
                        component_id, url, method.replace("Async", "").upper()
                    )
            ...
            if "rails" in content:
                ...
                if routes_file.exists():
                    ...
                    for method, route in rails_routes:
                        self._add_api_endpoint(
                            component_id, route, method.upper(), "ruby"
                        )
            ...
            if "laravel/framework" in requires:
                ...
                if routes_path.exists():
                    ...
                    for method, route in laravel_routes:
                        self._add_api_endpoint(
                            component_id, route, method.upper(), "php"
                        )
        ...
        for node_id, node_data in self.dependency_graph.nodes.items():
            ...
            for proto_file in path.rglob("*.proto"):
                ...
                for message_name in messages:
                    ...
                    self.dependency_graph.shared_structures[message_name] = (
                        shared_struct
                    )
            ...
            for spec_file in path.rglob("*swagger*.json"):
                ...
                if "definitions" in spec_data:
                    for def_name, def_schema in spec_data["definitions"].items():
                        ...
                        self.dependency_graph.shared_structures[def_name] = (
                            shared_struct
                        )

    def _add_api_endpoint(
        self, component_id: str, endpoint: str, method: str, language: str
    ) -> None:
        ...
        self.dependency_graph.nodes[component_id]["endpoints"].append(
            {"path": endpoint, "method": method, "language": language}
        )

    def _add_outgoing_api_call(self, component_id: str, url: str, method: str) -> None:
    ...
    def _add_protocol_usage(self, component_id: str, protocol: APIProtocol) -> None:
        ...
        if protocol.value not in self.dependency_graph.nodes[component_id]["protocols"]:
            self.dependency_graph.nodes[component_id]["protocols"].append(
                protocol.value
            )

    def _add_database_dependency(self, component_id: str, db_type: str) -> None:
    ...
    def _matches_endpoint(self, call_url: str, endpoint_path: str) -> bool:
        ...
        endpoint_base = (
            endpoint_path.split("/")[-1] if "/" in endpoint_path else endpoint_path
        )
    ...
    async def get_impact_analysis(
        self, component_id: str, change_type: str = "api"
    ) -> Dict[str, Any]:
        """
        ...
        return {
            ...
            "recommendations": self._generate_impact_recommendations(
                component_id, affected_components, change_type
            ),
        }

    def _generate_impact_recommendations(
        self, component_id: str, affected: Set[str], change_type: str
    ) -> List[str]:
