...
class TwoPhaseCommitCoordinator(TransactionCoordinator):
    ...
    async def prepare(self, transaction: DistributedTransaction) -> bool:
        ...
        for i, result in enumerate(results):
            ...
            if isinstance(result, Exception) or not result:
                ...
                participant.error = ServiceError(
                    ...
                    message=str(result) if isinstance(result, Exception) else "Prepare failed",
                    stack_trace=None,
        ...
        transaction.state = TransactionState.PREPARED if all_prepared else TransactionState.FAILED
        return all_prepared
    ...
    async def commit(self, transaction: DistributedTransaction) -> bool:
        ...
        transaction.state = TransactionState.COMMITTED if all_committed else TransactionState.FAILED
    ...
    async def abort(self, transaction: DistributedTransaction) -> bool:
        ...
        abort_tasks = []
        for participant in transaction.participants:
            if participant.state in [TransactionState.PREPARED, TransactionState.COMMITTED]:
        ...
        transaction.state = TransactionState.ABORTED if all_aborted else TransactionState.FAILED
        return all_aborted
    ...
    async def get_recovery_plan(
        ...
        affected = [p for p in transaction.participants if p.state != TransactionState.COMMITTED]
        ...
        if transaction.state == TransactionState.PREPARING:
            ...
            committed = [p for p in transaction.participants if p.state == TransactionState.COMMITTED]
            if committed:
        ...
        return TransactionRecoveryPlan(
            ...
            risk_assessment={
                'data_inconsistency_risk': len(committed) > 0 if 'committed' in locals() else False,
                'recovery_complexity': 'high' if strategy == CompensationStrategy.COMPENSATE else 'medium'
            }
        )
...
class SagaCoordinator(TransactionCoordinator):
    ...
    async def abort(self, transaction: DistributedTransaction) -> bool:
        ...
        return await self._compensate_saga(
            transaction, 
            len(transaction.participants) - 1
        )
...
class DistributedTransactionHealer:
    ...
    async def generate_healing_recommendations(
        self,
        failed_transaction: DistributedTransaction
    ) -> List[Dict[str, Any]]:
        ...
        for anomaly in anomalies:
            if anomaly['type'] == 'stuck_transaction':
                recommendations.append({
                    ...
                    'suggested_timeout': failed_transaction.timeout.total_seconds() * 2
                })
