...
class HealingOrchestrator:
                    ...
                    if service in errors_by_service:
                        ...
                        if engine:
                            ...
                            future = executor.submit(
                                self._heal_service, service, engine, service_errors
                            )
                            futures[service] = future
                        else:
                            logger.warning(
                                f"No healing engine registered for service: {service}"
                            )
    ...
    def _heal_service(
        self, service_name: str, engine: HealingEngine, errors: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
        ...
        results = []
        for error in errors:
            context = {"service_name": service_name, "orchestration": True}
