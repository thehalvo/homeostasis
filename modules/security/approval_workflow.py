...
class ApprovalWorkflowEngine:
    ...
    def create_workflow_template(self, name: str, description: str, category: str,
        ...
        for stage in workflow_stages:
            ...
            if stage.escalation_stage and stage.escalation_stage not in stage_ids:
                raise ValueError(f"Invalid escalation_stage reference: {stage.escalation_stage}")
    ...
    def initiate_workflow(self, template_id: str, request_id: str,
                         initiated_by: str, metadata: Dict = None) -> str:
    ...
    def process_approval_decision(self, instance_id: str, approver_id: str,
        ...
        current_stage = next((s for s in template.stages if s.stage_id == instance.current_stage), None)
        ...
        has_permission = (
            approver_id in current_stage.approver_users or
            any(role in current_stage.approver_roles for role in user['roles'])
        )
        
        if not has_permission:
            raise ValueError(f"User {approver_id} is not authorized to approve this stage")
    ...
    def get_pending_approvals(self, user_id: str) -> List[Dict]:
        ...
        for instance in self.instances.values():
            ...
            can_approve = (
                user_id in current_stage.approver_users or
                any(role in current_stage.approver_roles for role in user['roles'])
            )
            ...
            stage_decisions = [
                d for d in self.decisions.get(instance.instance_id, [])
        ...
        for instance in self.instances.values():
            ...
            if instance.sla_deadline:
                ...
                if now > deadline:
                    ...
                    self._trigger_escalation(instance.instance_id, EscalationTrigger.SLA_BREACH)
    ...
    def get_workflow_status(self, instance_id: str) -> Dict:
        ...
        current_stage_decisions = [
            d for d in self.decisions.get(instance_id, [])
    ...
    def _start_stage(self, instance_id: str, stage_id: str):
        ...
        if stage.timeout_hours:
            deadline = datetime.datetime.utcnow() + datetime.timedelta(hours=stage.timeout_hours)
    ...
    def _evaluate_risk_level(self, instance_id: str, stage: WorkflowStage) -> bool:
        ...
        request_metadata = approval_request.data if hasattr(approval_request, 'data') else {}
