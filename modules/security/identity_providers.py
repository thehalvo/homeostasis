...
class UserIdentity:
    ...
    last_authenticated: datetime.datetime = field(
...
class IdentityProviderIntegration:
    ...
    def configure_provider(
        ...
        self.audit_logger.log_event(
            ...
            user="system",
            details={"provider_id": provider_id, "name": name, "type": type.value},
        )
    ...
    def complete_authentication(
        self, session_id: str, callback_data: Dict
    ) -> Dict[str, Any]:
    ...
    def authenticate_ldap(
        ...
        for provider in sorted(ldap_providers, key=lambda p: p.priority):
            ...
            try:
                ...
                if user_identity:
                    ...
                    # Create local session
                    auth_result = self.user_management.authenticate_user(
                        local_user["username"], "ldap_authenticated"
                    )
        ...
        self.audit_logger.log_login(
            username=username, status="failure", details={"method": "ldap"}
        )
    ...
    def get_user_identities(self, user_id: str) -> List[UserIdentity]:
        ...
        for identity in self.user_identities.values():
            ...
            if identity.email and identity.email == self.user_management.users.get(
                user_id, {}
            ).get("email"):
    ...
    def _map_user_identity(
        self, identity: UserIdentity, provider: IdentityProviderConfig
    ) -> Dict:
    ...
    def _map_external_roles(
        self, external_roles: List[str], provider: IdentityProviderConfig
    ) -> List[str]:
    ...
    def _get_default_attribute_mapping(
        ...
        if provider_type == IdentityProviderType.OAUTH2:
            return {
                ...
                "full_name": "name",
                "groups": "groups",
                "roles": {"admin": "admin", "developer": "developer", "*": "user"},
            }
    ...
    def _initialize_default_providers(self):
        ...
        if "oauth2" in self.config:
            for name, oauth_config in self.config["oauth2"].items():
                self.configure_provider(
                    name=name, type=IdentityProviderType.OAUTH2, config=oauth_config
                )
        ...
        if "saml2" in self.config:
            for name, saml_config in self.config["saml2"].items():
                self.configure_provider(
                    name=name, type=IdentityProviderType.SAML2, config=saml_config
                )
        ...
        if "ldap" in self.config:
            for name, ldap_config in self.config["ldap"].items():
                self.configure_provider(
                    name=name, type=IdentityProviderType.LDAP, config=ldap_config
                )
...
class OAuth2Handler:
    ...
    def initiate_auth(
        ...
        params = {
            ...
            "state": session.state,
            "scope": " ".join(
                scope or config.get("default_scopes", ["openid", "profile", "email"])
            ),
        }
...
class SAML2Handler:
    ...
    def initiate_auth(
        ...
        # Build SSO URL
        params = {"SAMLRequest": encoded_request, "RelayState": session.state}
    ...
    def complete_auth(
        ...
        for attr in assertion.findall(".//saml:Attribute", namespaces):
            name = attr.get("Name")
            values = [
                v.text for v in attr.findall(".//saml:AttributeValue", namespaces)
            ]
...
class LDAPHandler:
    ...
    def authenticate(
        self, provider: IdentityProviderConfig, username: str, password: str
        ...
        try:
            ...
            # Search for user
            search_filter = (
                f"({config.get('username_attribute', 'sAMAccountName')}={username})"
            )
            result = conn.search_s(
                config["user_base_dn"], ldap.SCOPE_SUBTREE, search_filter, ["*"]
            )
            ...
            identity = UserIdentity(
                ...
                username=user_attrs.get(
                    mapping.get("username", "sAMAccountName"), [b""]
                )[0].decode(),
                email=user_attrs.get(mapping.get("email", "mail"), [b""])[0].decode(),
                full_name=user_attrs.get(
                    mapping.get("full_name", "displayName"), [b""]
                )[0].decode(),
                groups=groups,
                attributes={
                    k: v[0].decode() if v else "" for k, v in user_attrs.items()
                },
            )
...
class OIDCHandler(OAuth2Handler):
    ...
    def validate_config(self, config: Dict):
        ...
        if "issuer" not in config and "discovery_url" not in config:
            raise ValueError(
                "OIDC config must include either 'issuer' or 'discovery_url'"
            )
