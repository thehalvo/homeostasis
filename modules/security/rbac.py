...
class RBACManager:
    ...
    def _init_default_roles(self):
        ...
        self.roles = {
            ...
            'operator': {
                ...
                'permissions': {
                    'view_dashboard', 'view_logs', 'view_metrics',
                    'view_analyses', 'view_fixes', 'approve_fixes', 
                    'reject_fixes', 'deploy_fixes', 'view_audit_logs'
                }
            },
            'developer': {
                ...
                'permissions': {
                    'view_dashboard', 'view_logs', 'view_metrics',
                    'view_analyses', 'view_fixes', 'customize_rules',
                    'create_templates'
                }
            },
            'analyst': {
    ...
    def _load_custom_roles(self):
        ...
        for role_name, role_config in custom_roles.items():
            ...
            if role_name in self.roles and not self.config.get('allow_role_override', False):
                logger.warning(f"Role '{role_name}' already exists and cannot be overridden")
                continue
                
            # Create or update role
            self.roles[role_name] = {
                'description': role_config.get('description', f'Custom role: {role_name}'),
