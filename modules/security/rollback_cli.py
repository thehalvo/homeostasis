...
def cmd_create_snapshot(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        snapshot_id = rollback_manager.create_snapshot(
            context_id=args.context_id, file_paths=file_paths, project_root=project_root
        )
...
def cmd_undo(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        context_snapshots = [
            snapshot
            for snapshot in rollback_manager.snapshots.values()
        ...
        print(
            f"Restored {len(rollback_manager.rollback_operations[rollback_id].files_restored)} files"
        )
        ...
        if args.json:
            ...
            if summary["recent_rollbacks"]:
                ...
                for rollback in summary["recent_rollbacks"][:5]:
                    print(
                        f"  {rollback['rollback_id'][:8]}... - {rollback['trigger']} - {rollback['status']}"
                    )
...
def create_rollback_cli_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="Homeostasis Rollback Management CLI", prog="homeostasis rollback"
    )
    ...
    snapshot_parser = subparsers.add_parser(
        "create-snapshot", help="Create a snapshot before applying patches"
    )
    snapshot_parser.add_argument("context_id", help="Context ID for the patch")
    snapshot_parser.add_argument(
        "--files", "-f", nargs="+", help="List of files to include in snapshot"
    )
    snapshot_parser.add_argument(
    ...
    rollback_parser = subparsers.add_parser(
        "rollback", help="Rollback using a snapshot"
    )
    ...
    rollback_parser.add_argument("--user", help="User triggering the rollback")
    ...
    undo_parser = subparsers.add_parser(
        "undo", help="Quick undo for a specific context"
    )
    undo_parser.add_argument("context_id", help="Context ID to undo")
    undo_parser.add_argument(
        "--force", "-f", action="store_true", help="Skip confirmation prompt"
    )
    undo_parser.add_argument("--user", help="User performing the undo")
    undo_parser.set_defaults(func=cmd_undo)

    # list-snapshots command
    list_snapshots_parser = subparsers.add_parser(
        "list-snapshots", help="List available snapshots"
    )
    list_snapshots_parser.add_argument(
        "--context-id", "-c", help="Filter by context ID"
    )
    list_snapshots_parser.add_argument(
        "--limit",
        "-l",
    ...
    list_rollbacks_parser = subparsers.add_parser(
        "list-rollbacks", help="List rollback operations"
    )
    ...
    monitor_parser = subparsers.add_parser(
        "monitor", help="Monitor for issues and auto-rollback if needed"
    )
    monitor_parser.add_argument("context_id", help="Context ID to monitor")
    monitor_parser.add_argument(
        "snapshot_id", help="Snapshot ID to rollback to if issues detected"
    )
    monitor_parser.add_argument(
