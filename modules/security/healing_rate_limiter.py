...
class HealingRateLimiter:
                ...
                if current_env in env_limits:
                    for key, value in env_limits[current_env].items():
                        ...
                        if isinstance(value, (list, tuple)) and len(value) == 2:
                            ...
                            logger.warning(
                                f"Invalid environment rate limit format for {key}: {value}. Expected [count, seconds]."
                            )
        ...
        if "limits" in self.config:
            for key, value in self.config["limits"].items():
                ...
                if isinstance(value, (list, tuple)) and len(value) == 2:
                    ...
                    logger.warning(
                        f"Invalid rate limit format for {key}: {value}. Expected [count, seconds]."
                    )
    ...
    def check_healing_cycle_limit(self) -> bool:
        ...
        min_interval = self.config.get(
            "min_healing_cycle_interval", 60
        )  # Default: 60 seconds
        if now - self.last_healing_cycle < min_interval:
            logger.warning(
                f"Minimum interval between healing cycles not met. "
        ...
        if self.action_counts["healing_cycle"][key] > limit:
            logger.warning(
                f"Healing cycle rate limit exceeded: {limit} per {window} seconds."
            )
            return False
    ...
    def check_patch_application_limit(self, count: int = 1) -> bool:
        ...
        if new_count > limit:
            logger.warning(
                f"Patch application rate limit exceeded: {limit} per {window} seconds."
            )
            return False
    ...
    def check_deployment_limit(self) -> bool:
        ...
        if self.action_counts["deployment"][key] > limit:
            logger.warning(
                f"Deployment rate limit exceeded: {limit} per {window} seconds."
            )
            return False
    ...
    def check_file_limit(self, file_path: str) -> bool:
        ...
        if file_path in self.file_cooldowns:
            ...
            if now < cooldown_until:
                ...
                logger.warning(
                    f"File {file_path} is in cooldown for {remaining} more seconds."
                )
                return False
            else:
        ...
        if self.action_counts["file"][file_path]["default"] >= limit:
            logger.warning(
                f"File modification rate limit exceeded for {file_path}: "
            ...
            top_files.append(
                {
                    ...
                    "is_critical": any(
                        file_path.startswith(c) or file_path == c
                        for c in self.critical_files
                    ),
                }
            )
