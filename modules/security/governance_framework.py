...
from .auth import get_auth_manager, AuthenticationManager
from .user_management import get_user_management, UserManagementSystem
from .approval import get_approval_manager, ApprovalManager, ApprovalType
from .approval_workflow import get_workflow_engine, ApprovalWorkflowEngine
from .audit import get_audit_logger, AuditLogger
from .compliance_reporting import get_compliance_reporting, ComplianceReportingSystem, ComplianceFramework
...
class EnterpriseGovernanceFramework:
    ...
    def __init__(self, config: Union[Dict, GovernanceConfig] = None):
        ...
        logger.info(f"Enterprise Governance Framework initialized with capabilities: "
                   f"{[c.value for c in self.config.enabled_capabilities]}")
    
    def evaluate_healing_action(self, request: HealingActionRequest) -> GovernanceDecision:
        ...
        if GovernanceCapability.COMPLIANCE_REPORTING in self.config.enabled_capabilities:
    ...
    def authenticate_user(self, username: str, password: str, 
                         method: str = "password", **kwargs) -> Optional[Dict]:
        """Authenticate a user.
    ...
    def get_user_dashboard(self, user_id: str) -> Dict:
        ...
        if GovernanceCapability.APPROVAL_WORKFLOWS in self.config.enabled_capabilities:
            dashboard['pending_approvals'] = self.workflow_engine.get_pending_approvals(user_id)
        
        # Get compliance status
        if GovernanceCapability.COMPLIANCE_REPORTING in self.config.enabled_capabilities:
            dashboard['compliance_status'] = self.compliance_reporting.get_compliance_dashboard()
    ...
    def configure_policy(self, name: str, rules: List[Dict], **params) -> str:
        ...
        Returns:
            Policy ID
        """
        from .policy_enforcement import PolicyType, PolicyScope
    ...
    def get_governance_metrics(self) -> Dict:
        ...
        metrics = {
            'users': {
                ...
                'active': len([u for u in self.user_management.users.values() 
                             if u['status'] == 'active'])
            },
            'approvals': {
    ...
    def _check_user_permissions(self, request: HealingActionRequest) -> bool:
        ...
        required_permission = required_permissions.get(request.action_type, 'patch.request')
        permissions = self.user_management.get_user_permissions(request.requested_by)
        
        return required_permission in permissions
    
    def _evaluate_policies(self, request: HealingActionRequest) -> Tuple[bool, str, Dict]:
    ...
    def _check_compliance_requirements(self, request: HealingActionRequest) -> List[str]:
        ...
        for framework in self.config.compliance_frameworks:
            ...
            if framework == ComplianceFramework.HIPAA:
                if 'patient' in request.patch_content.lower() or 'medical' in request.patch_content.lower():
                    compliance_issues.append("HIPAA: Potential PHI exposure in patch")
            
            elif framework == ComplianceFramework.PCI_DSS:
                if 'card' in request.patch_content.lower() or 'payment' in request.patch_content.lower():
                    compliance_issues.append("PCI-DSS: Potential cardholder data handling")
            
            elif framework == ComplianceFramework.SOC2:
                if request.environment == 'production' and not request.requested_by:
                    compliance_issues.append("SOC2: Production changes require authenticated user")
    ...
    def _requires_approval(self, request: HealingActionRequest, risk_level: str) -> bool:
        """Determine if approval is required."""
        # Always require approval for production if configured
        if self.config.require_approval_for_production and request.environment == 'production':
            return True
    ...
    def _initiate_approval_workflow(self, request: HealingActionRequest) -> Dict:
        ...
        if request.environment == 'production' and request.risk_assessment.get('level') == 'high':
...
def evaluate_healing_action(action_type: str, error_context: Dict, 
                          patch_content: str, **kwargs) -> GovernanceDecision:
    ...
    request = HealingActionRequest(
        request_id=kwargs.get('request_id', f"req_{datetime.datetime.utcnow().timestamp()}"),
