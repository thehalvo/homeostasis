...
class LLMSecurityManager:
    ...
    def __init__(self, config: Optional[SecurityConfig] = None):
        ...
        logger.info(
            f"LLM Security Manager initialized with {len(self.sensitive_patterns)} patterns"
        )
    ...
    def scrub_sensitive_data(
        ...
        text: str,
        context_id: Optional[str] = None,
        preserve_anonymization: bool = True,
                ...
                detections.append(
                    {
                        ...
                        "compliance_frameworks": [
                            f.value for f in pattern.compliance_frameworks
                        ],
                    }
                )
    ...
    def detect_data_leakage(
        self, prompt: str, response: str, context_id: Optional[str] = None
        ...
        if detections:
            logger.warning(
                f"Detected {len(detections)} potential data leakage incidents"
            )
            if context_id:
    ...
    def check_compliance_violations(
        self, llm_context: "LLMContext", proposed_patch: str
        ...
        for framework in self.active_compliance_frameworks:
            if framework == ComplianceFramework.HIPAA:
                violations.extend(
                    self._check_hipaa_compliance(llm_context, proposed_patch)
                )
            elif framework == ComplianceFramework.PCI_DSS:
                violations.extend(
                    self._check_pci_compliance(llm_context, proposed_patch)
                )
            elif framework == ComplianceFramework.GDPR:
                violations.extend(
                    self._check_gdpr_compliance(llm_context, proposed_patch)
                )
            elif framework == ComplianceFramework.SOX:
                violations.extend(
                    self._check_sox_compliance(llm_context, proposed_patch)
                )
    ...
    def sanitize_llm_context(self, llm_context: "LLMContext") -> "LLMContext":
        ...
        if llm_context.error_context.error_message:
            sanitized_message, _ = self.scrub_sensitive_data(
                llm_context.error_context.error_message, llm_context.context_id
            )
        ...
        if llm_context.error_context.source_code_snippet:
            sanitized_code, _ = self.scrub_sensitive_data(
                llm_context.error_context.source_code_snippet, llm_context.context_id
            )
        ...
        sanitized_context.language_specific_analysis = (
            llm_context.language_specific_analysis
        )
    ...
    def validate_llm_response_safety(
        self, response: str, context_id: Optional[str] = None
    ) -> Tuple[bool, List[str]]:
        """
    ...
    def get_security_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "active_compliance_frameworks": [
                f.value for f in self.active_compliance_frameworks
            ],
    ...
    def _log_scrubbing_activity(
        self, context_id: str, detections: List[Dict[str, Any]]
    ):
        """Log data scrubbing activity."""
        logger.info(
            f"Data scrubbing activity for context {context_id}: {len(detections)} items scrubbed"
        )

    def _log_leakage_detections(
        self, context_id: str, detections: List[DataLeakageDetection]
    ):
        """Log data leakage detections."""
        logger.warning(
            f"Data leakage detections for context {context_id}: {len(detections)} incidents"
        )
...
def create_llm_security_manager(
