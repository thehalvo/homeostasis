...
class UserManagementSystem:
    ...
    def __init__(self, config: Dict = None, storage_path: str = None):
        ...
        self.mfa_required_roles = set(
            self.config.get("mfa_required_roles", ["admin", "operator"])
        )
    ...
    def authenticate_user(
        ...
        access_token, refresh_token = self.auth_manager.generate_token(
            {"username": username, "user_id": user["user_id"], "roles": user["roles"]}
        )
            ...
            if user:
                self.audit_logger.log_logout(
                    ...
                    session_duration=int(
                        (
                            datetime.datetime.utcnow() - session.created_at
                        ).total_seconds()
                    ),
    ...
    def enforce_password_expiration(self) -> List[str]:
        ...
        for user_id, user in self.users.items():
            ...
            password_changed = datetime.datetime.fromisoformat(
