...
class GovernanceManager:
    ...
    def __init__(
        ...
        logger.info(
            f"Governance Manager initialized with {len(self.policies)} policies"
        )
    ...
    def register_user(self, username: str, email: str, roles: List[UserRole]) -> str:
        ...
        logger.info(
            f"Registered user {username} with roles: {[r.value for r in roles]}"
        )
        return user_id
    ...
    def assess_patch_risk(
        self, patch_content: str, context_metadata: Dict[str, Any]
    ) -> Tuple[PatchRiskLevel, PatchCategory]:
        """
        ...
        error_category = context_metadata.get("error_classification", {}).get(
            "category", ""
        )
    ...
    def request_patch_approval(
        ...
        risk_level, category = self.assess_patch_risk(
            patch_content, context_metadata or {}
        )
        ...
        if not policy:
            raise ValueError(
                f"No applicable policy found for category {category.value} and risk {risk_level.value}"
            )
        ...
        if not self._requires_approval(policy, requested_by):
            ...
            return self._auto_approve_patch(
                context_id, patch_content, requested_by, category, risk_level
            )
        ...
        approval_request = ApprovalRequest(
            ...
            metadata={
                ...
                "required_approver_roles": [
                    r.value for r in policy.required_approver_roles
                ],
        ...
        logger.info(
            f"Created approval request {request_id} for patch in context {context_id}"
        )
        return request_id
    ...
    def approve_patch(
        ...
        has_required_role = any(
            role in policy.required_approver_roles for role in approver.roles
        )
        if not has_required_role:
            raise ValueError(
                "Approver does not have required role for this patch category"
            )
    ...
    def reject_patch(self, request_id: str, rejector_id: str, reason: str) -> bool:
        ...
        has_required_role = any(
            role in policy.required_approver_roles for role in rejector.roles
        )
        if not has_required_role:
        ...
        logger.info(
            f"Rejected patch request {request_id} by user {rejector_id}: {reason}"
        )
        return True
        ...
        if user_id:
            ...
            if user:
                ...
                for req in pending_requests:
                    ...
                    if policy and any(
                        role in policy.required_approver_roles for role in user.roles
                    ):
    ...
    def _generate_permissions_for_roles(self, roles: List[UserRole]) -> Set[str]:
        ...
        for role in roles:
            if role == UserRole.ADMIN:
                ...
                permissions.update(["patch.request", "patch.view"])
            elif role == UserRole.REVIEWER:
                permissions.update(["patch.approve", "patch.reject", "patch.view"])
            elif role == UserRole.OPERATOR:
                permissions.update(["patch.request", "patch.view", "system.operate"])
            elif role == UserRole.VIEWER:
                permissions.update(["patch.view"])

        return permissions

    def _find_applicable_policy(
        self, category: PatchCategory, risk_level: PatchRiskLevel
    ...
    def _auto_approve_patch(
        ...
        logger.info(
            f"Auto-approved patch request {request_id} for context {context_id}"
        )
        return request_id
    ...
    def _notify_approvers(self, request: ApprovalRequest, policy: GovernancePolicy):
        ...
        logger.info(
            f"Notifying {len(approver_users)} potential approvers for request {request.request_id}"
        )
    ...
    def _create_default_policies(self):
        ...
        bugfix_policy = GovernancePolicy(
            ...
            patch_categories=[
                PatchCategory.BUG_FIX,
                PatchCategory.PERFORMANCE_IMPROVEMENT,
            ],
    ...
    def _get_recent_activity(self) -> List[Dict[str, Any]]:
        ...
        recent_requests = sorted(
            self.approval_requests.values(), key=lambda r: r.requested_at, reverse=True
        )[:10]
                ...
                with open(policies_file, "r") as f:
                    ...
                    for policy_data in policies_data:
                        policy = GovernancePolicy(
                            ...
                            patch_categories=[
                                PatchCategory(cat)
                                for cat in policy_data["patch_categories"]
                            ],
                            risk_levels=[
                                PatchRiskLevel(level)
                                for level in policy_data["risk_levels"]
                            ],
                            required_approver_roles=[
                                UserRole(role)
                                for role in policy_data["required_approver_roles"]
                            ],
                            min_approvers=policy_data.get("min_approvers", 1),
                            max_approval_time_hours=policy_data.get(
                                "max_approval_time_hours", 24
                            ),
                            allow_self_approval=policy_data.get(
                                "allow_self_approval", False
                            ),
                            require_multi_stage_approval=policy_data.get(
                                "require_multi_stage_approval", False
                            ),
                            compliance_frameworks=policy_data.get(
            ...
            policies_data = [
                {
                    ...
                    "required_approver_roles": [
                        role.value for role in policy.required_approver_roles
                    ],
                    "min_approvers": policy.min_approvers,
...
def create_governance_manager(
    config: Optional[SecurityConfig] = None, storage_dir: Optional[Path] = None
) -> GovernanceManager:
