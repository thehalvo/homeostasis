...
class SecurityConfig:
    ...
    def __init__(self, config_path: Optional[str] = None, env_prefix: str = 'HOMEOSTASIS'):
    ...
    def _ensure_keys(self) -> None:
        ...
        if 'jwt_secret' not in self.config:
            ...
            logger.warning("Generated random JWT secret. "
        ...
        if 'encryption_key' not in self.config:
            ...
            logger.warning("Generated random encryption key. "
                          "This is insecure for production environments.")
    ...
    def _validate_config(self) -> None:
        ...
        if not isinstance(self.config.get('token_expiry', 0), int) or self.config.get('token_expiry', 0) <= 0:
            raise SecurityConfigError("Invalid token expiry, must be a positive integer")
            
        # Validate refresh token expiry
        if not isinstance(self.config.get('refresh_token_expiry', 0), int) or self.config.get('refresh_token_expiry', 0) <= 0:
            raise SecurityConfigError("Invalid refresh token expiry, must be a positive integer")
    ...
    def save(self, config_path: str) -> None:
        ...
        try:
            ...
            raise SecurityConfigError(f"Error saving configuration to {config_path}: {str(e)}")
