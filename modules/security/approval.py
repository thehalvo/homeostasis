...
class ApprovalRequest:
    ...
    def __init__(
        self,
        request_id: str,
        request_type: Union[ApprovalType, str],
        requester: str,
        title: str,
        description: str,
        data: Dict,
    ...
    def to_dict(self) -> Dict:
        ...
        return {
            ...
            "request_type": (
                ...
                if isinstance(self.request_type, ApprovalType)
    ...
    def is_rejected(self) -> bool:
        ...
        """
        return len(self.rejections) > 0 or self.status == ApprovalStatus.REJECTED
    ...
    def approve(self, username: str, comment: str = None) -> bool:
        ...
        # Add approval
        now = datetime.datetime.utcnow()
        self.approvals[username] = {"timestamp": now.isoformat(), "comment": comment}
        self.updated_at = now
    ...
    def reject(self, username: str, reason: str = None) -> bool:
        ...
        # Add rejection
        now = datetime.datetime.utcnow()
        self.rejections[username] = {"timestamp": now.isoformat(), "reason": reason}
        self.updated_at = now
    ...
    def cancel(self, username: str, reason: str = None) -> bool:
        ...
        if self.status != ApprovalStatus.PENDING:
            raise ApprovalError(
                f"Cannot cancel request with status: {self.status.value}"
            )
...
class ApprovalManager:
    ...
    def __init__(self, config: Dict = None, storage_path: str = None):
        ...
        self.critical_fix_types = set(
            self.config.get(
                "critical_fix_types",
                ["database_schema", "security", "authentication", "authorization"],
            )
        )
    ...
    def _load_requests(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"Loaded {len(self.requests)} approval requests from {self.storage_path}"
            )
    ...
    def _save_requests(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"Saved {len(self.requests)} approval requests to {self.storage_path}"
            )
        except Exception as e:
    ...
    def create_request(
        ...
        get_audit_logger().log_event(
            ...
            details={
                ...
                "request_type": (
                    request_type.value
                    if isinstance(request_type, ApprovalType)
                    else request_type
                ),
                "title": title,
                "required_approvers": required_approvers,
    ...
    def list_requests(
        ...
        for request in self.requests.values():
            ...
            if request_type:
                if isinstance(request_type, ApprovalType) and isinstance(
                    request.request_type, ApprovalType
                ):
    ...
    def approve_request(
        ...
        if not rbac.has_permission(
            {"username": username, "roles": ["admin", "operator"]}, "approve_fixes"
        ):
            raise ApprovalError(
                f"User {username} does not have permission to approve requests"
            )
        ...
        get_audit_logger().log_event(
            ...
            details={
                ...
                "request_type": (
                    ...
                    if isinstance(request.request_type, ApprovalType)
    ...
    def reject_request(
        ...
        rbac = get_rbac_manager()
        if not rbac.has_permission(
            {"username": username, "roles": ["admin", "operator"]}, "reject_fixes"
        ):
            raise ApprovalError(
                f"User {username} does not have permission to reject requests"
            )
    ...
    def cancel_request(
        ...
        rbac = get_rbac_manager()
        is_admin = rbac.has_permission(
            {"username": username, "roles": ["admin"]}, "manage_config"
        )
