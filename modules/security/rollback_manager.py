...
class RollbackManager:
        ...
        for file_path in file_paths:
            try:
                ...
                if file_path_obj.exists():
                    ...
                    relative_path = file_path_obj.relative_to(project_root) if project_root else file_path_obj
    ...
    def rollback_patch(self, 
        ...
        try:
            ...
            logger.info(f"Successfully rolled back {len(restored_files)} files using snapshot {snapshot_id}")
            return rollback_id
    ...
    def monitor_and_auto_rollback(self, 
                                 context_id: str,
                                 snapshot_id: str,
                                 monitoring_duration_minutes: int = 5) -> Optional[str]:
        """
        ...
        """
        logger.info(f"Starting monitoring for context {context_id} for {monitoring_duration_minutes} minutes")
        ...
        while datetime.now() < end_time:
            try:
                ...
                if self._check_performance_degradation(baseline_metrics, current_metrics):
        ...
        logger.info(f"Monitoring completed for context {context_id} - no issues detected")
        return None
    
    def get_rollback_history(self, context_id: Optional[str] = None) -> List[RollbackOperation]:
        """
    ...
    def get_rollback_summary(self) -> Dict[str, Any]:
        ...
        successful_rollbacks = len([
            op for op in self.rollback_operations.values()
    ...
    def _validate_rollback(self, rollback_op: RollbackOperation, snapshot: BackupSnapshot) -> Dict[str, Any]:
        """Validate that rollback was successful."""
        validation_results = {
            "files_restored": len(rollback_op.files_restored),
            "files_expected": len(snapshot.file_paths),
            "all_files_restored": len(rollback_op.files_restored) == len(snapshot.file_paths),
    ...
    def _check_performance_degradation(self, baseline: Dict[str, Any], current: Dict[str, Any]) -> bool:
        ...
        if baseline_response_time > 0:
            ...
            return degradation_ratio > self.monitoring_config.max_response_time_degradation
        
        return False
    
    def _check_error_rate_spike(self, baseline: Dict[str, Any], current: Dict[str, Any]) -> bool:
            ...
            if rollbacks_file.exists():
                with open(rollbacks_file, 'r') as f:
                    ...
                    for rollback_data in rollbacks_data:
                        # Convert enum values
                        rollback_data["trigger"] = RollbackTrigger(rollback_data["trigger"])
                        rollback_data["status"] = RollbackStatus(rollback_data["status"])
...
def create_rollback_manager(config: Optional[SecurityConfig] = None,
                          storage_dir: Optional[Path] = None,
                          monitoring_config: Optional[SecurityMonitoringConfig] = None) -> RollbackManager:
