...
class SuggestionManager:
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized suggestion manager with storage directory: {self.storage_dir}"
        )
            ...
            for file_path in self.storage_dir.glob("*.json"):
                try:
                    ...
                    if suggestions:
                        ...
                        logger.debug(
                            f"Loaded {len(suggestions)} suggestions for error {error_id}"
                        )
    ...
    def generate_suggestions(
        self, error_id: str, error_data: Dict[str, Any], max_suggestions: int = 3
    ) -> List[FixSuggestion]:
        ...
        if error_id in self.suggestions:
            ...
            if len(existing_suggestions) >= max_suggestions:
                logger.info(
                    f"Already have {len(existing_suggestions)} suggestions for error {error_id}"
                )
        ...
        try:
            ...
            primary_fix = self._create_fix_suggestion(
                ...
                suggested_code=original_code.replace(
            ...
            logger.info(
                f"Generated {len(ranked_suggestions)} suggestions for error {error_id}"
            )
    ...
    def get_best_suggestion(self, error_id: str) -> Optional[FixSuggestion]:
        ...
        sorted_suggestions = sorted(
            suggestions, key=lambda s: s.ranking_score, reverse=True
        )
