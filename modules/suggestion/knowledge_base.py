...
class FixTemplate:
    ...
    def __init__(self, template_id: str, error_type: str, fix_type: str,
                pattern: str, template: str, metadata: Dict[str, Any]):
...
class KnowledgeBase:
    ...
    def __init__(self, storage_dir: Optional[str] = None):
        ...
        logger.info(f"Initialized knowledge base with storage directory: {self.storage_dir}")
            ...
            for file_path in self.storage_dir.glob("*.json"):
                try:
                    ...
                    if templates:
                        ...
                        logger.debug(f"Loaded {len(templates)} templates for error type {error_type}")
    ...
    def _save_templates(self, error_type: str) -> None:
        ...
        try:
            ...
            logger.debug(f"Saved {len(templates)} templates for error type {error_type}")
            
        except Exception as e:
            logger.error(f"Error saving templates for error type {error_type}: {e}")
            
    def add_template(self, error_type: str, fix_type: str,
                    pattern: str, template: str,
                    metadata: Dict[str, Any] = None) -> FixTemplate:
        ...
        # Create template
        template_id = f"{error_type}_{fix_type}_{len(self.get_templates(error_type)) + 1}"
    ...
    def update_template(self, template: FixTemplate) -> bool:
        ...
        for i, existing in enumerate(self.templates[error_type]):
            if existing.template_id == template_id:
                ...
                self._save_templates(error_type)
                logger.info(f"Updated template {template_id} for error type {error_type}")
                return True
                
        logger.warning(f"Template ID {template_id} not found for error type {error_type}")
        return False
        ...
        for error_type, templates in self.templates.items():
            for i, template in enumerate(templates):
                if template.template_id == template_id:
                    ...
                    self._save_templates(error_type)
                    logger.info(f"Deleted template {template_id} for error type {error_type}")
                    return True
    ...
    def find_matching_templates(self, error_type: str, code: str) -> List[FixTemplate]:
        ...
        for template in self.templates[error_type]:
            try:
                ...
                logger.warning(f"Invalid regex pattern in template {template.template_id}")
                continue
                
        return matching_templates
        
    def learn_from_fix(self, error_type: str, fix_type: str, 
                      original_code: str, fixed_code: str,
                      metadata: Dict[str, Any] = None) -> Optional[FixTemplate]:
