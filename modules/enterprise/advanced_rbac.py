...
class AdvancedRBACManager:
    ...
    def _initialize_permissions(self):
        ...
        for perm_name, perm_desc in self.base_rbac.permissions.items():
            self.permissions[perm_name] = Permission(
                name=perm_name, type=PermissionType.SYSTEM, description=perm_desc
            )
    ...
    def _initialize_roles(self):
        ...
        enterprise_roles = [
            ...
            Role(
                ...
                permissions={"manage_users", "delegate_permissions", "view_audit_logs"},
                conditions={
    ...
    def delete_custom_role(self, name: str, deleted_by: Optional[str] = None) -> bool:
        ...
        for user_assignments in self.role_assignments.values():
            if any(a.role_name == name for a in user_assignments):
                raise ValueError(
                    f"Cannot delete role '{name}' - still assigned to users"
                )
    ...
    def assign_role_with_conditions(
        self,
        user_id: str,
        role_name: str,
        assigned_by: str,
    ...
    def revoke_role(self, user_id: str, role_name: str, revoked_by: str) -> bool:
        ...
        self.role_assignments[user_id] = [
            a for a in self.role_assignments[user_id] if a.role_name != role_name
        ]
    ...
    def get_effective_permissions(
        self, user_info: Dict[str, Any], context: Optional[Dict[str, Any]] = None
    ) -> Set[str]:
    ...
    def has_permission_with_context(
        ...
        if perm_obj and perm_obj.conditions:
            if not self._evaluate_conditions(
                perm_obj.conditions,
                {"user": user_info, "resource": resource, "context": context},
            ):
                return False
    ...
    def _apply_access_policies(
        ...
        for policy in sorted_policies:
            ...
            if self._evaluate_policy_conditions(
                policy.conditions,
                {"user": user_info, "permissions": permissions, "context": context},
            ):
    ...
    def _match_context(
        self, required_context: Dict[str, Any], actual_context: Dict[str, Any]
    ) -> bool:
    ...
    def _evaluate_time_range(
        self, time_range: Dict[str, str], context: Dict[str, Any]
    ) -> bool:
    ...
    def _evaluate_ip_whitelist(
        self, whitelist: List[str], context: Dict[str, Any]
    ) -> bool:
    ...
    def _evaluate_resource_owner(
        self, owner_field: str, context: Dict[str, Any]
    ) -> bool:
    ...
    def _evaluate_data_classification(
        self, allowed_levels: List[str], context: Dict[str, Any]
    ) -> bool:
