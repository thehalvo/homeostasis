...
class Team:
    ...
    notification_channels: Dict[NotificationChannel, Dict[str, Any]] = field(default_factory=dict)
...
class TeamCollaborationHub:
        ...
        try:
            ...
            if request.is_approved():
                ...
                if request.healing_action_id:
                    await self._approval_workflow.execute_approved_action(request.healing_action_id)
            ...
            # Track metrics
            await self._metrics_collector.record_approval_decision(request, approver_id, approved)
    ...
    async def get_team_metrics(self, team_id: str, 
                             start_date: datetime, 
                             end_date: datetime) -> Optional[TeamMetrics]:
...
class TeamNotificationService:
    ...
    def _initialize_adapters(self):
        ...
        if self.config.get('teams'):
            self._channel_adapters[NotificationChannel.TEAMS] = MicrosoftTeamsIntegration(
                self.config['teams']
            )
    ...
    async def send(self, notification: Notification, 
                  recipients: Set[str],
                  teams: Dict[str, Team],
                  members: Dict[str, TeamMember]) -> bool:
                ...
                try:
                    ...
                    for recipient_id in recipients:
                        if recipient_id in members:
                            ...
                            if channel.value in member.notification_preferences.get('channels', []):
                                channel_configs.append(member.notification_preferences)
                        
                        # Also check team configurations
                        for team_id in members.get(recipient_id, TeamMember('', '', '')).teams:
                            if team_id in teams and channel in teams[team_id].notification_channels:
                                channel_configs.append(teams[team_id].notification_channels[channel])
...
class ChangeApprovalWorkflow:
    ...
    async def create_approval_request(self, healing_action: Dict[str, Any],
                                    approvers: List[str]) -> ApprovalRequest:
...
class TeamMetricsCollector:
    ...
    async def record_approval_decision(self, request: ApprovalRequest, 
    ...
    async def record_healing_action(self, team_id: str, action: Dict[str, Any]):
        ...
        if action.get('start_time') and action.get('end_time'):
            resolution_time = (action['end_time'] - action['start_time']).total_seconds()
    ...
    async def get_team_metrics(self, team_id: str, 
                             ...
                             end_date: datetime,
                             teams: Dict[str, Team],
                             members: Dict[str, TeamMember]) -> TeamMetrics:
        ...
        if team:
            for member_id in team.members:
                ...
                metrics.healing_actions_triggered += member_metrics.get('healing_actions_triggered', 0)
                metrics.healing_actions_successful += member_metrics.get('healing_actions_successful', 0)
                metrics.healing_actions_failed += member_metrics.get('healing_actions_failed', 0)
                metrics.alerts_acknowledged += member_metrics.get('alerts_acknowledged', 0)
                ...
                if member_id in self._response_times:
                    ...
                    if member_response_times:
                        avg_response = sum(member_response_times) / len(member_response_times)
                        metrics.average_response_time = max(metrics.average_response_time, avg_response)
...
class SlackIntegration:
    ...
    async def send_notification(self, notification: Notification, 
                              channel_configs: List[Dict[str, Any]]) -> bool:
        ...
        card = {
            ...
            'sections': [{
                ...
                'facts': [
                    ...
                    {
                        'name': 'System',
                        'value': 'Homeostasis Healing System'
                    }
                ],
