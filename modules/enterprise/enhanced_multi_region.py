...
class EnhancedMultiRegionFailover:
    ...
    def __init__(
        self, config: Dict[str, Any], base_strategy: MultiRegionResilienceStrategy
    ):
        ...
        self.health_check_results: Dict[str, List[Tuple[datetime.datetime, bool]]] = (
            defaultdict(list)
        )
            ...
            while region_id in self.enhanced_regions:
                try:
                    ...
                    if len(self.health_check_results[region_id]) > 1000:
                        self.health_check_results[region_id] = (
                            self.health_check_results[region_id][-1000:]
                        )
    ...
    async def _handle_region_status_change(
        self, region_id: str, previous_status: RegionStatus, new_status: RegionStatus
    ):
        """Handle region status change"""
        logger.info(
            f"Region {region_id} status changed: {previous_status.value} -> {new_status.value}"
        )
        ...
        if new_status == RegionStatus.UNHEALTHY:
            self.alert_manager.send_alert(
                ...
                {"region_id": region_id, "status": new_status.value},
                level="critical",
            )
    ...
    async def _create_failover_decision(
        self, failed_region_id: str, healthy_regions: List[EnhancedRegion]
    ) -> FailoverDecision:
        """Create failover decision"""
        ...
        rollback_plan = {
            ...
            "original_weights": {
                r.id: r.weight for r in self.enhanced_regions.values()
            },
    ...
    def _calculate_failover_confidence(
        self, failed_region: str, target_regions: List[str]
    ) -> float:
        ...
        for region_id in target_regions:
            ...
            if recent_checks:
                success_rate = sum(1 for _, healthy in recent_checks if healthy) / len(
                    recent_checks
                )
        ...
        recent_failovers = [
            f
            for f in self.failover_decisions
    ...
    async def execute_enhanced_failover(self, decision: FailoverDecision):
        ...
        try:
            logger.info(
                f"Executing failover from {decision.from_region} to {decision.to_regions}"
            )
            ...
            if not consistency_ok:
                logger.warning(
                    "Data consistency check failed, but proceeding with failover"
                )
    ...
    async def _update_traffic_distribution(self, decision: FailoverDecision):
        ...
        for region_id, region in self.enhanced_regions.items():
            if region_id == decision.from_region:
                ...
                temp_policy.region_weights[region_id] = (
                    region.weight + weight_per_target
                )
            else:
    ...
    async def _rollback_failover(self, decision: FailoverDecision):
        ...
        self.alert_manager.send_alert(
            ...
            {"decision_id": decision.decision_id, "reason": "verification_failed"},
            level="warning",
        )
    ...
    def create_traffic_policy(
        self, name: str, strategy: TrafficDistributionStrategy, **kwargs
    ) -> str:
    ...
    async def perform_canary_deployment(
        self, region_id: str, canary_percentage: float, duration_minutes: int = 30
    ) -> Dict[str, Any]:
    ...
    async def _monitor_canary_deployment(
        self, region_id: str, percentage: float, duration_minutes: int
    ) -> Dict[str, Any]:
        ...
        while datetime.datetime.utcnow() < end_time:
            ...
            metrics["response_times"].append(
                region.traffic_metrics.get("response_time", 0)
            )
        ...
        return {
            ...
            "average_error_rate": (
                statistics.mean(metrics["error_rates"]) if metrics["error_rates"] else 0
            ),
            ...
            "recommendation": (
                "proceed" if metrics["error_rates"][-1] < 0.02 else "rollback"
            ),
        }
    ...
    def get_failover_dashboard(self) -> Dict[str, Any]:
        ...
        for region_id, region in self.enhanced_regions.items():
            ...
            region_health[region_id] = {
                ...
                "capacity_usage": (
                    sum(region.capacity.values()) / len(region.capacity)
                    if region.capacity
                    else 0
                ),
            }
        ...
        recent_failovers = sorted(
            self.failover_decisions, key=lambda x: x.timestamp, reverse=True
        )[:10]
        ...
        # Data consistency
        recent_consistency = (
            self.consistency_checks[-1] if self.consistency_checks else None
        )
        ...
        return {
            ...
            "data_consistency": {
                "last_check": (
                    recent_consistency.timestamp.isoformat()
                    if recent_consistency
                    else None
                ),
                "consistent": (
                    recent_consistency.consistent if recent_consistency else None
                ),
                "max_lag_seconds": (
                    max(recent_consistency.lag_seconds.values())
                    if recent_consistency and recent_consistency.lag_seconds
                    else 0
                ),
            },
            "auto_failover_enabled": self.auto_failover_enabled,
            "healthy_regions_count": sum(
                1
                for r in self.enhanced_regions.values()
    ...
    async def shutdown(self):
        ...
        all_tasks = list(self.health_check_tasks.values()) + list(
            self.sync_monitors.values()
        )
        if all_tasks:
...
# Factory function
def create_enhanced_multi_region(
    config: Dict[str, Any], base_strategy: MultiRegionResilienceStrategy
