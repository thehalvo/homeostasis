...
class ITSMConnector(ABC):
    ...
    @abstractmethod
    def link_to_healing_action(self, ticket_id: str, healing_action_id: str, 
                             ticket_type: str = "incident") -> bool:
...
class ServiceNowConnector(ITSMConnector):
    ...
    def authenticate(self) -> bool:
        ...
        try:
            ...
            if self.auth_method == 'oauth':
                ...
                response = self._session.post(token_url, data=data, verify=self.verify_ssl)
    ...
    def create_change_request(self, change: ITSMChangeRequest) -> Tuple[bool, str]:
        ...
        try:
            ...
            data = {
                ...
                'start_date': change.scheduled_start.isoformat() if change.scheduled_start else None,
                'end_date': change.scheduled_end.isoformat() if change.scheduled_end else None,
    ...
    def update_change_request(self, change_id: str, updates: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            # Map any status updates
            if 'status' in updates and isinstance(updates['status'], ChangeRequestStatus):
    ...
    def search_incidents(self, query: Dict[str, Any]) -> List[ITSMIncident]:
        ...
        try:
            ...
            query_parts = []
            if 'status' in query:
                query_parts.append(f"state={self._map_incident_status(query['status'])}")
    ...
    def link_to_healing_action(self, ticket_id: str, healing_action_id: str, 
        ...
        try:
            ...
            logger.info(f"Linked ServiceNow {ticket_type} {ticket_id} to healing action {healing_action_id}")
            return True
    ...
    def _parse_change_request(self, data: Dict[str, Any]) -> ITSMChangeRequest:
        ...
        return ITSMChangeRequest(
            ...
            scheduled_start=datetime.fromisoformat(data['start_date']) if data.get('start_date') else None,
            scheduled_end=datetime.fromisoformat(data['end_date']) if data.get('end_date') else None,
            actual_start=datetime.fromisoformat(data['work_start']) if data.get('work_start') else None,
            actual_end=datetime.fromisoformat(data['work_end']) if data.get('work_end') else None,
...
class JiraServiceManagementConnector(ITSMConnector):
    ...
    def create_incident(self, incident: ITSMIncident) -> Tuple[bool, str]:
        ...
        try:
            ...
            if incident.healing_context:
                self._add_comment(issue_id, f"Healing Context:\n```json\n{json.dumps(incident.healing_context, indent=2)}\n```")
        ...
        try:
            ...
            if 'priority' in updates:
                ...
                if isinstance(priority, IncidentPriority):
                    data['fields']['priority'] = {"name": self._map_priority_to_jira(priority)}
    ...
    def _parse_jira_incident(self, data: Dict[str, Any]) -> ITSMIncident:
        ...
        return ITSMIncident(
            ...
            priority=priority_map.get(fields.get('priority', {}).get('name'), IncidentPriority.MEDIUM),
            status=status_map.get(fields.get('status', {}).get('name'), IncidentStatus.NEW),
    ...
    def _parse_jira_change_request(self, data: Dict[str, Any]) -> ITSMChangeRequest:
        ...
        return ITSMChangeRequest(
            ...
            status=status_map.get(fields.get('status', {}).get('name'), ChangeRequestStatus.NEW),
            assigned_to=fields.get('assignee', {}).get('displayName') if fields.get('assignee') else None,
            scheduled_start=datetime.fromisoformat(fields['duedate']) if fields.get('duedate') else None
        )
...
def create_itsm_connector(provider: str, config: Dict[str, Any]) -> Optional[ITSMConnector]:
