...
from modules.security.auth import AuthenticationError, AuthenticationManager
...
class SAMLAuthenticationManager:
    ...
    def create_authn_request(
        self, idp_id: str, relay_state: Optional[str] = None
    ) -> str:
        ...
        if not idp or not idp.enabled:
            raise AuthenticationError(
                f"Invalid or disabled identity provider: {idp_id}"
            )
    ...
    def _build_authn_request(self, request_id: str, idp: SAMLIdentityProvider) -> str:
        ...
        requested_authn_context = etree.SubElement(
            root, "{%s}RequestedAuthnContext" % SAMLP_NAMESPACE
        )
        requested_authn_context.set("Comparison", "exact")

        authn_context_class_ref = etree.SubElement(
            requested_authn_context, "{%s}AuthnContextClassRef" % SAML_NAMESPACE
        )
    ...
    def _sign_request(self, request_xml: str) -> str:
        ...
        signature = xmlsec.template.create(
            doc, xmlsec.Transform.EXCL_C14N, xmlsec.Transform.RSA_SHA256
        )
        ...
        ref = xmlsec.template.add_reference(
            signature, xmlsec.Transform.SHA256, uri="#" + doc.get("ID")
        )
    ...
    def process_saml_response(
        ...
        try:
            ...
            self.audit_logger.log_event(
                ...
                user="unknown",
                details={"error": str(e), "relay_state": relay_state},
            )
            raise AuthenticationError(f"SAML authentication failed: {str(e)}")

    def _extract_and_validate_assertion(
        self, response_doc: etree.Element
    ) -> SAMLAssertion:
        ...
        authn_instant = datetime.datetime.strptime(
            authn_statement.get("AuthnInstant"), "%Y-%m-%dT%H:%M:%SZ"
        )

        # Extract conditions
        conditions = assertion_elem.find(".//{%s}Conditions" % SAML_NAMESPACE)
        not_before = datetime.datetime.strptime(
            conditions.get("NotBefore"), "%Y-%m-%dT%H:%M:%SZ"
        )
        not_on_or_after = datetime.datetime.strptime(
            conditions.get("NotOnOrAfter"), "%Y-%m-%dT%H:%M:%SZ"
        )
        ...
        audience_elem = conditions.find(
            ".//{%s}AudienceRestriction/{%s}Audience" % (SAML_NAMESPACE, SAML_NAMESPACE)
        )
        ...
        # Extract attributes
        attributes = {}
        attribute_statement = assertion_elem.find(
            ".//{%s}AttributeStatement" % SAML_NAMESPACE
        )
        if attribute_statement is not None:
            for attribute in attribute_statement.findall(
                ...
                attr_values = []
                for value_elem in attribute.findall(
                    ".//{%s}AttributeValue" % SAML_NAMESPACE
                ):
    ...
    def _create_or_update_user(
        self, user_info: Dict[str, Any], assertion: SAMLAssertion
    ) -> Dict[str, Any]:
    ...
    def create_logout_request(self, username: str) -> Optional[str]:
        ...
        logout_request = self._build_logout_request(
            request_id, idp, session["name_id"], session["session_index"]
        )
        ...
        # Encode request
        encoded_request = base64.b64encode(logout_request.encode("utf-8")).decode(
            "utf-8"
        )
    ...
    def _build_logout_request(
        ...
        session_index_elem = etree.SubElement(
            root, "{%s}SessionIndex" % SAMLP_NAMESPACE
        )
    ...
    def generate_metadata(self) -> str:
        ...
        sp_sso = etree.SubElement(
            root, "{urn:oasis:names:tc:SAML:2.0:metadata}SPSSODescriptor"
        )
        sp_sso.set(
            "AuthnRequestsSigned", str(self.sp_config.authn_requests_signed).lower()
        )
        sp_sso.set(
        ...
        if self.sp_config.certificate:
            key_descriptor = etree.SubElement(
                sp_sso, "{urn:oasis:names:tc:SAML:2.0:metadata}KeyDescriptor"
            )
            key_descriptor.set("use", "signing")

            key_info = etree.SubElement(
                key_descriptor, "{%s}KeyInfo" % XMLDSIG_NAMESPACE
            )

            x509_data = etree.SubElement(key_info, "{%s}X509Data" % XMLDSIG_NAMESPACE)

            x509_cert = etree.SubElement(
                x509_data, "{%s}X509Certificate" % XMLDSIG_NAMESPACE
            )
        ...
        name_id_format = etree.SubElement(
            sp_sso, "{urn:oasis:names:tc:SAML:2.0:metadata}NameIDFormat"
        )
        name_id_format.text = self.sp_config.name_id_format

        # Add AssertionConsumerService
        acs = etree.SubElement(
            sp_sso, "{urn:oasis:names:tc:SAML:2.0:metadata}AssertionConsumerService"
        )
        ...
        if self.sp_config.slo_url:
            slo = etree.SubElement(
                sp_sso, "{urn:oasis:names:tc:SAML:2.0:metadata}SingleLogoutService"
            )
    ...
    def import_idp_metadata(self, metadata_xml: str, idp_id: str, name: str) -> bool:
        ...
        try:
            ...
            # Extract certificate
            x509_cert = idp_sso.find(".//{%s}X509Certificate" % XMLDSIG_NAMESPACE)
...
# Factory function
def create_saml_manager(
    config: Dict[str, Any], auth_manager: AuthenticationManager
