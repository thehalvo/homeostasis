...
class CMDBSynchronizer(ABC):
    ...
    def sync_from_homeostasis(self, discovered_items: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    def _traverse_relationships(self, ci_id: str, impact_map: Dict, 
                              visited: Set[str], current_depth: int, max_depth: int):
        ...
        for rel in relationships:
            if rel.relationship_type in [RelationshipType.USED_BY, RelationshipType.DEPENDS_ON]:
                ...
                if target_ci:
                    ...
                    self._traverse_relationships(
                        target_ci.ci_id, impact_map, visited, 
        ...
        try:
            ...
            for data in response.json()['result']:
                rel = CMDBRelationship(
                    ...
                    relationship_type=self._parse_relationship_type(data['type']['value'])
                )
    ...
    def create_relationship(self, relationship: CMDBRelationship) -> bool:
        ...
        try:
            ...
            data = {
                ...
                'type': self._map_relationship_to_servicenow(relationship.relationship_type)
            }
    ...
    def _parse_servicenow_ci(self, data: Dict[str, Any]) -> Optional[CMDBItem]:
        ...
        try:
            ...
            return CMDBItem(
                ...
                last_modified=datetime.fromisoformat(data['sys_updated_on']) if data.get('sys_updated_on') else None
            )
    ...
    def _parse_device42_ci(self, data: Dict[str, Any]) -> Optional[CMDBItem]:
        ...
        try:
            ...
            return CMDBItem(
                ...
                name=data.get('name', ''),
                ci_type=ci_type,
                status=CIStatus.ACTIVE if data.get('in_service', True) else CIStatus.INACTIVE,
                description=data.get('notes', ''),
                tags=tags,
                attributes=data.get('custom_fields', {}),
                last_modified=datetime.fromisoformat(data['last_updated']) if data.get('last_updated') else None
            )
...
def create_cmdb_synchronizer(provider: str, config: Dict[str, Any]) -> Optional[CMDBSynchronizer]:
