...
class MonitoringAdapter(ABC):
    ...
    @abstractmethod
    def get_metrics(self, query: Dict[str, Any], 
                   start_time: datetime, 
    ...
    @abstractmethod
    def get_events(self, filters: Dict[str, Any],
                  start_time: datetime,
            ...
            for monitor in monitors:
                ...
                if state in ['Alert', 'Warn', 'No Data']:
                    alert = Alert(
                        ...
                        source='datadog',
                        timestamp=datetime.fromisoformat(monitor.get('modified').replace('Z', '+00:00')),
        ...
        try:
            ...
            for event_data in data.get('events', []):
                event = Event(
                    ...
                    timestamp=datetime.fromtimestamp(event_data.get('date_happened', 0)),
                    event_type=event_data.get('alert_type', 'info'),
                    tags={tag.split(':')[0]: tag.split(':')[1] for tag in event_data.get('tags', []) if ':' in tag},
...
class PrometheusAdapter(MonitoringAdapter):
    ...
    def get_metrics(self, query: Dict[str, Any], 
        ...
        try:
            ...
            if data['status'] != 'success':
                logger.error(f"Prometheus query error: {data.get('error', 'Unknown error')}")
                return []
        ...
        try:
            ...
            for alert_data in data['data']:
                ...
                alert = Alert(
                    ...
                    severity=self._map_prometheus_severity(tags.get('severity', 'warning')),
                    ...
                    dashboard_url=alert_data.get('annotations', {}).get('dashboard_url')
                )
                alerts.append(alert)
    ...
    def send_metric(self, metric: Metric) -> bool:
        ...
        try:
            pushgateway_url = self.config.get('pushgateway_url', 'http://localhost:9091')
            ...
            response = requests.post(url, data=metric_line, 
...
class ElasticAdapter(MonitoringAdapter):
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            if self.cloud_id:
                ...
                self.base_url = f"https://{es_host}.{cluster_name}.cloud.elastic.co:9243"
        ...
        try:
            ...
            for hit in data['hits']['hits']:
                ...
                alert = Alert(
                    ...
                    status=AlertStatus.FIRING,
                    message=source.get('result', {}).get('condition', {}).get('met', ''),
    ...
    def _determine_elastic_severity(self, source: Dict[str, Any]) -> AlertSeverity:
        ...
        if 'metadata' in source and 'severity' in source['metadata']:
            ...
            return severity_map.get(source['metadata']['severity'].lower(), 
...
class NewRelicAdapter(MonitoringAdapter):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.base_url = f"https://api.{'eu.' if self.region == 'eu' else ''}newrelic.com"
        self._session = None
            ...
            for result in results:
                ...
                for key, value in result.items():
                    if key not in ['timestamp', 'beginTimeSeconds', 'endTimeSeconds']:
                        timestamp = result.get('timestamp') or result.get('beginTimeSeconds', 0)
        ...
        try:
            ...
            for violation in data.get('violations', []):
                alert = Alert(
                    ...
                    severity=self._map_new_relic_severity(violation.get('priority', 'WARNING')),
                    status=AlertStatus.FIRING if violation['opened_at'] and not violation.get('closed_at') else AlertStatus.RESOLVED,
        ...
        try:
            ...
            data = {
                "deployment": {
                    ...
                    "description": message or f"Alert {alert_id} acknowledged by Homeostasis",
                    "user": "homeostasis"
                }
            }
...
def create_monitoring_adapter(provider: str, config: Dict[str, Any]) -> Optional[MonitoringAdapter]:
