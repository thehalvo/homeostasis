...
class ContainerManager:
    ...
    def create_container(self, 
        ...
        if use_cache:
            ...
            if cache_key in self.container_cache:
                ...
                try:
                    result = subprocess.run(
                        ["docker", "inspect", "--format", "{{.State.Running}}", cached_container_id],
                        stdout=subprocess.PIPE,
                        stderr=subprocess.PIPE,
                        text=True,
                        check=True
                    ...
                    if result.stdout.strip() == "true":
                        self.logger.info(f"Reusing cached container {cached_container_id}")
                        self.containers[test_id] = cached_container_id
                        return test_id
                except subprocess.SubprocessError:
                    self.logger.info(f"Cached container {cached_container_id} no longer exists")
        ...
        try:
            ...
            self.logger.error(f"Failed to create container: {str(e)}", stderr=e.stderr if hasattr(e, 'stderr') else None)
    ...
    def copy_files(self, test_id: str, 
                  source_path: Path, 
                  container_path: str = "/app") -> bool:
        """
        ...
        try:
            ...
            self.logger.info(f"Copied {source_path} to container {container_id}:{container_path}")
            return True
            
        except subprocess.SubprocessError as e:
            self.logger.error(f"Failed to copy files: {str(e)}", stderr=e.stderr if hasattr(e, 'stderr') else None)
            return False
    ...
    def stop_container(self, test_id: str, remove: bool = True) -> bool:
        ...
        try:
            ...
            self.logger.error(f"Failed to stop container: {str(e)}", stderr=e.stderr if hasattr(e, 'stderr') else None)
            return False
...
if __name__ == "__main__":
    ...
    # Validate a patch
    validation = manager.validate_patch("example-patch-id", test_type="unit", test_command="pytest tests/test_patches.py -v")
