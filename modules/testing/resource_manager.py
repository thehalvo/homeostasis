        ...
        def _track_resources():
            ...
            while not stop_flag.is_set():
                try:
                    ...
                    self.metrics[test_id]["elapsed_time"] = (
                        time.time() - self.metrics[test_id]["start_time"]
                    )
                    ...
                    self.logger.exception(
                        e, message=f"Error tracking resources for test {test_id}"
                    )
                    time.sleep(interval)
    ...
    def stop_tracking(self, test_id: str) -> Dict[str, Any]:
        ...
        if test_id in self.metrics:
            ...
            summary = {
                ...
                "max_cpu_percent": (
                    max(metrics["cpu_usage"]) if metrics["cpu_usage"] else 0
                ),
                "avg_cpu_percent": (
    ...
    def enforce_resource_limits(
        ...
        try:
            ...
            if "cpu" in resource_limits:
                ...
                self.logger.info(
                    f"CPU limits must be enforced at the container level: {cpu_limit} cores"
                )
            ...
            if "memory" in resource_limits:
                ...
                self.logger.info(
                    f"Memory limits should be enforced at the container level: {memory_bytes} bytes"
                )
