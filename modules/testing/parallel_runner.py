...
class ParallelTestRunner:
    ...
    def __init__(self, 
        ...
        self.max_workers = max_workers or min(os.cpu_count() * 2, 8)  # Default to CPU count * 2 or 8, whichever is smaller
        ...
        self.logger.info(f"Initialized parallel test runner with {self.max_workers} workers")
    
    def _validate_patch_container(self, 
                               patch_id: str, 
                               test_type: str,
                               test_command: str,
                               timeout: int,
        ...
        if not self.use_containers:
            self.logger.warning("Container validation requested but containers are disabled")
    ...
    def _validate_patch_local(self, 
                           patch_id: str, 
                           test_command: str,
    ...
    def validate_patches(self, 
                       ...
                       test_type: str = "unit",
                       timeout: int = 60,
                       resource_limits: Dict[str, str] = None) -> List[Dict[str, Any]]:
        """
        ...
        """
        self.logger.info(f"Validating {len(patches)} patches in parallel with {self.max_workers} workers")
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            for future in concurrent.futures.as_completed(futures):
                ...
                try:
                    ...
                    self.logger.exception(e, message=f"Error validating patch {patch_id}")
    ...
    def run_graduated_tests(self, 
                          ...
                          resource_limits: Dict[str, Dict[str, str]] = None) -> Dict[str, Any]:
        """
        ...
        for test_type in test_types:
            ...
            # Get test parameters
            test_command = test_commands.get(test_type, f"pytest tests/ -m {test_type} -v")
    ...
    def validate_patches_graduated(self, 
                                patches: List[Dict[str, Any]], 
                                test_types: List[str] = ["unit", "integration", "system"],
                                test_commands: Dict[str, str] = None,
                                timeouts: Dict[str, int] = None,
                                resource_limits: Dict[str, Dict[str, str]] = None) -> List[Dict[str, Any]]:
        """
            ...
            for future in concurrent.futures.as_completed(futures):
                ...
                try:
                    ...
                    if result.get("valid", False):
                        ...
                        self.logger.warning(f"Patch {patch_id} failed at test level: {failed_level}")
...
if __name__ == "__main__":
    ...
    results = runner.validate_patches(patches, test_command="pytest tests/test_patches.py -v")
    
    print(f"Validated {len(results)} patches")
    for result in results:
        print(f"Patch {result['patch_id']}: {'valid' if result['valid'] else 'invalid'}")
    
    # Run graduated testing for a patch
    graduated_result = runner.run_graduated_tests(patches[0])
    
    print(f"Graduated testing for patch {graduated_result['patch_id']}: {'passed all levels' if graduated_result['valid'] else 'failed'}")
