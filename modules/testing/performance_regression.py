...
class PerformanceRegressionDetector:
    ...
    def detect_regression(
        self, current_metric: PerformanceMetric
        ...
        if baseline.mean_duration > 0:
            ...
            z_score = abs(current_metric.duration - baseline.mean_duration) / (
                baseline.std_duration + 0.001
            )
            ...
            results.append(
                RegressionResult(
                    ...
                    details={
                        ...
                        "severity": (
                            "critical"
                            if duration_factor > self.thresholds["duration"]["critical"]
                            else "warning"
                        ),
                    },
                )
        ...
        if (
            abs(baseline.mean_memory) > 0.1
        ):  # Only check if baseline uses significant memory
            ...
            results.append(
                RegressionResult(
                    ...
                    details={
                        "severity": (
                            "critical"
                            if memory_factor > self.thresholds["memory"]["critical"]
                            else "warning"
                        )
                    },
                )
            ...
            results.append(
                RegressionResult(
                    ...
                    details={
                        "severity": (
                            "critical"
                            if cpu_factor > self.thresholds["cpu"]["critical"]
                            else "warning"
                        )
                    },
                )
    ...
    def benchmark(
        ...
        results = {
            ...
            "memory": {
                ...
                "std": (
                    statistics.stdev(m.memory_delta for m in metrics)
                    if len(metrics) > 1
                    else 0
                ),
            },
            "cpu": {
                ...
                "std": (
                    statistics.stdev(m.cpu_percent for m in metrics)
                    if len(metrics) > 1
                    else 0
                ),
            },
