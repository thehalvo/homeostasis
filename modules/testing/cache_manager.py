...
class CacheManager:
    ...
    def __init__(
        ...
        self.logger.info(
            f"Initialized cache manager with {len(self.cache_index)} cache entries"
        )
        ...
        if cache_path.exists():
            try:
                ...
                self.logger.exception(
                    e, message=f"Failed to remove cache directory: {cache_path}"
                )
    ...
    def _docker_commit(
        ...
        try:
            ...
            self.logger.info(
                f"Created image {repository}:{tag} from container {container_id}"
            )
            return image_id

        except subprocess.SubprocessError as e:
            self.logger.error(
                f"Failed to commit container: {str(e)}",
    ...
    def _docker_load(self, repository: str, tag: str) -> Optional[str]:
        ...
        try:
            ...
            self.logger.info(
                f"Created container {container_id} from image {repository}:{tag}"
            )
            return container_id
    ...
    def load_cached_container(
        self, test_type: str, patch_id: Optional[str] = None
    ) -> Optional[str]:
        """
        ...
        with self.cache_lock:
            ...
            if not matching_entries:
                self.logger.info(
                    f"No cached container found for test_type={test_type}, patch_id={patch_id}"
                )
                return None
            ...
            if container_id:
                self.logger.info(
                    f"Loaded cached container {container_id} from key {key}"
                )
                return container_id
...
class TestEnvironmentCache:
    ...
    def __init__(self, cache_dir: Optional[Path] = None, log_level: str = "INFO"):
        ...
        self.cache_dir = cache_dir or (
            project_root / "modules" / "testing" / "env_cache"
        )
        self.cache_dir.mkdir(parents=True, exist_ok=True)
        self.container_cache = CacheManager(
            self.cache_dir / "containers", log_level=log_level
        )
    ...
    def _restore_file_snapshot(self, snapshot_id: str) -> bool:
        ...
        try:
            ...
            self.logger.exception(
                e, message=f"Failed to restore snapshot {snapshot_id}"
            )
            return False
    ...
    def create_environment_snapshot(
        ...
        if container_id:
            container_cache_key = self.container_cache.cache_container(
                container_id, test_type, environment_id, metadata
            )
        ...
        if files_to_snapshot:
            file_snapshot_created = self._create_file_snapshot(
                snapshot_id, files_to_snapshot
            )
        ...
        try:
            ...
            self.logger.exception(
                e, message=f"Failed to save metadata for snapshot {snapshot_id}"
            )

        self.logger.info(f"Created environment snapshot {snapshot_id} for {test_type}")
        return snapshot_id

    def restore_environment_snapshot(
        self, snapshot_id: str, restore_files: bool = True
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            self.logger.exception(
                e, message=f"Failed to load metadata for snapshot {snapshot_id}"
            )
        ...
        if "container_cache_key" in metadata:
            container_id = self.container_cache.load_cached_container(
                metadata["test_type"], metadata.get("environment_id")
            )
    ...
    def delete_environment_snapshot(self, snapshot_id: str) -> bool:
        ...
        try:
            ...
            self.logger.exception(
                e, message=f"Failed to delete metadata for snapshot {snapshot_id}"
            )
            return False
    ...
    def list_snapshots(self, test_type: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        for metadata_file in self.cache_dir.glob("*.json"):
            try:
                ...
                self.logger.exception(
                    e, message=f"Failed to load metadata from {metadata_file}"
                )
