...
class DependencyVulnerabilityScanner(SecurityScanner):
            ...
            if stdout:
                ...
                for advisory_id, advisory in audit_data.get("advisories", {}).items():
                    vulnerability = SecurityVulnerability(
                        ...
                        cwe_ids=(
                            [f"CWE-{advisory.get('cwe', '')}"]
                            if advisory.get("cwe")
                            else []
                        ),
                        remediation=advisory.get("recommendation", ""),
                        references=(
                            [advisory.get("url", "")] if advisory.get("url") else []
                        ),
                        metadata={
                            "vulnerable_versions": advisory.get(
                                "vulnerable_versions", ""
                            ),
            ...
            if stdout:
                ...
                for warning in audit_data.get("warnings", []):
                    ...
                    vulnerability = SecurityVulnerability(
                        ...
                        cve_ids=(
                            [advisory.get("id", "")]
                            if advisory.get("id", "").startswith("CVE")
                            else []
                        ),
                        metadata={
    ...
    def _map_python_severity(self, vuln: Dict[str, Any]) -> str:
        ...
        if any(
            word in description for word in ["critical", "severe", "rce", "remote code"]
        ):
            return "critical"
...
class StaticCodeSecurityScanner(SecurityScanner):
            ...
            if stdout:
                ...
                for issue in bandit_data.get("results", []):
                    vulnerability = SecurityVulnerability(
                        ...
                        cwe_ids=(
                            [f"CWE-{issue.get('issue_cwe', {}).get('id', '')}"]
    ...
    async def _scan_javascript_code(
        self, file_path: Path
    ...
    async def _scan_typescript_code(
        self, file_path: Path
    ) -> List[SecurityVulnerability]:
    ...
    async def _scan_hardcoded_passwords(
    ...
    async def _scan_insecure_random(
        self, target_path: Path
    ) -> List[SecurityVulnerability]:
    ...
    async def _scan_xss_vulnerabilities(
        self, target_path: Path
    ) -> List[SecurityVulnerability]:
    ...
    async def _scan_insecure_deserialization(
        self, target_path: Path
    ) -> List[SecurityVulnerability]:
...
class SecurityTestOrchestrator:
            ...
            if scan_type in self.scanners:
                ...
                if scanner.is_applicable(target_path):
                    task = asyncio.create_task(
                        scanner.scan(target_path, options), name=f"scan_{scan_type}"
                    )
    ...
    def generate_security_report(
        ...
        critical_high = [
            v for v in all_vulnerabilities if v.severity in ["critical", "high"]
        ]
    ...
    async def run_continuous_security_monitoring(
        self, target_path: Path, interval_minutes: int = 60
    ) -> None:
    ...
    def _check_new_vulnerabilities(
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                if hasattr(result, "metadata"):
                    ...
                    result.metadata["security_scan"] = (
                        orchestrator.generate_security_report(scan_results)
                    )
