...
class BackupPolicy:
    ...
    retention_days: int = 30
    destinations: List[BackupDestination] = field(default_factory=lambda: [BackupDestination.LOCAL])
    include_patterns: List[str] = field(default_factory=lambda: ["**/*"])
    exclude_patterns: List[str] = field(default_factory=lambda: ["*.log", "*.tmp", "__pycache__"])
...
class BackupRestoreManager:
    ...
    def _perform_backup(
        ...
        try:
            ...
            for destination in destinations:
                self._upload_backup(backup_id, archive_path, manifest_path, destination, tenant_id)
            ...
            metadata = BackupMetadata(
                ...
                compression_ratio=total_size / archive_path.stat().st_size if total_size > 0 else 1.0,
    ...
    def restore_backup(
        ...
        try:
            ...
            try:
                ...
                archive_path = self._download_backup(backup_id, temp_dir, metadata.destinations[0])
                if not archive_path:
                ...
                if hooks:
                    hooks._send_event(
                        ...
                        tags={"backup_id": backup_id, "tenant_id": tenant_id} if tenant_id else {"backup_id": backup_id}
                    )
    ...
    def create_restore_point(
        ...
        if backup_immediately:
            ...
            source_paths = [Path(p) for p in self.config.get('default_backup_paths', ['.'])]
            ...
            for backup_id, metadata in list(self._backup_index.items()):
                ...
                if age_days > metadata.retention_days:
                    if dry_run:
                        ...
                        logger.info(f"Deleted backup {backup_id} (age: {age_days} days)")
                    
                    deleted_backups.append(backup_id)
        
        return deleted_backups
    
    def get_backup_size_summary(self, tenant_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_backup_size_mb": (total_size / backup_count / (1024 ** 2)) if backup_count > 0 else 0
        }
        ...
        try:
            if destination == BackupDestination.LOCAL:
                ...
                blob_client = self.azure_client.get_blob_client(
                    ...
                    blob=f"{prefix}/{archive_path.name}"
                )
    ...
    def _download_backup(
        ...
        try:
            ...
            if destination == BackupDestination.LOCAL:
                # Local storage
                source_path = self.local_backup_path / (tenant_id or 'default') / backup_id / archive_name
    ...
    def _delete_backup(self, backup_id: str, destination: BackupDestination):
        ...
        try:
            ...
            if destination == BackupDestination.LOCAL:
                # Local storage
                backup_dir = self.local_backup_path / (tenant_id or 'default') / backup_id
    ...
    def _find_last_full_backup(self, tenant_id: Optional[str]) -> Optional[str]:
        ...
        full_backups = [
            b for b in self._backup_index.values()
    ...
    def _filter_changed_files(
        ...
        parent_files = {
            f['path']: f for f in parent_manifest.get('files', [])
        }
    ...
    def _save_restore_point(self, restore_point: RestorePoint):
        ...
        restore_point_file = self.catalog_path / f"restore_point_{restore_point.restore_point_id}.json"
    ...
    def _load_catalog(self):
        ...
        for metadata_file in self.catalog_path.glob("*.json"):
            ...
            try:
                ...
                metadata = BackupMetadata(
                    ...
                    destinations=[BackupDestination(d) for d in data.get('destinations', ['local'])]
                )
        ...
        for restore_point_file in self.catalog_path.glob("restore_point_*.json"):
            try:
                ...
                logger.error(f"Failed to load restore point from {restore_point_file}: {e}")
    ...
    def _load_policies(self):
        ...
        for policy_config in self.config.get('backup_policies', []):
            policy = BackupPolicy(
                ...
                destinations=[BackupDestination(d) for d in policy_config.get('destinations', ['local'])],
                include_patterns=policy_config.get('include_patterns', ['**/*']),
                exclude_patterns=policy_config.get('exclude_patterns', ['*.log', '*.tmp']),
