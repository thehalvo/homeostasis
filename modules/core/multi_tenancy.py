...
class TenantConfiguration:
    ...
    allowed_languages: Set[str] = field(default_factory=lambda: {"python", "javascript", "java"})
...
class MultiTenancyManager:
    ...
    def suspend_tenant(self, tenant_id: str, reason: str) -> bool:
        ...
        return self.update_tenant(
            tenant_id,
            {
                ...
                'metadata': {'suspension_reason': reason, 'suspended_at': datetime.utcnow().isoformat()}
            }
        ) is not None
    ...
    def tenant_context(self, tenant_id: str):
        ...
        try:
            ...
            if tenant.status != TenantStatus.ACTIVE:
                raise ValueError(f"Tenant {tenant_id} is not active: {tenant.status.value}")
    ...
    def check_quota(
        ...
        if resource_type == "healing_cycle":
            ...
            if current + requested_amount > limit:
                return False, f"Healing cycle quota exceeded: {current}/{limit} per hour"
            ...
            if current + requested_amount > limit:
                return False, f"File modification quota exceeded: {current}/{limit} per hour"
            ...
            if (datetime.utcnow() - tenant.usage.last_reset_time).seconds < 60:
                if current + requested_amount > limit:
                    return False, f"API call quota exceeded: {current}/{limit} per minute"
        ...
        # Check custom limits
            ...
            if current + requested_amount > limit:
                return False, f"Custom quota '{resource_type}' exceeded: {current}/{limit}"
    ...
    def track_usage(
        ...
        with self._usage_lock:
            ...
            if resource_type == "healing_cycle":
                ...
                tenant.usage.storage_used_mb = amount  # Storage is absolute, not incremental
            elif resource_type == "cpu_seconds":
    ...
    def get_usage_summary(self, tenant_id: str) -> Dict[str, Any]:
        ...
        usage_pct = {
            "healing_cycles": (tenant.usage.healing_cycles_count / tenant.quota.max_healing_cycles_per_hour * 100),
            "patches": (tenant.usage.patches_count / tenant.quota.max_patches_per_day * 100),
            "deployments": (tenant.usage.deployments_count / tenant.quota.max_deployments_per_day * 100),
            "file_modifications": (tenant.usage.file_modifications_count / tenant.quota.max_file_modifications_per_hour * 100),
            "storage": (tenant.usage.storage_used_mb / tenant.quota.max_storage_mb * 100),
            "cpu_seconds": (tenant.usage.cpu_seconds_used / tenant.quota.max_cpu_seconds_per_hour * 100),
            "api_calls": (tenant.usage.api_calls_count / tenant.quota.max_api_calls_per_minute * 100)
        }
        ...
        for tenant_file in self.storage_path.glob("*.json"):
            try:
                ...
                tenant.configuration = TenantConfiguration(
                    ...
                    preferred_monitoring_backend=config_data.get('preferred_monitoring_backend'),
                ...
                tenant.usage = TenantUsage(
                    ...
                    file_modifications_count=usage_data.get('file_modifications_count', 0),
                    ...
                    last_reset_time=datetime.fromisoformat(usage_data.get('last_reset_time', datetime.utcnow().isoformat())),
                    period_start_time=datetime.fromisoformat(usage_data.get('period_start_time', datetime.utcnow().isoformat())),
...
def require_tenant(func: Callable) -> Callable:
    ...
    def wrapper(*args, **kwargs):
        ...
        if not tenant_id:
            raise ValueError("No tenant context found. Operations must be executed within a tenant context.")
