...
class ResourceQuotaManager:
    ...
    def allocate_resource(
        ...
        with self._usage_lock:
            ...
            # Check quota
            result = self._check_and_allocate(usage, quota, amount, operation_id)
            ...
            if usage.current_usage >= quota.limit * (
                quota.warning_threshold_percent / 100
            ):
    ...
    def get_usage_summary(
        self, entity_id: Optional[str] = None, level: QuotaLevel = QuotaLevel.TENANT
    ) -> Dict[str, Any]:
            ...
            for resource_type in ResourceType:
                ...
                if usage or quota:
                    resource_summary = {
                        ...
                        "usage_percent": (
                            (usage.current_usage / quota.limit * 100)
                            if usage and quota and quota.limit > 0
                            else 0
                        ),
                        ...
                        "enforcement_mode": (
                            quota.enforcement_mode.value if quota else None
                        ),
                    }
    ...
    def remove_quota(
        self, entity_id: Optional[str], level: QuotaLevel, resource_type: ResourceType
    ) -> bool:
    ...
    def get_quota_recommendations(
        self, entity_id: Optional[str], level: QuotaLevel, lookback_days: int = 7
    ...
    def create_rate_limiter_from_quotas(
        self, entity_id: Optional[str], level: QuotaLevel = QuotaLevel.TENANT
    ) -> HealingRateLimiter:
    ...
    def _get_applicable_policy(
        self, entity_id: Optional[str], level: QuotaLevel
    ...
    def _check_and_allocate(
        ...
        if quota.enforcement_mode == QuotaEnforcementMode.STRICT:
            ...
            if amount > remaining:
                logger.warning(
                    f"Soft quota exceeded: {amount} requested, {remaining} available"
                )
    ...
    def _send_quota_warning(
        ...
        if hooks:
            usage_percent = (
                (usage.current_usage / quota.limit * 100) if quota.limit > 0 else 0
            )
    ...
    def _monitor_usage(self):
        ...
        while True:
            try:
                ...
                with self._usage_lock:
                    for usage_key, allocations in list(
                        self._active_allocations.items()
                    ):
    ...
    def _save_policy(self, policy: QuotaPolicy):
        """Save policy to disk"""
        policy_file = (
            self.storage_path / f"policy_{policy.policy_id.replace(':', '_')}.json"
        )
            ...
            for usage_key, resources in self._usage.items():
                ...
                for resource_type, usage in resources.items():
                    usage_data[usage_key][resource_type.value] = {
                        ...
                        "burst_start": (
                            usage.burst_start.isoformat() if usage.burst_start else None
                        ),
                    }
    ...
    try:
        ...
        for resource_type, amount in allocations:
            result = quota_manager.allocate_resource(
                resource_type, amount, entity_id, level
            )
