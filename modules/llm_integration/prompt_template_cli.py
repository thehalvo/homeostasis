...
from .prompt_template_manager import PromptType, get_prompt_template_manager
    ...
    if output_json:
        ...
        for template_id in templates:
            ...
            if info:
                ...
                click.echo(
                    f"   Language: {info['language']}, Framework: {info['framework'] or 'Generic'}"
                )
                click.echo(
                    f"   Complexity: {info['complexity_level']}, Author: {info['author']}"
                )
                if info["tags"]:
...
def info(template_id: str, output_json: bool):
    ...
    if output_json:
        ...
        user_indicator = (
            "👤 User Template" if info["is_user_template"] else "🏠 Built-in Template"
        )
...
@click.option(
    "--as-builtin", is_flag=True, help="Import as built-in template (requires admin)"
)
...
def delete(template_id: str):
    ...
    if not template_id.startswith("user:"):
        click.echo(
            "❌ Can only delete user templates (template ID must start with 'user:')"
        )
        return
...
@click.option(
    "--output", "-o", type=click.Path(), help="Output file for rendered prompt"
)
...
def create():
    ...
    domain = click.prompt(
        "Domain (e.g., web_development, data_science)", default="general"
    )
...
def find(
    ...
    if template_id:
        ...
        if info:
            ...
            click.echo(
                f"   Domain: {info['domain']}, Framework: {info['framework'] or 'Generic'}"
            )
    else:
...
Please provide the corrected code and explain the fix.""",
    ...
    click.echo(
        f"  homeostasis templates test python_error_fix_example {variables_file}"
    )
