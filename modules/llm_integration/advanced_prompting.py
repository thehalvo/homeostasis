    ...
    def add_learned_example(self,
        ...
        for template in self.templates.values():
            if task_type in template.name:
                ...
                template.examples.sort(key=lambda x: x.effectiveness_score, reverse=True)
...
{coordination_requirements}""",
    ...
    def _select_strategy(self, task_type: str, context: Dict[str, Any]) -> PromptingStrategy:
    ...
    def _has_good_examples(self, task_type: str) -> bool:
        ...
        for template in self.templates.values():
            if task_type in template.name:
                good_examples = [e for e in template.examples if e.effectiveness_score > 0.7]
                return len(good_examples) >= 3
        return False
    
    def _get_template(self, task_type: str, strategy: PromptingStrategy) -> PromptTemplate:
        ...
        return PromptTemplate(
            ...
            success_criteria=["Complete the task successfully", "Maintain code quality"]
        )
...
Apply these patterns to your solution."""
    ...
    def _build_tree_of_thought_prompt(self,
                                    template: PromptTemplate,
                                    context: Dict[str, Any]) -> str:
...
""".format("\n   ".join(branches))
    ...
    def _build_self_consistency_prompt(self,
                                     template: PromptTemplate,
                                     context: Dict[str, Any]) -> str:
...
If any issues are found, revise your solution."""
    ...
    def _build_least_to_most_prompt(self,
                                  template: PromptTemplate,
                                  context: Dict[str, Any]) -> str:
...
Provide the complete solution that addresses the main problem."""
    ...
    def _build_decomposition_prompt(self,
                                  template: PromptTemplate,
                                  context: Dict[str, Any]) -> str:
...
Provide each change as a separate, clearly labeled section."""
    ...
    def _select_best_examples(self,
                            template: PromptTemplate,
                            context: Dict[str, Any]) -> List[PromptExample]:
        ...
        scored_examples.sort(key=lambda x: (x[0], x[1].effectiveness_score), reverse=True)
        
        # Return top examples
        return [ex[1] for ex in scored_examples[:self.max_examples]]
    
    def _score_example_relevance(self,
                               example: PromptExample,
        ...
        if context_tags and example_tags:
            overlap = len(context_tags & example_tags) / len(context_tags | example_tags)
            score += overlap * 0.3
...
def create_advanced_prompt_manager(config: Optional[Dict[str, Any]] = None) -> AdvancedPromptManager:
    """
