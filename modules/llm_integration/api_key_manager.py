...
from .secrets_managers import SecretsManagerError, secrets_registry
...
class APIKeyManager:
    ...
    def __init__(
        self, config_dir: Optional[Path] = None, use_external_secrets: bool = True
    ):
        """
    ...
    def set_key(self, provider: str, api_key: str, validate: bool = True) -> None:
        ...
        if self._use_external_secrets:
            ...
            if external_success:
                print(
                    f"✓ API key for {provider} saved to encrypted storage and external secrets"
                )
            else:
    ...
    def validate_key(self, provider: str, api_key: str) -> bool:
        ...
        try:
            if provider == "openai":
                ...
                raise KeyValidationError(
                    f"Validation not implemented for provider: {provider}"
                )
    ...
    def _validate_openai_key(self, api_key: str) -> bool:
        ...
        if response.status_code == 200:
            ...
            raise KeyValidationError(
                f"OpenAI API validation failed with status {response.status_code}. "
    ...
    def _validate_anthropic_key(self, api_key: str) -> bool:
        ...
        if response.status_code == 200:
            ...
            try:
                ...
                raise KeyValidationError(
                    "Anthropic API returned invalid response during validation."
                )
        else:
            raise KeyValidationError(
                f"Anthropic API validation failed with status {response.status_code}. "
                f"Please check your internet connection and try again."
            )
    ...
    def _validate_openrouter_key(self, api_key: str) -> bool:
        ...
        if response.status_code == 200:
            ...
            raise KeyValidationError(
                f"OpenRouter API validation failed with status {response.status_code}. "
                f"Please check your internet connection and try again."
            )
    ...
    def detect_key_issues(self, provider: str, api_key: str) -> List[str]:
        ...
        if provider == "openai":
            ...
            if len(api_key) < 60:
                issues.append(
                    "Anthropic key appears too short (should be 90+ characters)"
                )
            elif len(api_key) > 120:
                issues.append(
                    "Anthropic key appears too long (typically ~100 characters)"
                )
            ...
            if len(api_key) < 40:
                issues.append(
                    "OpenRouter key appears too short (should be 60+ characters)"
                )
            elif len(api_key) > 80:
                issues.append(
                    "OpenRouter key appears too long (typically ~64 characters)"
                )
    ...
    def set_openrouter_unified_mode(
        self,
        enabled: bool,
        proxy_to_anthropic: bool = True,
        proxy_to_openai: bool = True,
    ) -> None:
        """
        ...
        config["openrouter_unified_config"] = {
            ...
            "fallback_model_mapping": config.get("openrouter_unified_config", {}).get(
                "fallback_model_mapping",
                {
                    "anthropic": "anthropic/claude-3-haiku",
                    "openai": "openai/gpt-3.5-turbo",
    ...
    def set_provider_policies(
        ...
        reliability_preference: str = "high",
    ) -> None:
        """
        ...
        if cost_preference not in valid_preferences:
            raise KeyValidationError(
                f"Invalid cost preference: {cost_preference}. Valid: {valid_preferences}"
            )
        if latency_preference not in valid_preferences:
            raise KeyValidationError(
                f"Invalid latency preference: {latency_preference}. Valid: {valid_preferences}"
            )
        if reliability_preference not in valid_preferences:
            raise KeyValidationError(
                f"Invalid reliability preference: {reliability_preference}. Valid: {valid_preferences}"
            )
    ...
    def get_provider_config_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "openrouter_unified": config.get("openrouter_unified_config", {}).get(
                "enabled", False
            ),
