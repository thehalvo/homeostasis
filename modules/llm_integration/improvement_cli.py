...
@click.option(
    ...
    type=int,
    default=10,
    help="Minimum samples for pattern identification",
)
    ...
    for pattern_type, type_patterns in pattern_groups.items():
        ...
        for pattern in sorted(type_patterns, key=lambda p: p.confidence, reverse=True):
            confidence_indicator = (
                ...
                else "🟡" if pattern.confidence < 0.7 else "🟢"
            )
...
def recommend(category: Optional[str], priority: Optional[str], output_json: bool):
    ...
    if output_json:
        ...
        for prio in ["high", "medium", "low"]:
            ...
            priority_icon = (
                "🔴" if prio == "high" else "🟡" if prio == "medium" else "🟢"
            )
            click.echo(
                f"{priority_icon} {prio.upper()} PRIORITY ({len(priority_groups[prio])}):"
            )

            for rec in priority_groups[prio]:
                click.echo(f"  📋 {rec.category}: {rec.description[:80]}...")
                click.echo(
                    f"     Implementation: {rec.implementation_effort}, Impact: {rec.expected_impact}"
                )
...
def patterns(pattern_type: Optional[str], min_confidence: float, output_json: bool):
    ...
    if output_json:
        ...
        for pattern in patterns:
            ...
            click.echo(
                f"{confidence_indicator} {pattern.pattern_id} ({pattern.pattern_type})"
            )
            click.echo(
                f"   Confidence: {pattern.confidence:.2f}, Samples: {pattern.sample_count}"
            )
...
@click.option(
    "--feedback-type",
    type=click.Choice([t.value for t in FeedbackType]),
    default="human",
)
...
def stats(output_json):
    ...
    if output_json:
        ...
        if last_updated:
            ...
            click.echo(
                f"🔄 Cache Last Updated: {updated_time.strftime('%Y-%m-%d %H:%M:%S')}"
            )
...
@click.option(
    "--status",
    type=click.Choice(["implemented", "rejected", "pending"]),
    default="implemented",
    help="New status for the recommendation",
)
