...
def list(
    ...
    providers = registry.list_providers(
        with_capability=capability, feature_filter=feature, tag_filter=tag
    )
    ...
    if latency:
        providers = [
            p for p in providers if p in registry.get_providers_by_latency(latency)
        ]
    ...
    if output_json:
        ...
        for provider_name in providers:
            ...
            if metadata:
                ...
                click.echo(
                    f"   Pricing: {metadata.pricing_tier}, Latency: {metadata.latency_class}"
                )
...
def info(provider_name: str, output_json: bool):
    ...
    if output_json:
        ...
        for capability in metadata.capabilities:
            ...
            click.echo(
                f"  • {capability.name}: {capability.description}{required_indicator}"
            )
...
@click.option(
    "--min-reliability", type=float, default=0.8, help="Minimum reliability score"
)
