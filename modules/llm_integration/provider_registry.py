...
class ProviderRegistry:
    ...
    def register_provider(self, provider_class: Type[ProviderPlugin]) -> None:
        ...
        try:
            ...
            if metadata.name in self._provider_classes:
                self.logger.warning(f"Provider {metadata.name} is already registered, overwriting")
            ...
            self.logger.info(f"Registered provider: {metadata.name} v{metadata.version}")
            
        except Exception as e:
            self.logger.error(f"Failed to register provider {provider_class.__name__}: {e}")
            raise
    ...
    def list_providers(self, 
        ...
        if with_capability and with_capability in self._capabilities:
            providers = [p for p in providers if p in self._capabilities[with_capability]]
    ...
    def create_provider_instance(self, name: str, api_key: str, **kwargs) -> Optional[LLMProviderInterface]:
        """
    ...
    def _load_builtin_providers(self) -> None:
        ...
        try:
            from .builtin_providers import OpenAIPlugin, AnthropicPlugin, OpenRouterPlugin
