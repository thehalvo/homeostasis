    ...
    try:
        ...
        for provider, sources in keys_info.items():
            ...
            if args.verbose or not has_any_key:
                print("  Sources:")
                print(
                    f"    Environment Variable: {'✓' if sources['environment'] else '✗'}"
                )
                print(
                    f"    External Secrets:     {'✓' if sources['external_secrets'] else '✗'}"
                )
                print(
                    f"    Encrypted Storage:    {'✓' if sources['encrypted_storage'] else '✗'}"
                )
        ...
        print(
            "  • External secrets managers: AWS Secrets Manager, Azure Key Vault, HashiCorp Vault"
        )
...
def cmd_set_openrouter_unified(args: argparse.Namespace) -> None:
    ...
    try:
        manager.set_openrouter_unified_mode(
            args.enabled, args.proxy_anthropic, args.proxy_openai
        )
    except Exception as e:
        print(f"Error: {e}")
        sys.exit(1)
...
def cmd_get_provider_policies(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        print(
            f"  Reliability preference: {policies.get('reliability_preference', 'high')}"
        )
    except Exception as e:
        print(f"Error: {e}")
        sys.exit(1)
...
def cmd_provider_status(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        print(
            f"Available Providers: {', '.join(summary['available_providers']) if summary['available_providers'] else 'None'}"
        )
        ...
        print(
            f"OpenRouter Unified: {'Enabled' if summary['openrouter_unified'] else 'Disabled'}"
        )
        ...
        if summary["openrouter_unified"]:
            ...
            print(
                f"  Proxy to Anthropic: {'Yes' if unified_config.get('proxy_to_anthropic') else 'No'}"
            )
            print(
                f"  Proxy to OpenAI: {'Yes' if unified_config.get('proxy_to_openai') else 'No'}"
            )
...
def create_llm_cli_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="Homeostasis LLM Integration CLI", prog="homeostasis"
    )
    ...
    set_key_parser.add_argument(
        "provider", choices=APIKeyManager.SUPPORTED_PROVIDERS, help="LLM provider name"
    )
    set_key_parser.add_argument(
    ...
    list_keys_parser.add_argument(
        "--verbose", "-v", action="store_true", help="Show detailed source information"
    )
    ...
    set_active_parser = subparsers.add_parser(
        "set-active-provider", help="Set the active LLM provider"
    )
    ...
    get_active_parser = subparsers.add_parser(
        "get-active-provider", help="Get the current active provider"
    )
    ...
    set_fallback_parser = subparsers.add_parser(
        "set-fallback-order", help="Set the fallback order for providers"
    )
    ...
    get_fallback_parser = subparsers.add_parser(
        "get-fallback-order", help="Get the current fallback order"
    )
    ...
    set_fallback_enabled_parser = subparsers.add_parser(
        "set-fallback-enabled", help="Enable or disable fallback"
    )
    ...
    set_openrouter_parser = subparsers.add_parser(
        "set-openrouter-unified", help="Configure OpenRouter unified mode"
    )
    ...
    set_policies_parser = subparsers.add_parser(
        "set-provider-policies", help="Set provider selection policies"
    )
    ...
    get_policies_parser = subparsers.add_parser(
        "get-provider-policies", help="Get current provider selection policies"
    )
    ...
    provider_status_parser = subparsers.add_parser(
        "provider-status", help="Show multi-provider configuration status"
    )
