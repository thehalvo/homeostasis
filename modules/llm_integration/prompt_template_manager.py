...
class PromptMetadata:
    ...
    )  # Provider name -> preference score
...
class PromptTemplate:
    ...
    def render(self, variables: Dict[str, Any]) -> Dict[str, str]:
        ...
        if self.context_template:
            context_template = BaseTemplate(
                "temp_context", content=self.context_template
            )
...
class PromptTemplateManager:
    ...
    def _load_user_templates(self) -> None:
        ...
        self._load_templates_from_directory(
            self.user_templates_dir, is_user_template=True
        )

    def _load_templates_from_directory(
        self, directory: Path, is_user_template: bool = False
    ) -> None:
        """
        ...
        for template_file in directory.glob("*.yaml"):
            try:
                ...
                template_id = (
                    f"user:{template.metadata.name}"
                    if is_user_template
                    else template.metadata.name
                )
                self.templates[template_id] = template

                self.logger.info(
                    f"Loaded {'user' if is_user_template else 'built-in'} template: {template_id}"
                )
    ...
    def _parse_template_data(
        ...
        metadata = PromptMetadata(
            ...
            prompt_type=PromptType(
                metadata_data.get("prompt_type", "patch_generation")
            ),
    ...
    def list_templates(
        ...
        user_templates_only: bool = False,
    ) -> List[str]:
        """
    ...
    def create_user_template(
        self, template_data: Dict[str, Any], filename: Optional[str] = None
    ) -> str:
        """
    ...
    def update_user_template(
        ...
        try:
            template = self._parse_template_data(
                template_data, template_id.split(":", 1)[1]
            )
    ...
    def import_template(
        self, template_path: Path, as_user_template: bool = True
    ) -> Optional[str]:
        """
