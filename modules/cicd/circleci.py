...
class CircleCIIntegration:
    ...
    def __init__(self, api_token: Optional[str] = None, project_slug: Optional[str] = None):
        """
        ...
        if not self.project_slug:
            raise ValueError("Project slug required (format: vcs-type/org-name/repo-name)")
    ...
    def _get_project_slug_from_env(self) -> Optional[str]:
        ...
        if os.getenv('CIRCLE_PROJECT_REPONAME') and os.getenv('CIRCLE_PROJECT_USERNAME'):
        ...
        config = {
            ...
            'workflows': {
                'advanced-healing': {
                    'when': {
                        'not': {
                            'equal': ['', '<< pipeline.parameters.failed-workflow-id >>']
                        }
                    },
                    'jobs': [
                        {
    ...
    def trigger_healing_pipeline(self, failed_workflow_id: str, branch: str = 'main') -> Dict:
        """
    ...
    def setup_circleci_integration(self, create_configs: bool = True) -> Dict:
        ...
        if create_configs:
            ...
            with open('.circleci/config-healing-advanced.yml', 'w') as f:
                yaml.dump(advanced_workflow, f, default_flow_style=False, sort_keys=False)
