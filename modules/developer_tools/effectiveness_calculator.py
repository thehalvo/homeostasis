...
class EffectivenessCalculator:
    ...
    def calculate_effectiveness(
        ...
        human_intervention_count = sum(
            1 for m in filtered_metrics if m.human_intervention_required
        )
        ...
        confidence_dist = self._calculate_distribution(
            filtered_metrics, "confidence_score"
        )
        complexity_dist = self._calculate_distribution(
            filtered_metrics, "complexity_score"
        )

        # Generate recommendations
        recommendations = self._generate_recommendations(
            filtered_metrics, successful / total if total > 0 else 0
        )
        ...
        return EffectivenessReport(
            ...
            human_intervention_rate=(
                human_intervention_count / total if total > 0 else 0
            ),
    ...
    def _calculate_by_dimension(
        self, metrics: List[HealingMetrics], dimension: str
        ...
        for key, group_metrics in grouped.items():
            ...
            results[key] = {
                ...
                "success_rate": successful / total if total > 0 else 0,
                "average_time": (
                    statistics.mean([m.total_time for m in group_metrics if m.success])
                    if successful > 0
                    else 0
                ),
                "average_patches": statistics.mean(
                    [m.patches_generated for m in group_metrics]
                ),
            }

        return results

    def _calculate_time_trends(
        self, metrics: List[HealingMetrics]
    ) -> Dict[str, Dict[str, float]]:
        ...
        for day, day_metrics in sorted(daily_groups.items()):
            ...
            results[day] = {
                ...
                "success_rate": successful / total if total > 0 else 0,
                "average_time": (
                    statistics.mean([m.total_time for m in day_metrics if m.success])
                    if successful > 0
                    else 0
                ),
            }
    ...
    def _calculate_distribution(
        self, metrics: List[HealingMetrics], field: str
    ) -> Dict[str, int]:
    ...
    def _generate_recommendations(
        self, metrics: List[HealingMetrics], success_rate: float
    ) -> List[str]:
    ...
    def export_report(self, report: EffectivenessReport, format: str = "json") -> str:
...
"""
    ...
    def compare_periods(
        ...
        comparison = {
            "period1": {"start": period1_start, "end": period1_end, "metrics": report1},
    ...
    def predict_effectiveness(
        self, error_type: str, language: str, complexity: float
    ) -> Tuple[float, float]:
        ...
        if not similar_metrics:
            ...
            similar_metrics = [m for m in self.metrics if m.language == language]
        ...
        success_rate = sum(1 for m in similar_metrics if m.success) / len(
            similar_metrics
        )
        avg_time = (
            statistics.mean([m.total_time for m in similar_metrics if m.success])
            if any(m.success for m in similar_metrics)
            else 60.0
        )
