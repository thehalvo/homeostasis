...
class TemplateValidator:
    ...
    def __init__(self, templates_dir: Optional[Path] = None):
        self.templates_dir = templates_dir or Path(__file__).parent.parent / "patch_generation" / "templates"
    ...
    def _extract_metadata(self, content: str, template_path: Path, 
                         result: ValidationResult) -> Optional[TemplateMetadata]:
    ...
    def _infer_language(self, template_path: Path) -> Optional[str]:
        ...
        if parent_name in ["python", "javascript", "typescript", "go", "java", 
                          "ruby", "rust", "cpp", "csharp"]:
            return parent_name
    ...
    def _validate_structure(self, content: str, result: ValidationResult):
        ...
        if open_braces != close_braces:
            result.errors.append(f"Unbalanced template tags: {open_braces} open, {close_braces} close")
    ...
    def _validate_python_template(self, content: str, metadata: TemplateMetadata, 
        ...
        if "import" in content and "{% if" not in content:
            result.warnings.append("Import statements should be conditional on requirements")
    
    def _validate_javascript_template(self, content: str, metadata: TemplateMetadata, 
                                     result: ValidationResult):
    ...
    def _validate_typescript_template(self, content: str, metadata: TemplateMetadata, 
                                     result: ValidationResult):
        ...
        if ": any" in content:
            result.warnings.append("Template uses 'any' type, consider more specific types")
    
    def _validate_go_template(self, content: str, metadata: TemplateMetadata, 
                             result: ValidationResult):
        """Validate Go-specific template"""
        # Check for error handling
        if "if err != nil" not in content and "error" in metadata.error_type.lower():
            result.warnings.append("Go template for error handling should check 'err != nil'")
        
        # Check for proper imports
        if "import" in content and "(" not in content:
            result.warnings.append("Go imports should use parentheses for multiple imports")
    
    def _validate_java_template(self, content: str, metadata: TemplateMetadata, 
                               result: ValidationResult):
    ...
    def _validate_ruby_template(self, content: str, metadata: TemplateMetadata, 
                               result: ValidationResult):
    ...
    def _validate_rust_template(self, content: str, metadata: TemplateMetadata, 
                               result: ValidationResult):
    ...
    def _validate_cpp_template(self, content: str, metadata: TemplateMetadata, 
                              result: ValidationResult):
    ...
    def _validate_csharp_template(self, content: str, metadata: TemplateMetadata, 
                                  result: ValidationResult):
        """Validate C#-specific template"""
        # Check for using statements
        if "IDisposable" in content and "using" not in content:
            result.warnings.append("IDisposable objects should be used with 'using' statement")
    
    def validate_directory(self, directory: Path, 
                          recursive: bool = True) -> List[ValidationResult]:
