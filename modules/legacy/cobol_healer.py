...
class COBOLHealer:
    ...
    def _compile_error_patterns(self) -> Dict[str, re.Pattern]:
        ...
        patterns = {
            ...
            "ibm_error": re.compile(
                r"^(IGY[A-Z]\d{4}[I|W|E|S])-(\d+)\s+(.*)$"
            ),
    ...
    def _load_healing_rules(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            ...
            "IGYDS1089": {
                ...
                "category": "data_definition",
                "healing": {
                    "action": "move_to_working_storage",
                    "automated": False
                }
            },
    ...
    def analyze_source(self, source_code: str) -> List[Dict[str, Any]]:
        ...
        for i, line in enumerate(lines, 1):
            ...
            if in_procedure and code_line.strip() and not code_line.rstrip().endswith('.'):
                # Check if it's a statement that requires a period
                if any(code_upper.startswith(stmt) for stmt in 
                      ["MOVE", "COMPUTE", "PERFORM", "IF", "EVALUATE", "CALL"]):
                    next_line_idx = i
            ...
            if action == "add_period":
                ...
                if "PIC" in error_line.upper():
                    ...
                    if match:
                        ...
                        new_size = min(current_size * 2, 18)  # Max 18 digits for numeric
                        
                        fixed_line = pic_pattern.sub(f'PIC {pic_type}({new_size})', error_line)
                        lines[error_line_idx] = fixed_line
                        return '\n'.join(lines)
