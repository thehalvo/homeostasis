...
class HybridOrchestrator:
    ...
    def _check_conditions(self, request: Dict[str, Any], conditions: Dict[str, Any]) -> bool:
    ...
    def _select_targets(self, rule: RoutingRule) -> List[str]:
        ...
        if rule.legacy_weight > 0:
            # Find legacy systems
            legacy_systems = [
                name for name, system in self.systems.items()
                if system.type in [SystemType.MAINFRAME, SystemType.MONOLITH, SystemType.LEGACY_API]
            ]
            if legacy_systems:
        ...
        if rule.modern_weight > 0:
            ...
            modern_systems = [
                name for name, system in self.systems.items()
                if system.type in [SystemType.MICROSERVICE, SystemType.SERVERLESS, SystemType.MODERN_API]
            ]
            if modern_systems:
    ...
    async def _execute_shadow(self, transaction: HybridTransaction, 
        ...
        legacy_task = asyncio.create_task(
            self._send_to_system(
                transaction,
                self._get_legacy_system(route["targets"]),
                is_primary=True
            )
        )
        ...
        modern_task = asyncio.create_task(
            self._send_to_system(
                transaction,
                self._get_modern_system(route["targets"]),
                is_primary=False
            )
        )
    ...
    async def _execute_canary(self, transaction: HybridTransaction, 
                            route: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _execute_blue_green(self, transaction: HybridTransaction, 
                                route: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _execute_percentage(self, transaction: HybridTransaction, 
                                route: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _send_to_system(self, transaction: HybridTransaction, 
                            system_name: str, is_primary: bool) -> Dict[str, Any]:
        ...
        try:
            ...
            if is_primary and self._is_state_changing(transaction.payload):
                transaction.compensation_actions.append({
                    "system": system_name,
                    "action": self._create_compensation_action(adapted_request, system)
                })
...
<soap:Envelope xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/">
    ...
    def _is_state_changing(self, request: Dict[str, Any]) -> bool:
        ...
        return (operation in state_changing_ops or 
                method in state_changing_methods)
                
    def _create_compensation_action(self, request: Any, system: SystemEndpoint) -> Dict[str, Any]:
    ...
    def _compare_results(self, legacy_result: Any, modern_result: Any, 
                        transaction: HybridTransaction):
        ...
        if not comparison["match"]:
            ...
            logger.warning(f"Result mismatch for transaction {transaction.transaction_id}")
