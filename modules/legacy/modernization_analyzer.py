...
class ModernizationAnalyzer:
    ...
    def _analyze_file(
        self, file_path: str, filename: str, content: str, language: str
    ...
    def _detect_component_type(self, content: str, language: str) -> str:
        ...
        if language in ["cobol", "fortran"]:
            if "PROGRAM-ID" in content_upper or "PROGRAM " in content_upper:
                if any(
                    db in content_upper for db in ["EXEC SQL", "SELECT ", "INSERT "]
                ):
    ...
    def _extract_interfaces(self, content: str, language: str) -> List[str]:
        ...
        if language == "cobol":
            ...
            interfaces.extend(
                [f"IMPORT:{i}" for i in imports if not i.startswith("java.")]
            )

            # REST endpoints
            endpoints = re.findall(
                r"@(?:Get|Post|Put|Delete)Mapping\(['\"]([^'\"]+)", content
            )
    ...
    def _calculate_technical_debt(self, content: str, language: str) -> float:
        ...
        # Commented code
        commented_code = len(
            [line for line in lines if re.match(r"^\s*[\*C!]\s*\w+", line)]
        )
    ...
    def create_modernization_plan(
        ...
        for comp in components:
            languages[comp.language] = (
                languages.get(comp.language, 0) + comp.lines_of_code
            )
    ...
    def _determine_strategy(
        self, components: List[LegacyComponent], languages: Dict[str, int]
        ...
        avg_tech_debt = (
            sum(c.technical_debt_score for c in components) / total_components
        )
    ...
    def _create_component_path(
        self, component: LegacyComponent, strategy: ModernizationStrategy
    ) -> ModernizationPath:
    ...
    def _calculate_roi(
        ...
        return {
            ...
            "roi_percentage": round(
                (total_annual_benefit * 5 - migration_cost) / migration_cost * 100, 1
            ),
        }
    ...
    def generate_report(self, analysis: SystemAnalysis) -> str:
        ...
        report.append(
            f"- Estimated Effort: {analysis.estimated_total_effort:,} person-days"
        )
        report.append(
            f"- Estimated Cost: ${analysis.roi_analysis['migration_cost']:,.0f}"
        )
        report.append(
            f"- Payback Period: {analysis.roi_analysis['payback_period_years']} years"
        )
        ...
        for risk, count in sorted(
            analysis.risk_assessment.items(), key=lambda x: x[0].value
        ):
            percentage = (count / analysis.total_components) * 100
        ...
        for wave in analysis.roadmap:
            report.append(
                f"### Wave {wave['wave']} (Months {wave['start_month']}-{wave['start_month'] + wave['duration_months']})"
            )
        ...
        report.append(
            f"- Current Annual Maintenance: ${roi['current_annual_maintenance']:,.0f}"
        )
        report.append(
            f"- Future Annual Maintenance: ${roi['future_annual_maintenance']:,.0f}"
        )
        ...
        report.append(
            "1. **Start with Low-Risk Components**: Begin modernization with components that have low complexity and few dependencies"
        )
        report.append(
            "2. **Establish Modern Infrastructure**: Set up CI/CD pipelines, automated testing, and cloud infrastructure early"
        )
        report.append(
            "3. **Maintain Parallel Systems**: Run legacy and modern systems in parallel during transition"
        )
        report.append(
            "4. **Invest in Training**: Ensure team has skills for both legacy maintenance and modern development"
        )
        report.append(
            "5. **Monitor Progress**: Track KPIs and adjust approach based on learnings"
        )
