...
class FortranHealer:
    ...
    def _load_healing_rules(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            ...
            "implicit_type": {
                ...
                "category": "type",
                "healing": {
                    "action": "add_explicit_declaration",
                    "automated": True
                }
            },
            # Array errors
    ...
    def parse_error(self, error_text: str, compiler: str = "gfortran") -> Optional[FortranError]:
    ...
    def _add_bounds_check(self, lines: List[str], error_idx: int) -> str:
        ...
        if array_match:
            ...
            check_lines = [
                ...
                error_line,
                ' ' * indent + "ELSE",
                ' ' * indent + f"  PRINT *, 'Array index out of bounds for {array_name}'",
                ' ' * indent + "  STOP",
                ' ' * indent + "END IF"
            ]
    ...
    def modernize_code(self, source_code: str, target_standard: FortranStandard = None) -> Tuple[str, List[Dict[str, Any]]]:
    ...
    def _convert_to_free_format(self, lines: List[str]) -> Tuple[List[str], List[Dict[str, Any]]]:
    ...
    def _replace_common_blocks(self, lines: List[str]) -> Tuple[List[str], List[Dict[str, Any]]]:
        ...
        for i, line in enumerate(lines):
            ...
            if common_match:
                ...
                common_blocks[block_name].append({
                    "line": i,
                    "vars": var_list
                })
        ...
        for block_name, occurrences in common_blocks.items():
            ...
            module_lines.extend([
                f"END MODULE {block_name}_MODULE",
                ""
            ])
    ...
    def _modernize_do_loops(self, lines: List[str]) -> Tuple[List[str], List[Dict[str, Any]]]:
        """Modernize DO loops."""
        modifications = []
        ...
        for i, line in enumerate(lines):
            ...
            old_do_match = re.match(r'(\s*)DO\s+(\d+)\s+(\w+)\s*=\s*(.+)$', line, re.IGNORECASE)
            if old_do_match:
    ...
    def _replace_arithmetic_if(self, lines: List[str]) -> Tuple[List[str], List[Dict[str, Any]]]:
        ...
        for i, line in enumerate(lines):
            ...
            arith_if_match = re.match(r'(\s*)IF\s*\(([^)]+)\)\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)', 
                                     line, re.IGNORECASE)
            if arith_if_match:
    ...
    def _add_implicit_none_everywhere(self, lines: List[str]) -> Tuple[List[str], List[Dict[str, Any]]]:
        ...
        while i < len(new_lines):
            ...
            if any(re.search(rf'\b{keyword}\s+\w+', line_upper) for keyword in 
                   ["PROGRAM", "SUBROUTINE", "FUNCTION", "MODULE"]):
                ...
                for j in range(i + 1, min(i + 10, len(new_lines))):
                    ...
                    if any(keyword in new_lines[j].upper() for keyword in 
    ...
    def _modernize_io_statements(self, lines: List[str]) -> Tuple[List[str], List[Dict[str, Any]]]:
        ...
        for i, line in enumerate(lines):
            ...
            old_write = re.sub(r'WRITE\s*\(\s*6\s*,', 'WRITE(*,', line, flags=re.IGNORECASE)
            if old_write != line:
            ...
            old_read = re.sub(r'READ\s*\(\s*5\s*,', 'READ(*,', line, flags=re.IGNORECASE)
            if old_read != line:
    ...
    def _use_array_syntax(self, lines: List[str]) -> Tuple[List[str], List[Dict[str, Any]]]:
            ...
            if re.match(r'\s*DO\s+\w+\s*=', lines[i], re.IGNORECASE):
                ...
                if do_match:
                    ...
                    for j in range(i + 1, min(i + 5, len(lines))):
                        assign_match = re.match(rf'\s*(\w+)\s*\(\s*{loop_var}\s*\)\s*=\s*(.+)', 
