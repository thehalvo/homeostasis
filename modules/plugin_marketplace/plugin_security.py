...
class PluginSandbox:
    ...
    def create_sandbox(
        self, plugin_id: str, permissions: Dict[str, List[str]]
    ) -> "SandboxContext":
        """
    ...
    def enforce_permission(self, permission: PermissionType):
        ...
        if not self.check_permission(permission):
            raise PermissionError(
                f"Plugin {self.plugin_id} does not have {permission.value} permission"
            )
...
class ContainerSandbox(SandboxContext):
    ...
    def _get_environment(self) -> Dict[str, str]:
        ...
        env = {"PLUGIN_ID": self.plugin_id, "SANDBOX_MODE": "container"}
...
class PluginSigner:
    ...
    def sign_plugin(
        self, plugin_path: Path, key_fingerprint: str, passphrase: Optional[str] = None
    ) -> bool:
        """
        ...
        shutil.make_archive(str(archive_path.with_suffix("")), "gztar", plugin_path)
...
class VulnerabilityScanner:
    ...
    def _analyze_python_code(self, file_path: Path) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            result = subprocess.run(
                ["bandit", "-f", "json", str(file_path)], capture_output=True, text=True
            )
    ...
    def _analyze_go_code(self, file_path: Path) -> List[Dict[str, Any]]:
        ...
        try:
            # Use gosec
            result = subprocess.run(
                ["gosec", "-fmt=json", str(file_path)], capture_output=True, text=True
            )
...
class PluginSecurityManager:
    ...
    def _check_dangerous_permissions(
        ...
        if "network" in permissions and "any" in permissions["network"]:
            if "filesystem" in permissions and "write" in permissions["filesystem"]:
                issues.append(
                    "Plugin requests unrestricted network and filesystem write access"
                )

        # Check for process spawning with network access
        if "process" in permissions and any(
            p in permissions["process"] for p in ["spawn", "exec"]
        ):
    ...
    def create_secure_context(
        ...
        if not user_approved and self.security_level != SecurityLevel.UNRESTRICTED:
            ...
            granted_permissions = {"filesystem": ["read"], "network": ["https"]}
        else:
    ...
    def sign_plugin(
        self, plugin_path: Path, author_email: str, passphrase: Optional[str] = None
    ) -> bool:
        """
        Sign a plugin package.
        ...
        if not keys:
            ...
            fingerprint = self.signer.generate_key_pair(plugin_path.name, author_email)
        else:
