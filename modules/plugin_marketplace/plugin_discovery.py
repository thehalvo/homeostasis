...
class PluginManifest:
    ...
    MANIFEST_SCHEMA_PATH = Path(__file__).parent.parent.parent / "standards/v1.0/schemas/plugin-manifest.json"
    
    def __init__(self, manifest_data: Dict[str, Any], source_path: Optional[Path] = None):
        """
...
class PluginRegistry:
    ...
    def find_plugins(self, **criteria) -> List[PluginInfo]:
        ...
        if 'capabilities' in criteria:
            ...
            results = [
                p for p in results
                if required_caps.issubset(p.manifest.required_capabilities | 
                                         p.manifest.optional_capabilities)
            ]
    ...
    def _check_permissions(self, plugin_info: PluginInfo) -> List[str]:
        ...
        if 'filesystem' in permissions and 'write' in permissions['filesystem']:
            if 'process' in permissions and 'spawn' in permissions['process']:
                issues.append("Plugin requests both write and process spawn permissions")
...
if __name__ == "__main__":
    ...
    analyze_plugins = registry.get_plugins_by_capability(PluginCapability.ERROR_ANALYZE.value)
