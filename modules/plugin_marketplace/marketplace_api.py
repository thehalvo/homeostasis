...
class MarketplaceAPI:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.storage = PluginStorage(config.get('storage_path', '/var/lib/homeostasis/plugins'))
        
        # Setup database
        self.engine = create_engine(config.get('database_url', 'sqlite:///marketplace.db'))
    ...
    def _register_routes(self):
        ...
        # Plugin download
        self.app.route('/api/v1/plugins/<plugin_id>/download', methods=['GET'])(self.download_plugin)
        
        # Plugin publishing
        self.app.route('/api/v1/plugins', methods=['POST'])(self.publish_plugin)
        self.app.route('/api/v1/plugins/<plugin_id>', methods=['PUT'])(self.update_plugin)
        self.app.route('/api/v1/plugins/<plugin_id>', methods=['DELETE'])(self.unpublish_plugin)
        
        # Reviews
        self.app.route('/api/v1/plugins/<plugin_id>/reviews', methods=['GET'])(self.list_reviews)
        self.app.route('/api/v1/plugins/<plugin_id>/reviews', methods=['POST'])(self.submit_review)
        ...
        try:
            ...
            if capabilities:
                ...
                for plugin in results:
                    manifest_path = self.storage.get_plugin_path(plugin.id) / 'manifest.json'
                    if manifest_path.exists():
                        with open(manifest_path, 'r') as f:
                            manifest = json.load(f)
                        
                        plugin_caps = set(manifest.get('capabilities', {}).get('required', []))
                        plugin_caps.update(manifest.get('capabilities', {}).get('optional', []))
    ...
    def publish_plugin(self):
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                is_secure, security_issues = self.security_manager.validate_plugin_security(plugin_dir)
                ...
                plugin = Plugin(
                    ...
                    status='pending' if self.config.get('review_required', True) else 'approved'
                )
        ...
        try:
            ...
            if 'marketplace' in data:
                ...
                if 'price' in marketplace:
                    ...
                    plugin.currency = marketplace['price'].get('currency', plugin.currency)
    ...
    def unpublish_plugin(self, plugin_id: str):
        ...
        try:
            ...
            if plugin.author_id != author_id:
                return jsonify({'error': 'Not authorized to unpublish this plugin'}), 403
    ...
    def list_reviews(self, plugin_id: str):
        ...
        try:
            ...
            # Get reviews
            query = session.query(PluginReview).filter(
                PluginReview.plugin_id == plugin_id
            ).order_by(PluginReview.helpful_count.desc(), PluginReview.created_at.desc())
    ...
    def submit_review(self, plugin_id: str):
        ...
        try:
            ...
            existing = session.query(PluginReview).filter(
                PluginReview.plugin_id == plugin_id,
            ...
            avg_rating = session.query(func.avg(PluginReview.rating)).filter(
                PluginReview.plugin_id == plugin_id
            ).scalar()
            
            rating_count = session.query(func.count(PluginReview.id)).filter(
                PluginReview.plugin_id == plugin_id
            ).scalar()
    ...
    def register_author(self):
        ...
        try:
            ...
            existing = session.query(Author).filter(Author.email == data['email']).first()
            if existing:
    ...
    def get_statistics(self):
        ...
        try:
            ...
            stats = {
                'total_plugins': session.query(Plugin).filter(Plugin.status == 'approved').count(),
                'total_downloads': session.query(func.sum(Plugin.downloads)).scalar() or 0,
            ...
            # Plugins by type
            for plugin_type in ['language', 'analysis', 'integration', 'deployment', 'monitoring']:
                count = session.query(Plugin).filter(
            ...
            # Top plugins
            top_plugins = session.query(Plugin).filter(
                Plugin.status == 'approved'
            ).order_by(Plugin.downloads.desc()).limit(10).all()
            ...
            # Recent plugins
            recent_plugins = session.query(Plugin).filter(
                Plugin.status == 'approved'
            ).order_by(Plugin.published_at.desc()).limit(10).all()
