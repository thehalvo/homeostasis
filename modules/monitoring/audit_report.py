...
def get_summary_report(report: Dict[str, Any]) -> str:
    ...
    error_section = "\nRecent Error Events:\n"
    if error_table:
        error_section += format_table(error_table, ["Timestamp", "Event Type", "User", "Details"])
    else:
...
def get_user_activity_report(report: Dict[str, Any]) -> str:
    ...
    user_section = ""
    for user, activities in sorted(user_activity.items(), key=lambda x: sum(x[1].values()), reverse=True):
        user_section += f"User: {user}\n"
        
        activity_table = []
        for event_type, count in sorted(activities.items(), key=lambda x: x[1], reverse=True):
...
def main():
    ...
    if args.report_type == "export":
        ...
        if args.output:
            ...
            if success:
                print(f"Exported audit data to {args.output} in {args.export_format} format")
            else:
