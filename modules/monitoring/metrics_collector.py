...
class MetricsCollector:
    ...
    def __init__(
        ...
        self.logger.info(
            f"Initialized metrics collector with storage at {self.storage_dir}"
        )
            ...
            for metric_file in self.storage_dir.glob("*.json"):
                try:
                    ...
                    self.logger.exception(
                        e, message=f"Failed to load metrics from {metric_file}"
                    )
    ...
    def _save_metrics(self, metric_type: str) -> None:
        ...
        try:
            ...
            self.logger.info(
                f"Saved {len(self.metrics[metric_type])} {metric_type} metrics to {file_path}"
            )
        ...
        if len(files) > self.history_limit:
            for file in files[: -self.history_limit]:
                try:
                    ...
                    self.logger.exception(
                        e, message=f"Failed to remove old file: {file}"
                    )

    def record_metric(
        self, metric_type: str, metric_data: Dict[str, Any], save: bool = True
    ) -> None:
        """
        Record a new metric.
    ...
    def analyze_metrics(
        ...
        for field in numeric_fields:
            ...
            try:
                ...
                self.logger.warning(
                    f"Failed to calculate statistics for {field}: {str(e)}"
                )
    ...
    def generate_report(
        ...
        for metric_type in metric_types:
            ...
            report["metrics"][metric_type] = {"overall": overall, "entities": entities}
        ...
        if output_file:
            try:
                ...
                self.logger.exception(
                    e, message=f"Failed to write report to {output_file}"
                )
    ...
    def get_effectiveness_score(self, patch_id: str) -> float:
        ...
        weights = {"test": 0.3, "deployment": 0.3, "fix": 0.4}
        ...
        if test_metrics:
            success_rate = sum(
                1 for m in test_metrics if m.get("success", False)
            ) / len(test_metrics)
        ...
        if deployment_metrics:
            success_rate = sum(
                1 for m in deployment_metrics if m.get("success", False)
            ) / len(deployment_metrics)
            scores["deployment"] = success_rate

        # Calculate fix score
        fix_metrics = self.get_metrics("fix", patch_id)
        if fix_metrics:
            success_rate = sum(1 for m in fix_metrics if m.get("success", False)) / len(
                fix_metrics
            )
