    ...
    def analyze_error_patterns(
        ...
        if temporal_patterns:
            ...
            if "hourly" in temporal_patterns:
                ...
                if (
                    peak_hour[1] > max(hourly_data.values()) * 0.8
                ):  # Peak hour has 80%+ of max
                    insights.append(
                        {
        ...
        if severity_patterns:
            ...
            high_severity = severity_patterns.get("high", 0) + severity_patterns.get(
                "critical", 0
            )
...
class RuleOptimizer:
    ...
    def analyze_rule_performance(
        self, rule_data: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
        ...
        for rule_id, stats in rule_performance.items():
            ...
            avg_confidence = (
                statistics.mean(stats["confidence_scores"])
            ...
            # Underutilized rules
            if (
                match_rate < 0.05 and stats["executions"] > 50
            ):  # Less than 5% match rate with many executions
                optimization_suggestions.append(
                    {
                        "rule_id": rule_id,
        ...
        return {
            ...
            "summary": {
                ...
                "total_executions": sum(
                    stats["executions"] for stats in rule_performance.values()
                ),
                "analysis_timestamp": datetime.utcnow().isoformat(),
            },
        }
...
class TelemetryRuleEnhancementSystem:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.error_analyzer = ErrorPatternAnalyzer(
            self.config.get("error_analysis", {})
        )
        ...
        # Data storage
        self.telemetry_storage = Path(
            self.config.get("telemetry_storage", "logs/telemetry")
        )
    ...
    def _flush_telemetry_buffer(self) -> None:
        ...
        try:
            ...
            self.monitoring_logger.info(
                f"Flushed {len(self.telemetry_buffer)} telemetry points to {file_path}"
            )
    ...
    def analyze_error_patterns(self, days_back: int = 7) -> Dict[str, Any]:
        ...
        error_data = self.load_telemetry_data(
            event_types=["error_detected", "error_analyzed"], start_time=start_time
        )
        ...
        self.monitoring_logger.info(
            f"Analyzed {len(error_data)} error events from last {days_back} days"
        )
    ...
    def analyze_llm_performance(self, days_back: int = 7) -> Dict[str, Any]:
        ...
        self.monitoring_logger.info(
            f"Analyzed {len(llm_data)} LLM interactions from last {days_back} days"
        )
    ...
    def analyze_rule_performance(self, days_back: int = 7) -> Dict[str, Any]:
        ...
        self.monitoring_logger.info(
            f"Analyzed {len(rule_data)} rule executions from last {days_back} days"
        )
    ...
    def generate_comprehensive_insights(self, days_back: int = 7) -> Dict[str, Any]:
        ...
        insights["cross_cutting_insights"] = self._generate_cross_cutting_insights(
            insights
        )

        # Prioritize recommendations
        insights["priority_recommendations"] = self._prioritize_recommendations(
            insights
        )
    ...
    def _calculate_priority_score(self, recommendation: Dict[str, Any]) -> float:
        ...
        if any(
            keyword in recommendation.get("message", "").lower()
    ...
    def _cache_result(self, cache_key: str, data: Any) -> None:
        ...
        self.analytics_cache[cache_key] = {"data": data, "timestamp": datetime.utcnow()}
    ...
    def cleanup_old_data(self, days_to_keep: int = 30) -> int:
        ...
        try:
            ...
            if cleaned_count > 0:
                self.monitoring_logger.info(
                    f"Cleaned up {cleaned_count} old telemetry files"
                )
