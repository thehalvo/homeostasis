...
class FeedbackLoop:
    ...
    def __init__(self, 
        ...
        self.template_stats = defaultdict(lambda: {"uses": 0, "success": 0, "failure": 0})
        
        # Bug type effectiveness tracking
        self.bug_type_stats = defaultdict(lambda: {"fixes": 0, "success": 0, "failure": 0})
    ...
    def _load_insights(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.exception(e, message=f"Failed to load insights from {self.insights_file}")
            return {
                "templates": {},
                "bug_types": {},
                "recommendations": [],
    ...
    def _save_insights(self) -> None:
        ...
        try:
            ...
            self.logger.exception(e, message=f"Failed to save insights to {self.insights_file}")
    ...
    def get_template_effectiveness(self, template_name: Optional[str] = None) -> Dict[str, Any]:
        """
    ...
    def get_bug_type_effectiveness(self, bug_type: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        if bug_type:
            if bug_type in self.bug_type_stats:
                ...
                success_rate = stats["success"] / stats["fixes"] if stats["fixes"] > 0 else 0
                return {
                    "bug_type": bug_type,
                    "fixes": stats["fixes"],
                    "success": stats["success"],
    ...
    def analyze_fixes(self) -> Dict[str, Any]:
        ...
        if not fix_metrics:
            return {
                "count": 0,
                "message": "No fix metrics available"
            }
    ...
    def generate_recommendations(self) -> List[Dict[str, Any]]:
        ...
        if fix_metrics:
            ...
            if test_metrics:
                ...
                success_patch_ids = {m.get("patch_id") for m in success_metrics if "patch_id" in m}
                failure_patch_ids = {m.get("patch_id") for m in failure_metrics if "patch_id" in m}
                ...
                if success_test_counts and failure_test_counts:
                    avg_success_tests = sum(success_test_counts) / len(success_test_counts)
                    avg_failure_tests = sum(failure_test_counts) / len(failure_test_counts)
                    
                    if avg_success_tests > avg_failure_tests * 1.5:  # 50% more tests on average
                        recommendations.append({
    ...
    def apply_feedback_to_template(self, 
                                 template_name: str, 
                                 template_path: Path,
                                 improvements: Dict[str, Any]) -> Optional[Path]:
        """
        ...
        try:
            ...
            if "additional_checks" in improvements:
                for check in improvements["additional_checks"]:
                    ...
                    if check_placeholder in new_content:
                        new_content = new_content.replace(check_placeholder, f"{check_placeholder}\n{check}")
                        
            # Modify patterns
            if "pattern_replacements" in improvements:
                for old_pattern, new_pattern in improvements["pattern_replacements"].items():
            ...
            self.logger.info(f"Applied feedback to template {template_name}, backup saved to {backup_path}")
            
            return template_path
            
        except Exception as e:
            self.logger.exception(e, message=f"Failed to apply feedback to template {template_name}")
            return None
...
class FixImprovement:
    ...
    def identify_patterns(self) -> Dict[str, Any]:
        ...
        template_effectiveness = [(name, stats["success_rate"]) for name, stats in template_stats.items() if stats["uses"] >= 5]
        ...
        bug_type_effectiveness = [(name, stats["success_rate"]) for name, stats in bug_type_stats.items() if stats["fixes"] >= 5]
        ...
        if fix_metrics:
            ...
            if len(fix_metrics) >= 10:
                # Sort by timestamp
                sorted_metrics = sorted(fix_metrics, key=lambda x: x.get("timestamp", 0))
    ...
    def generate_template_improvements(self, template_name: str) -> Dict[str, Any]:
        ...
        fix_metrics = self.feedback_loop.metrics_collector.get_metrics("fix")
        template_metrics = [m for m in fix_metrics if m.get("template") == template_name]
        ...
        for pattern in failure_patterns:
            if "null" in pattern.lower():
                improvements["additional_checks"].append("# Check for null/None values\nif value is None:")
            elif "undefined" in pattern.lower():
                improvements["additional_checks"].append("# Check for undefined values\nif 'undefined' in str(value):")
            elif "index" in pattern.lower():
                improvements["additional_checks"].append("# Check for index errors\nif index >= len(collection):")
            elif "key" in pattern.lower():
                improvements["additional_checks"].append("# Check for missing keys\nif key not in dictionary:")
    ...
    def apply_improvements(self, templates_dir: Path) -> List[Dict[str, Any]]:
        ...
        template_recommendations = [r for r in recommendations if r["type"] == "template_improvement"]
...
if __name__ == "__main__":
    ...
    template_improvements = improvement.generate_template_improvements("keyerror_fix.py.template")
