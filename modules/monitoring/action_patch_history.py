...
class ActionPatchHistoryLogger:
        ...
        try:
            ...
            while current_dir != current_dir.parent:
                if (current_dir / '.git').exists():
                    ...
                    self.monitoring_logger.info(f"Connected to git repository at {current_dir}")
                    return
            ...
            if not git_dir.exists():
                ...
                self.monitoring_logger.info(f"Initialized git repository at {self.base_dir}")
            else:
    ...
    def log_action_start(self, session_id: str, action_type: str, 
    ...
    def log_llm_interaction(self, action_id: str, provider: str, model: str,
        ...
        self._update_action_record(action_id, {
            'llm_interactions': [interaction_id]
        }, append_lists=True)
    ...
    def log_file_change(self, action_id: str, file_path: str, 
                       change_type: str, old_content: str = None,
        ...
        if self.include_file_diffs and old_content is not None and new_content is not None:
            ...
            change_record['lines_added'] = len([line for line in diff if line.startswith('+')])
            change_record['lines_removed'] = len([line for line in diff if line.startswith('-')])
    ...
    def log_patch_application(self, action_id: str, patch_id: str,
                            target_file: str, patch_content: str,
        ...
        # Update action record
        self._update_action_record(action_id, {
            'patch_applications': [patch_app_id]
        }, append_lists=True)
    ...
    def log_action_completion(self, action_id: str, status: str,
    ...
    def get_action_history(self, session_id: str = None, 
        ...
        for action_file in self.base_dir.glob('action_*.json'):
            try:
                ...
                self.monitoring_logger.warning(f"Failed to read action file {action_file}: {e}")
    ...
    def get_file_change_history(self, file_path: str = None,
                               days_back: int = 7) -> List[Dict[str, Any]]:
        """
        ...
        for change_file in self.base_dir.glob('change_*.json'):
            try:
                ...
                self.monitoring_logger.warning(f"Failed to read change file {change_file}: {e}")
    ...
    def get_llm_interaction_history(self, action_id: str = None,
                                  provider: str = None,
                                  days_back: int = 7) -> List[Dict[str, Any]]:
        """
        ...
        for interaction_file in self.llm_logs_dir.glob('interaction_*.json'):
            try:
                ...
                # Check date filter
                timestamp = datetime.fromisoformat(interaction_record.get('timestamp', ''))
                if timestamp < cutoff_date:
                    continue
                ...
                self.monitoring_logger.warning(f"Failed to read interaction file {interaction_file}: {e}")
    ...
    def get_patch_application_history(self, days_back: int = 7) -> List[Dict[str, Any]]:
        ...
        for patch_app_file in self.base_dir.glob('patch_app_*.json'):
            try:
                ...
                self.monitoring_logger.warning(f"Failed to read patch application file {patch_app_file}: {e}")
    ...
    def cleanup_old_entries(self) -> int:
        ...
        for action_file in self.base_dir.glob('action_*.json'):
            try:
                ...
                self.monitoring_logger.warning(f"Failed to process action file {action_file}: {e}")
        ...
        for change_file in self.base_dir.glob('change_*.json'):
            try:
                ...
                self.monitoring_logger.warning(f"Failed to process change file {change_file}: {e}")
        ...
        for interaction_file in self.llm_logs_dir.glob('interaction_*.json'):
            try:
                ...
                self.monitoring_logger.warning(f"Failed to process interaction file {interaction_file}: {e}")
        ...
        for patch_app_file in self.base_dir.glob('patch_app_*.json'):
            try:
                ...
                self.monitoring_logger.warning(f"Failed to process patch app file {patch_app_file}: {e}")
        
        if cleaned_count > 0:
            self.monitoring_logger.info(f"Cleaned up {cleaned_count} old history entries")
        
        return cleaned_count
    
    def _update_action_record(self, action_id: str, updates: Dict[str, Any],
                            append_lists: bool = False) -> None:
        ...
        try:
            ...
            for key, value in updates.items():
                if append_lists and key in action_record and isinstance(action_record[key], list):
            ...
            self.monitoring_logger.warning(f"Failed to update action record {action_id}: {e}")
    ...
    def _get_action_record(self, action_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            self.monitoring_logger.warning(f"Failed to read action record {action_id}: {e}")
            return None
...
def log_action_start(session_id: str, action_type: str, 
                    context: Dict[str, Any] = None) -> str:
    """Log the start of a healing action."""
    return get_action_patch_history_logger().log_action_start(session_id, action_type, context)


def log_llm_interaction(action_id: str, provider: str, model: str,
                       prompt: str, response: str, 
                       metadata: Dict[str, Any] = None) -> str:
...
def log_file_change(action_id: str, file_path: str, change_type: str,
...
def log_patch_application(action_id: str, patch_id: str, target_file: str,
                         patch_content: str, application_result: Dict[str, Any]) -> str:
...
def log_action_completion(action_id: str, status: str,
                         performance_metrics: Dict[str, Any] = None,
                         error_details: Dict[str, Any] = None) -> None:
    """Log the completion of a healing action."""
