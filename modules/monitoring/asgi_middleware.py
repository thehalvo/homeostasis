...
class HomeostasisASGIMiddleware:
    ...
    def __init__(self, app, **config):
        ...
        try:
            ...
            logger.info(f"Homeostasis ASGI middleware initialized for {self.config['FRAMEWORK']}")
    ...
    async def __call__(self, scope, receive, send):
        ...
        async def send_wrapper(message):
            ...
            elif message_type == "http.response.body" and self.config["LOG_RESPONSE_BODY"]:
                if "body" in message:
    ...
    async def _log_request(self, scope: Dict[str, Any], request_id: str) -> None:
        ...
        try:
            ...
            if "server" in scope:
                server = scope["server"]
                request_data["server"] = f"{server[0]}:{server[1]}" if server else "unknown"
    ...
    async def _log_response(self, scope: Dict[str, Any], request_id: str, 
                           status: Optional[int], headers: Optional[list],
                           body: Optional[bytes], duration: float) -> None:
        """
    ...
    async def _log_slow_request(self, scope: Dict[str, Any], request_id: str, 
                              duration: float) -> None:
        """
    ...
    async def _capture_exception(self, scope: Dict[str, Any], request_id: str, 
                               exc: Exception, duration: float,
        ...
        try:
            ...
            if "server" in scope:
                server = scope["server"]
                error_data["request"]["server"] = f"{server[0]}:{server[1]}" if server else "unknown"
    ...
    async def _attempt_recovery(self, scope: Dict[str, Any], request_id: str, 
        ...
        if not hasattr(self, "homeostasis_logger"):
            try:
                self.homeostasis_logger = MonitoringLogger(
                    "tornado",
                    log_level=self.homeostasis_config["LOG_LEVEL"]
                )
            except Exception as e:
                ...
                try:
                    ...
                    if "application/json" in content_type:
                        ...
                        try:
                            ...
                            request_data["body"] = self.request.body.decode("utf-8", errors="ignore")
