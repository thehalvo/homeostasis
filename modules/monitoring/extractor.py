...
def extract_errors(log_file: Path = LOG_FILE, 
...
def get_latest_errors(limit: int = 10, 
                    levels: Optional[List[str]] = None,
                    service_name: Optional[str] = None,
                    exception_types: Optional[List[str]] = None,
...
def get_error_summary(days_back: int = 7, service_name: Optional[str] = None) -> Dict[str, Any]:
    """
    ...
    frequent_patterns = [
        {
            ...
            "latest": max([e.get("timestamp", "1970-01-01T00:00:00") for e in errors_list])
        }
    ...
    return {
        ...
        "frequent_patterns": frequent_patterns[:10]  # Top 10 most frequent error patterns
    }
...
if __name__ == "__main__":
    ...
    for error in get_latest_errors(5, hours_back=24):
        ...
        error_details = error.get("error_details", {})
        error_type = error_details.get("exception_type", error.get("exception_type", "Unknown"))
    ...
    print(f"Total errors: {summary['total_errors']} (in the last {summary['summary_period']})")
    print(f"Errors by severity: Critical: {summary['severity']['critical']}, High: {summary['severity']['high']}, Medium: {summary['severity']['medium']}")
    ...
    print("\nError types:")
    for error_type, count in sorted(summary['error_types'].items(), key=lambda x: x[1], reverse=True)[:5]:
        print(f"- {error_type}: {count}")
    
    print("\nMost affected endpoints:")
    for endpoint, count in sorted(summary['endpoints'].items(), key=lambda x: x[1], reverse=True)[:5]:
        print(f"- {endpoint}: {count}")
    
    print("\nMost frequent error patterns:")
    for i, pattern in enumerate(summary['frequent_patterns'][:3], 1):
        print(f"{i}. {pattern['pattern']} - {pattern['count']} occurrences, latest at {pattern['latest']}")