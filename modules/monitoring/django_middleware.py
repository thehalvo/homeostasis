...
class HomeostasisMiddleware:
    ...
    def __init__(self, get_response):
        ...
        try:
            ...
            self.include_sensitive_data = homeostasis_config.get("INCLUDE_SENSITIVE_DATA", False)
            ...
            self.ignore_exceptions = set(homeostasis_config.get("IGNORE_EXCEPTIONS", []))
            
            # Performance monitoring options
            self.monitor_performance = homeostasis_config.get("MONITOR_PERFORMANCE", False)
            self.slow_request_threshold = homeostasis_config.get("SLOW_REQUEST_THRESHOLD", 1.0)  # seconds
            
            logger.info("Homeostasis Django middleware initialized")
            
            if not self.enabled:
                raise MiddlewareNotUsed("Homeostasis middleware is disabled in settings")
    ...
    def _log_request(self, request: HttpRequest) -> None:
        ...
        try:
            ...
            request_data = {
                "request_id": getattr(request, "homeostasis_request_id", str(uuid.uuid4())),
    ...
    def _capture_exception(self, request: HttpRequest, exc: Exception, 
    ...
    def _attempt_recovery(self, request: HttpRequest, exc: Exception, 
                         error_data: Dict[str, Any]) -> Optional[HttpResponse]:
        """
    ...
    def _should_ignore_exception(self, exc: Exception) -> bool:
        ...
        if isinstance(exc, PermissionDenied) and "PermissionDenied" in self.ignore_exceptions:
            return True
