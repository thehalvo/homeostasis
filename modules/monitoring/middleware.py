...
class LoggingMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app: ASGIApp, service_name: str = "example_service", 
                 log_level: str = "INFO", exclude_paths: Optional[List[str]] = None,
        ...
        self.sensitive_headers = set([
            ...
            "token", "session", "password", "secret", "credential", "apikey"
        ])
        ...
        try:
            ...
            if is_error:
                ...
                self.logger.error(
                    ...
                    tags=["request", "error", f"status-{status_code}", request.method.lower()],
                )
            else:
                log_msg = f"Request completed: {request.method} {full_path} - {status_code}"
                self.logger.info(
                    log_msg,
                    include_call_location=True,
                    request_info=request_info,
                    response_info=response_info,
                    tags=["request", "success", f"status-{status_code}", request.method.lower()],
                )
...
def add_logging_middleware(app: FastAPI, service_name: str = "example_service", 
                     log_level: str = "INFO", exclude_paths: Optional[List[str]] = None,
                     sensitive_headers: Optional[List[str]] = None) -> None:
    """
