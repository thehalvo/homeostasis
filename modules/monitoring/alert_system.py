...
class AlertManager:
    ...
    def register_alert_handler(self, channel: str, handler: Callable[[str, Dict[str, Any]], None]) -> None:
        """
    ...
    def send_alert(self, 
        ...
        if channels is None:
            channels = [ch for ch, enabled in self.config["channels"].items() if enabled]
        ...
        for channel in channels:
            if channel in self.alert_handlers:
                try:
                    ...
                    self.logger.exception(e, message=f"Error in alert handler for {channel}: {str(e)}")
            else:
    ...
    def check_for_anomalies(self, 
        ...
        for metric_name in current_metrics:
            ...
            historical_values = [m.get(metric_name) for m in historical_metrics if metric_name in m 
                               and isinstance(m.get(metric_name), (int, float))]
    ...
    def generate_alert_digest(self) -> Dict[str, Any]:
        ...
        # Get recent alerts
        recent_alerts = sorted(self.alert_history, key=lambda x: x["timestamp"], reverse=True)[:10]
...
class AnomalyDetector:
    ...
    def establish_baseline(self, 
        ...
        try:
            ...
            self.logger.info(f"Established baseline for {service_url}", baseline=baseline)
            
            return baseline
            
        except Exception as e:
            self.logger.exception(e, message=f"Failed to establish baseline for {service_url}")
            return {}
    ...
    def monitor_for_anomalies(self, 
        ...
        Args:
            ...
            interval: Check interval in seconds
        """
        self.logger.info(f"Starting anomaly monitoring for patch {patch_id} at {service_url}")
        ...
        try:
            ...
            # Generate summary
            summary = self._generate_monitoring_summary(monitoring_metrics, baseline, patch_id)
            
            # Log summary
            self.logger.info(f"Monitoring summary for patch {patch_id}", summary=summary)
            
            return summary
            
        except Exception as e:
            self.logger.exception(e, message=f"Failed to monitor {service_url} for anomalies")
            return None
            ...
            if baseline_rt_stddev > 0:
                ...
                if z_score > 3:
                    anomalies.append({
                        ...
                        "z_score": z_score,
                        "increase_percent": ((current_rt - baseline_rt) / baseline_rt) * 100 if baseline_rt > 0 else 0
                    })
    ...
    def _generate_monitoring_summary(self, 
        ...
        for metric_name in ["response_time", "error_rate", "memory_usage"]:
            if metric_name in monitoring_stats and metric_name in baseline:
                ...
                percent_change = (change / baseline_mean) * 100 if baseline_mean > 0 else 0
        ...
        return {
            ...
            "monitoring_period": {
                ...
                "duration": (metrics[-1]["timestamp"] - metrics[0]["timestamp"]) if metrics else 0
            },
...
if __name__ == "__main__":
    ...
    try:
        # Establish baseline
        baseline = detector.establish_baseline("http://localhost:8000", duration=10, interval=2)
