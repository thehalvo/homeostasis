...
class CostTracker:
    ...
    def add_budget(self, budget_id: str, budget: Budget) -> None:
        ...
        self.logger.info(f"Added budget {budget_id}: ${budget.amount} per {budget.period.value}")
    ...
    def _load_cost_data(self) -> None:
        ...
        try:
            ...
            if cost_file.exists():
                ...
                self.logger.info(f"Loaded {len(recent_transactions)} recent cost transactions")
            ...
            if budget_file.exists():
                ...
                self.logger.info(f"Loaded budget usage for {len(self.budget_usage)} budgets")
    ...
    def _check_budget_period_reset(self, budget_id: str, budget: Budget) -> bool:
        ...
        if current_time - period_start >= period_duration:
            ...
            self.logger.info(f"Reset budget period for {budget_id}. "
                           f"Previous usage: ${old_usage:.4f}, "
                           f"Rollover: ${-self.budget_usage[budget_id]:.4f}" if budget.rollover else "No rollover")
    ...
    def _check_budget_alerts(self, budget_id: str, budget: Budget, cost: float) -> None:
        ...
        for threshold in budget.alert_thresholds:
            if usage_percentage >= threshold and threshold not in self.sent_alerts[budget_id]:
        ...
        with self._lock:
            ...
            if provider_config and provider_config.cost_per_token_limit:
                ...
                if cost_per_token > provider_config.cost_per_token_limit:
                    self.logger.error(f"Cost per token ({cost_per_token:.6f}) exceeds limit "
            ...
            for budget_id, budget in self.budgets.items():
                ...
                if budget.hard_limit and projected_usage > budget.amount:
                    self.logger.error(f"Transaction blocked by budget {budget_id}: "
            ...
            self.logger.info(f"Recorded cost: ${cost:.4f} for {provider}/{model} "
        ...
        with self._lock:
            if budget_id:
                ...
                return {
                    ...
                    'time_remaining': self._get_period_duration(budget.period) - (time.time() - period_start)
                }
            else:
                result = {}
    ...
    def get_cost_breakdown(self, 
        ...
        with self._lock:
            recent_transactions = [
                tx for tx in self.cost_history
                if tx['timestamp'] >= cutoff_time
            ]
    ...
    def get_optimization_recommendations(self) -> List[str]:
        ...
        provider_costs = {
            k: v for k, v in breakdown['breakdown'].items()
        ...
        if len(provider_costs) > 1:
            ...
            if most_expensive[1]['cost_per_token'] > cheapest[1]['cost_per_token'] * 1.5:
