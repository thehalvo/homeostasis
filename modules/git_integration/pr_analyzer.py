...
class PRAnalyzer:
    ...
    def analyze_pull_request(self, pr_number: int) -> PRAnalysisResult:
        ...
        try:
            ...
            self.logger.info(f"PR #{pr_number} analysis completed: {len(issues_found)} issues, risk={risk_score:.2f}")
            return result
    ...
    def _get_github_pr_changes(self, pr_number: int) -> List[PRChange]:
        ...
        if not self.github_token:
            self.logger.warning("GitHub token not configured, falling back to local analysis")
            return self._get_local_pr_changes(pr_number)
        ...
        try:
            ...
            for file_data in files_data:
                change = PRChange(
                    ...
                    language=self.language_manager.detect_language(file_data['filename']),
    ...
    def _get_gitlab_pr_changes(self, pr_number: int) -> List[PRChange]:
        ...
        if not self.gitlab_token:
            self.logger.warning("GitLab token not configured, falling back to local analysis")
            return self._get_local_pr_changes(pr_number)
    ...
    def _analyze_file_changes(self, change: PRChange) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.error(f"Error analyzing file changes for {change.file_path}: {e}")
            return []
    
    def _generate_suggestions(self, change: PRChange, issues: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    def _calculate_risk_score(self, issues: List[Dict[str, Any]], 
                            complexity_metrics: Dict[str, Any], 
                            changes: List[PRChange]) -> float:
    ...
    def _generate_healing_recommendations(self, issues: List[Dict[str, Any]], 
    ...
    def _format_pr_comment(self, result: PRAnalysisResult) -> str:
        ...
        # Risk score
        risk_emoji = "🟢" if result.risk_score < 0.3 else "🟡" if result.risk_score < 0.7 else "🔴"
        comment += f"**Risk Score:** {risk_emoji} {result.risk_score:.2f}\n\n"
        
        # Issues summary
        if result.issues_found:
            critical = len([i for i in result.issues_found if i.get('severity') == 'critical'])
            warnings = len([i for i in result.issues_found if i.get('severity') == 'warning'])
        ...
        if result.complexity_metrics:
            ...
            comment += f"- Languages: {', '.join(metrics.get('languages', {}).keys())}\n\n"
