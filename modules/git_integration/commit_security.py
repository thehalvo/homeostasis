...
class CommitSecurity:
    ...
    def _load_default_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'audit_trail': {
                'enabled': True,
                'log_file': '.homeostasis-audit.log'
            }
        }
...
"""
            ...
            try:
                ...
                list_result = subprocess.run(
                    ['gpg', '--list-secret-keys', '--with-colons', 'homeostasis@localhost'],
                    capture_output=True,
                    text=True
                )
    ...
    def sign_healing_commit(self, commit_message: str, healing_metadata: Dict[str, Any]) -> str:
        """
    ...
    def _add_healing_metadata(self, message: str, metadata: Dict[str, Any]) -> str:
        ...
        if metadata_config.get('include_checksum') and 'file_content' in metadata:
            checksum = hashlib.sha256(metadata['file_content'].encode()).hexdigest()[:16]
        ...
        if metadata.get('original_error'):
            metadata_lines.append(f"Healing-Original-Error: {metadata['original_error']}")
    ...
    def verify_healing_commit(self, commit_hash: str) -> Dict[str, Any]:
        ...
        try:
            ...
            gpg_verification = self._verify_gpg_signature(commit_hash) if self.signing_enabled else None
        ...
        try:
            ...
            for line in lines:
                if line.startswith(f"{self.healing_signature_prefix}:"):
                    signature_line = line[len(self.healing_signature_prefix) + 1:].strip()
                    break
    ...
    def create_healing_commit(self, file_changes: Dict[str, str], 
        ...
        try:
            ...
            base_message = commit_analyzer.generate_healing_commit_message(healing_metadata)
    ...
    def _log_to_audit_trail(self, commit_hash: str, healing_metadata: Dict[str, Any]) -> None:
