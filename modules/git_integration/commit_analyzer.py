...
class CommitAnalyzer:
    ...
    def analyze_commit(self, commit_hash: str) -> CommitAnalysis:
        ...
        """
        # Check cache first
        if self.config.get('cache_enabled', True) and commit_hash in self._analysis_cache:
    ...
    def _analyze_commit_message(self, commit_info: CommitInfo) -> CommitAnalysis:
        ...
        is_test_commit = self._is_test_commit(subject, message, commit_info.files_changed)
    ...
    def _is_test_commit(self, subject: str, message: str, files_changed: List[str]) -> bool:
        ...
        test_files = [f for f in files_changed if 'test' in f.lower() or f.endswith(('.test.js', '.spec.js', '_test.py', 'test_*.py'))]
    ...
    def analyze_commit_history(self, limit: Optional[int] = None, 
                             since: Optional[datetime] = None) -> List[CommitAnalysis]:
        """
        ...
        try:
            ...
            # Analyze each commit
            analyses = []
            for commit_hash in commit_hashes[:self.config.get('max_commit_history', 1000)]:
    ...
    def get_healing_context_for_error(self, error_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if error_file:
                ...
                context['suggested_scope'] = directory.split('/')[-1] if directory != '.' else None
            
            # Calculate confidence boost based on similar fixes
            if context['similar_fixes']:
                avg_confidence = sum(fix['confidence'] for fix in context['similar_fixes']) / len(context['similar_fixes'])
