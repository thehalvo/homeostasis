...
class GitIntegration:
    ...
    def __init__(self, repo_path: str, config_path: Optional[str] = None):
        ...
        self.pre_commit_hooks = PreCommitHooks(
            str(self.repo_path), 
            self.config.get('pre_commit', {})
        )
        
        self.pr_analyzer = PRAnalyzer(
            str(self.repo_path),
            self.config.get('pr_analysis', {})
        )
        
        self.branch_strategy = BranchStrategy(
            str(self.repo_path),
            self.config.get('branch_strategy', {})
        )
        
        self.commit_analyzer = CommitAnalyzer(
            str(self.repo_path),
            self.config.get('commit_analysis', {})
        )
        
        self.commit_security = CommitSecurity(
            str(self.repo_path),
            self.config.get('commit_signing', {})
        )
    ...
    def filter_issues_by_branch_strategy(self, issues: List[Dict[str, Any]], 
                                       branch_name: Optional[str] = None) -> List[Dict[str, Any]]:
        """Filter issues based on branch healing strategy."""
        return self.branch_strategy.filter_issues_by_strategy(issues, branch_name)
    
    def get_branch_risk_assessment(self, branch_name: Optional[str] = None) -> Dict[str, Any]:
    ...
    def get_healing_context_for_error(self, error_info: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def create_healing_commit(self, file_changes: Dict[str, str], 
                            healing_metadata: Dict[str, Any]) -> bool:
        """Create a signed healing commit."""
        return self.commit_security.create_healing_commit(file_changes, healing_metadata)
    ...
    def apply_branch_healing(self, issues: List[Dict[str, Any]], 
        ...
        try:
            ...
            branch_info = (self.branch_strategy.get_branch_info(branch_name) 
                         if branch_name else self.branch_strategy.get_current_branch_info())
            
            strategy = self.branch_strategy.get_healing_strategy(branch_name)
            
            # Filter issues based on branch strategy
            filtered_issues = self.branch_strategy.filter_issues_by_strategy(issues, branch_name)
            ...
            # Get risk assessment
            risk_assessment = self.branch_strategy.get_branch_risk_assessment(branch_name)
            ...
            for issue in filtered_issues:
                ...
                healing_context = self.commit_analyzer.get_healing_context_for_error(issue)
                
                # Apply confidence boost from similar fixes
                issue['confidence'] = min(
                    issue.get('confidence', 0.5) + healing_context.get('confidence_boost', 0.0),
                    1.0
                )
                ...
                healing_metadata = {
                    ...
                    'healing_type': 'auto' if strategy.auto_healing_enabled else 'manual'
                }
    ...
    def create_comprehensive_report(self, branch_name: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        try:
            # Branch analysis
            branch_report = self.branch_strategy.create_branch_healing_report(branch_name)
            ...
            return {
                ...
                'recent_commits': {
                    ...
                    'feature_commits': len([c for c in recent_commits if c.is_feature_commit]),
                    'avg_confidence': sum(c.confidence for c in recent_commits) / len(recent_commits) if recent_commits else 0
                },
        ...
        try:
            ...
            if self.config.get('commit_signing', {}).get('enabled', False):
                if not self.commit_security.gpg_available:
                    validation_results['warnings'].append("Commit signing enabled but GPG not available")
                    validation_results['recommendations'].append("Install GPG or disable commit signing")
            
            # Check pre-commit hooks
            if self.config.get('pre_commit', {}).get('enabled', True):
                if not self.pre_commit_hooks.hooks_installed:
                    validation_results['warnings'].append("Pre-commit hooks not installed")
                    validation_results['recommendations'].append("Run install_pre_commit_hooks() to enable")
            ...
            if self.config.get('pr_analysis', {}).get('enabled', True):
                ...
                if repo_type == 'github' and not self.pr_analyzer.github_token:
                    validation_results['warnings'].append("GitHub token not configured for PR analysis")
                    validation_results['recommendations'].append("Set GITHUB_TOKEN environment variable")
                elif repo_type == 'gitlab' and not self.pr_analyzer.gitlab_token:
                    validation_results['warnings'].append("GitLab token not configured for PR analysis")
                    validation_results['recommendations'].append("Set GITLAB_TOKEN environment variable")
            
            # Check branch protection
            current_branch = self.branch_strategy.get_current_branch_info()
            if current_branch.is_protected and current_branch.branch_type.value in ['main', 'production']:
                healing_strategy = self.branch_strategy.get_healing_strategy()
                if healing_strategy.auto_healing_enabled:
                    validation_results['warnings'].append(f"Auto-healing enabled on protected branch: {current_branch.name}")
                    validation_results['recommendations'].append("Consider disabling auto-healing on protected branches")
