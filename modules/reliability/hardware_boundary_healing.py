    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "predicted_failure_time": (
                self.predicted_failure_time.total_seconds()
...
class CPUMonitor(HardwareMonitor):
    ...
    async def get_metrics(self, component: HardwareComponent) -> Dict[str, float]:
        ...
        try:
            ...
            mce_count = 0
            mce_path = Path(
                "/sys/devices/system/machinecheck/machinecheck0/bank0/ce_count"
            )
            if mce_path.exists():
            ...
            if temp > max_temp:
                faults.append(
                    HardwareFault(
                        ...
                        recommended_actions=[
                            HealingAction.THROTTLE_CPU,
                            HealingAction.ADJUST_COOLING,
                        ],
                    )
                )
        ...
        if ue_errors > 0:
            faults.append(
                HardwareFault(
                    ...
                    recommended_actions=[
                        HealingAction.MIGRATE_MEMORY,
                        HealingAction.COMPONENT_ISOLATION,
                    ],
                )
            )
...
class DiskMonitor(HardwareMonitor):
    ...
    async def get_metrics(self, component: HardwareComponent) -> Dict[str, float]:
        ...
        try:
            ...
            smart_output = subprocess.check_output(
                ["smartctl", "-A", "-j", component.location], text=True
            )
            ...
            df_output = subprocess.check_output(
                ["df", "-B1", component.location], text=True
            )
    ...
    async def detect_faults(self, component: HardwareComponent) -> List[HardwareFault]:
        ...
        if realloc > 0:
            severity = (
                "critical" if realloc > 100 else "high" if realloc > 10 else "medium"
            )
            faults.append(
                HardwareFault(
                    ...
                    recommended_actions=[
                        HealingAction.REMAP_DISK,
                        HealingAction.WORKLOAD_MIGRATION,
                    ],
                )
            )
...
class HardwareHealer:
    ...
    async def monitor_components(self) -> List[HardwareFault]:
        ...
        for component in self.components.values():
            ...
            try:
                ...
                logger.error(
                    f"Error monitoring component {component.component_id}: {e}"
                )

        return all_faults

    async def heal_fault(
        self, fault: HardwareFault, action: HealingAction, force: bool = False
    ) -> HealingResult:
        ...
        logger.info(
            f"Executing healing action {action.value} for {component.component_id}"
        )
        ...
        try:
            ...
            if not force and action not in fault.recommended_actions:
                logger.warning(
                    f"Action {action.value} not recommended for fault {fault.fault_type.value}"
                )
    ...
    async def _execute_healing_action(
        self, component: HardwareComponent, action: HealingAction, fault: HardwareFault
    ) -> bool:
    ...
    async def _throttle_cpu(
        self, component: HardwareComponent, fault: HardwareFault
    ) -> bool:
        ...
        try:
            ...
            max_freq_path = (
                f"/sys/devices/system/cpu/{cpu_num}/cpufreq/scaling_max_freq"
            )
            cur_freq_path = (
                f"/sys/devices/system/cpu/{cpu_num}/cpufreq/scaling_cur_freq"
            )
    ...
    async def _migrate_memory(
        ...
        try:
            ...
            memory_block = (
                component.location
            )  # e.g., "/sys/devices/system/memory/memory32"
        ...
        try:
            ...
            subprocess.run(
                [
                    ...
                    str(fault.details.get("bad_sector", 0)),
                    component.location,
                ],
                check=True,
            )
    ...
    async def _adjust_cooling(
        ...
        try:
            ...
            subprocess.run(
                ["ipmitool", "raw", "0x30", "0x30", "0x01", "0xff"], check=True
            )
    ...
    async def _recover_firmware(
        ...
        try:
            ...
            if Path(backup_path).exists():
                subprocess.run(
                    ["flashrom", "-p", "internal", "-w", backup_path], check=True
                )
    ...
    async def _isolate_component(
        ...
        try:
            ...
            if component.component_type == HardwareType.CPU:
                ...
                subprocess.run(
                    ["mdadm", "--fail", "/dev/md0", component.location], check=False
                )
    ...
    def get_system_health(self) -> Dict[str, Any]:
        ...
        degraded_components = [
            c for c in self.components.values() if c.status == "degraded"
        ]
        failed_components = [
            c for c in self.components.values() if c.status in ["failed", "isolated"]
        ]
        ...
        return {
            ...
            "critical_faults": len(
                [f for f in self.active_faults.values() if f.severity == "critical"]
            ),
        ...
        try:
            ...
            for fault in faults:
                if fault.severity == "critical" and fault.recommended_actions:
                    ...
                    if result.success:
                        logger.info(
                            f"Successfully healed {fault.fault_type.value} on {fault.component.component_id}"
                        )
                    else:
                        logger.error(
                            f"Failed to heal {fault.fault_type.value}: {result.error_message}"
                        )

            # Get system health
            health = healer.get_system_health()
            logger.info(
                f"System health: {health['healthy']} healthy, {health['degraded']} degraded, {health['failed']} failed"
            )
