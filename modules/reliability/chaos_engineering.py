    ...
    async def inject(self, fault: FaultInjection) -> bool:
        ...
        try:
            if fault.fault_type == FaultType.NETWORK_LATENCY:
                latency_ms = int(
                    fault.parameters.get("latency_ms", 100) * fault.intensity
                )
                ...
                await self._create_partition(
                    fault.target, fault.parameters.get("isolated_from", [])
                )
            else:
                return False
            return True
...
class ChaosMonkey:
    ...
    async def run_experiment(
        ...
        try:
            ...
            if not self._verify_steady_state(
                result.steady_state_before, experiment.hypothesis
            ):
            ...
            result.hypothesis_verified = self._verify_steady_state(
                result.steady_state_after, experiment.hypothesis
            )
    ...
    async def _inject_fault(self, fault: FaultInjection) -> bool:
        ...
        if fault.fault_type in [
            ...
            FaultType.RESOURCE_CPU,
            FaultType.RESOURCE_MEMORY,
            FaultType.RESOURCE_DISK,
        ]:
    ...
    def _verify_steady_state(
        self, metrics: Dict[str, float], hypothesis: SteadyStateHypothesis
    ) -> bool:
        ...
        for metric_name, (min_val, max_val) in hypothesis.thresholds.items():
            ...
            if value < min_val or value > max_val:
                logger.warning(
                    f"Metric {metric_name} out of bounds: {value} not in [{min_val}, {max_val}]"
                )
                return False
        return True
            ...
            try:
                ...
                if not self._verify_steady_state(metrics, experiment.hypothesis):
                    result.failures_detected.append(
                        f"Steady state violation at {datetime.now()}"
                    )

                    # Trigger healing if available
                    if self.healing_system:
                        healing_action = await self.healing_system.diagnose_and_heal(
                            metrics
                        )
                        if healing_action:
                ...
                await asyncio.sleep(
                    experiment.hypothesis.verification_interval.total_seconds()
                )
    ...
    def get_experiment_history(
        self, limit: int = 100, status: Optional[ExperimentStatus] = None
    ...
    def get_failure_insights(self) -> Dict[str, Any]:
        ...
        insights["common_failures"] = dict(
            sorted(failure_counts.items(), key=lambda x: x[1], reverse=True)[:10]
        )

        # Calculate healing effectiveness
        experiments_with_failures = [
            r for r in self.experiment_history if r.failures_detected
        ]
        if experiments_with_failures:
            healed = sum(
                1 for r in experiments_with_failures if r.healing_actions_triggered
            )
