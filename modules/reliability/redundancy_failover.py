...
class RedundancyGroup:
    ...
    def get_healthy_instances(self) -> List[Instance]:
        ...
        return [i for i in self.instances if i.state in [InstanceState.ACTIVE, InstanceState.STANDBY]]
...
class FailoverEvent:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "replacement_instance": self.replacement_instance.instance_id if self.replacement_instance else None,
...
class FailoverCoordinator:
                ...
                for instance in group.instances:
                    if instance.state in [InstanceState.ACTIVE, InstanceState.STANDBY]:
                        for health_check in instance.health_checks:
                            ...
                            await self._update_instance_health(group, instance, is_healthy)
    ...
    async def _update_instance_health(
        ...
        if is_healthy:
            ...
            if instance.consecutive_failures >= group.health_check_config.unhealthy_threshold:
    ...
    async def _check_failover_needed(self, group: RedundancyGroup) -> None:
        ...
        failing_instances = [i for i in group.instances if i.state == InstanceState.FAILING]
    ...
    async def initiate_failover(
        ...
        try:
            ...
            if group.failover_strategy == FailoverStrategy.IMMEDIATE:
                event.replacement_instance = await self._immediate_failover(group, failed_instance)
            elif group.failover_strategy == FailoverStrategy.GRACEFUL:
                event.replacement_instance = await self._graceful_failover(group, failed_instance)
            elif group.failover_strategy == FailoverStrategy.STAGED:
                event.replacement_instance = await self._staged_failover(group, failed_instance)
            elif group.failover_strategy == FailoverStrategy.MANUAL:
                event.replacement_instance = await self._manual_failover(group, failed_instance)
            else:
                event.replacement_instance = await self._automatic_failover(group, failed_instance)
            ...
            if event.success:
                ...
                logger.info(f"Failover completed: {failed_instance.instance_id} -> {event.replacement_instance.instance_id}")
    ...
    async def _graceful_failover(
        ...
        while failed_instance.current_load > 0 and time.time() - start_time < drain_timeout:
    ...
    async def _staged_failover(
        ...
        for i in range(stages):
            ...
            failed_instance.current_load = failed_instance.current_load * (1 - transfer_ratio)
    ...
    def _select_best_standby(
        self,
        standby_instances: List[Instance],
        failed_instance: Instance
    ) -> Instance:
        ...
        same_region = [i for i in standby_instances if i.region == failed_instance.region]
        if same_region:
    ...
    def _get_strategy_metrics(self) -> Dict[str, Dict[str, float]]:
        ...
        for strategy in FailoverStrategy:
            ...
            if events:
                ...
                avg_duration = sum(e.duration.total_seconds() for e in events) / len(events)
