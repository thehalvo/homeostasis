...
class CodeVisitor(ast.NodeVisitor):
    ...
    def visit_Import(self, node):
        ...
        for name, asname in names:
            if asname:
                ...
                self._add_variable(name.split('.')[0], node, is_imported=True, import_source=name)
    ...
    def visit_ImportFrom(self, node):
        ...
        for name, asname in names:
            ...
            self._add_variable(var_name, node, is_imported=True, 
    ...
    def visit_FunctionDef(self, node):
        ...
        if node.args.vararg:
            parameters.append({
                ...
                'annotation': self._get_annotation_name(node.args.vararg.annotation),
                'is_vararg': True
            })
    ...
    def visit_Assign(self, node):
        ...
        for target in node.targets:
            if isinstance(target, ast.Name):
                ...
                if target.value.id == 'self' and self.current_function and self.current_class:
    ...
    def _add_variable(self, name: str, node: ast.AST, 
                     value_type: Optional[str] = None,
                     is_parameter: bool = False,
                     is_imported: bool = False,
                     import_source: Optional[str] = None) -> None:
    ...
    def _add_class_attribute(self, name: str, node: ast.AST, 
        ...
        if name in self.current_class.attributes:
            ...
            self.current_class.attributes[name] = VariableInfo(
                ...
                assignments=[node],
                usages=[],
                defined_in_scope=self.current_function.node if self.current_function else None,
                value_type=value_type
            )
            
    def _try_infer_type(self, name: str, value_node: ast.AST, 
        ...
        if inferred_type:
            if is_attribute and self.current_class and name in self.current_class.attributes:
...
class ASTAnalyzer:
    ...
    def find_undefined_variables(self) -> List[str]:
        ...
        for name, var_info in self.visitor.variables.items():
            if var_info.usages and not var_info.assignments and not var_info.is_parameter:
    ...
    def find_containing_function(self, line_number: int) -> Optional[FunctionInfo]:
        ...
        for func_name, func_info in self.visitor.functions.items():
            if hasattr(func_info.node, 'lineno') and hasattr(func_info.node, 'end_lineno'):
    ...
    def find_containing_class(self, line_number: int) -> Optional[ClassInfo]:
        ...
        for class_name, class_info in self.visitor.classes.items():
            if hasattr(class_info.node, 'lineno') and hasattr(class_info.node, 'end_lineno'):
...
"""
    ...
    if analyzer.parse_code(test_code):
        ...
        for name, func_info in analyzer.get_functions().items():
            params = ", ".join(f"{p['name']}: {p['annotation']}" for p in func_info.parameters 
                             if p.get('annotation'))
            returns = f" -> {func_info.return_annotation}" if func_info.return_annotation else ""
        ...
        for param in helper_params:
            default = f" = {param['default']}" if 'default' in param and param['default'] is not None else ""
