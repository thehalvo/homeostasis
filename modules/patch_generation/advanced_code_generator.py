...
class AdvancedCodeGenerator:
    ...
    def generate_with_context(self,
                            ...
                            source_code: str,
                            mode: CodeGenerationMode = CodeGenerationMode.BUG_FIX) -> GenerationResult:
        """
    ...
    def _build_code_context(self, 
                          file_path: str, 
                          source_code: str,
                          error_context: Dict[str, Any]) -> CodeContext:
        """
        ...
        context.imports_map = self._extract_imports(source_code, language_info.language.value)
    ...
    def _perform_semantic_analysis(self,
        ...
        fix_impact = self._analyze_fix_impact(
            code_context, error_context, source_code
        )
    ...
    def _requires_multi_file_changes(self,
    ...
    def _generate_single_file_changes(self,
                                    code_context: CodeContext,
                                    semantic_analysis: Dict[str, Any],
                                    error_context: Dict[str, Any],
                                    source_code: str,
                                    mode: CodeGenerationMode) -> GenerationResult:
        """
    ...
    def _generate_multi_file_changes(self,
                                   code_context: CodeContext,
                                   semantic_analysis: Dict[str, Any],
                                   error_context: Dict[str, Any],
                                   mode: CodeGenerationMode) -> GenerationResult:
        ...
        for file_path in affected_files:
            ...
            file_changes = self._generate_coordinated_changes(
                file_path, file_content, file_context, 
                code_context, semantic_analysis, error_context, mode
            )
    ...
    def _build_advanced_prompt(self,
        ...
        # System context
        prompt_parts.append(f"""You are an expert software engineer with deep understanding of code semantics and architecture.
...
```""")
    ...
    def _extract_class_hierarchy(self, source_code: str, language: str) -> Dict[str, List[str]]:
    ...
    def _extract_function_signatures(self, source_code: str, language: str) -> Dict[str, str]:
    ...
    def _analyze_variable_scopes(self, source_code: str, language: str) -> Dict[str, Dict[str, Any]]:
            ...
            if isinstance(child, ast.FunctionDef):
                ...
                for target in child.targets:
                    if isinstance(target, ast.Name):
                        scopes[current_scope].setdefault('variables', []).append(target.id)
            else:
    ...
    def _identify_architectural_patterns(self, 
        ...
        if any('service' in f.lower() for f in related_files) and len(related_files) > 5:
    ...
    def _analyze_data_flow(self, code_context: CodeContext, source_code: str) -> Dict[str, Any]:
    ...
    def _identify_side_effects(self, code_context: CodeContext, source_code: str) -> List[Dict[str, Any]]:
    ...
    def _analyze_error_propagation(self, 
                                 code_context: CodeContext, 
                                 error_context: Dict[str, Any],
                                 source_code: str) -> List[List[Dict[str, Any]]]:
    ...
    def _analyze_fix_impact(self, 
        ...
        if error_context.get('error_type') in ['AttributeError', 'TypeError']:
            ...
            if not any(name.startswith('_') for name in error_context.get('root_cause', '').split('.')):
    ...
    def _apply_style_preservation(self,
                                changes: List[Dict[str, Any]],
                                code_context: CodeContext,
    ...
    def _validate_semantic_correctness(self,
                                     changes: List[Dict[str, Any]],
                                     code_context: CodeContext,
        ...
        for change in changes:
            if 'def ' in change.get('original_code', ''):
                ...
                original_sig = re.search(r'def\s+(\w+)\s*\((.*?)\)', 
                                       change.get('original_code', ''))
                new_sig = re.search(r'def\s+(\w+)\s*\((.*?)\)', 
                                  change.get('new_code', ''))
        ...
        for change in changes:
            ...
            if 'requests.' in new_code and 'requests.' not in change.get('original_code', ''):
    ...
    def _identify_affected_files(self,
                               code_context: CodeContext,
                               semantic_analysis: Dict[str, Any],
    ...
    def _build_file_specific_context(self,
                                   file_path: str,
                                   file_content: str,
                                   code_context: CodeContext,
                                   semantic_analysis: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _find_imports_from_target(self, file_content: str, target_file: str) -> List[str]:
    ...
    def _find_shared_dependencies(self, 
                                file_path: str, 
    ...
    def _determine_change_requirements(self,
                                     file_path: str,
                                     code_context: CodeContext,
    ...
    def _generate_coordinated_changes(self,
                                    file_path: str,
                                    file_content: str,
...
```"""
    ...
    def _validate_cross_file_consistency(self,
        ...
        for change_info in all_changes:
            ...
            if 'def ' in change.get('original_code', '') or 'class ' in change.get('original_code', ''):
    ...
    def _generate_multi_file_reasoning(self,
                                     multi_file_changes: Dict[str, List[Dict[str, Any]]],
    ...
    def _generate_multi_file_test_suggestions(self,
                                            multi_file_changes: Dict[str, List[Dict[str, Any]]],
                                            code_context: CodeContext) -> List[str]:
        ...
        # Integration tests
        suggestions.append(
            "Add integration tests to verify cross-file functionality"
        )
        
        # Interface tests
        if any('def ' in str(changes) or 'class ' in str(changes) 
               for changes in multi_file_changes.values()):
            suggestions.append(
                "Test all public interfaces to ensure compatibility"
            )
        
        # Regression tests
        suggestions.append(
            "Add regression tests for the original error scenario"
        )
        ...
        for file_path in multi_file_changes:
            ...
            suggestions.append(
                f"Update tests for {file_name} module"
            )
        
        return suggestions
    
    def _identify_dependency_updates(self,
                                   multi_file_changes: Dict[str, List[Dict[str, Any]]],
                                   code_context: CodeContext) -> Dict[str, Any]:
            ...
            for change in changes:
                ...
                for imp in new_imports:
                    if imp not in original_imports:
                        updates['imports'].append({
                            'file': file_path,
                            'import': imp
                        })
        
        # Suggest version bumps for interface changes
        if any('class ' in str(changes) or 'def ' in str(changes) 
               for changes in multi_file_changes.values()):
            updates['version_bumps'].append({
                'type': 'minor',
                'reason': 'Interface changes detected'
            })
...
Fix: value = getattr(obj, 'non_existent_attr', default_value)  # Safe access with default"""
    ...
    def _resolve_import_path(self, import_name: str, base_dir: Path, language: str) -> List[str]:
...
def create_advanced_code_generator(llm_manager: LLMManager,
                                 context_manager: LLMContextManager,
                                 config: Optional[Dict[str, Any]] = None) -> AdvancedCodeGenerator:
    """
