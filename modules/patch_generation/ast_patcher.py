...
class ASTPatcher:
    ...
    def get_variables_in_scope(self, line_number: int) -> Dict[str, VariableInfo]:
        ...
        for name, var_info in self.analyzer.get_variables().items():
            if var_info.defined_in_scope and isinstance(var_info.defined_in_scope, ast.Module):
    ...
    def generate_exception_handler(self, line_number: int, exception_type: str) -> Optional[Dict[str, Any]]:
        """
        ...
        for node in ast.walk(containing_function.node):
            for child in ast.iter_child_nodes(node):
                if any(child is n or hasattr(child, 'body') and any(b is n for b in child.body) 
        ...
        recovery_action = self._generate_recovery_action(exception_type, statement_node, variables_in_scope)
        
        # Detect if we need to import the exception type
        import_statement = None
        if '.' in exception_type and exception_type not in str(self.analyzer.get_imports()):
    ...
    def generate_parameter_check(self, function_name: str, parameter_name: str) -> Optional[Dict[str, Any]]:
        """
        ...
        if param_type == 'str':
            ...
            default_value = '""' if param_info.get('default') is None else repr(param_info.get('default', ''))
        elif param_type in ('int', 'float'):
            check_code = f"if {parameter_name} is None:"
            default_value = '0' if param_info.get('default') is None else str(param_info.get('default', 0))
        elif param_type in ('list', 'List', 'tuple', 'Tuple', 'set', 'Set'):
            check_code = f"if {parameter_name} is None:"
            default_value = '[]' if param_info.get('default') is None else str(param_info.get('default', []))
            ...
            check_code = f"if {parameter_name} is None:"
            default_value = 'None' if param_info.get('default') is None else str(param_info.get('default', 'None'))
        ...
        for node in ast.walk(function_info.node):
            if isinstance(node, ast.If) and hasattr(node, 'test'):
                test_code = self.analyzer.extract_code_at_lines(node.test.lineno, node.test.lineno)
    ...
    def generate_type_conversion(self, line_number: int, variable_name: str, target_type: str) -> Optional[Dict[str, Any]]:
        """
    ...
    def add_missing_import(self, module_name: str, name: str, alias: Optional[str] = None) -> Optional[Dict[str, Any]]:
        """
    ...
    def fix_attribute_error(self, line_number: int, obj_name: str, attr_name: str) -> Optional[Dict[str, Any]]:
        """
    ...
    def _generate_recovery_action(self, exception_type: str, node: ast.AST, 
                                variables_in_scope: Dict[str, VariableInfo]) -> str:
        """
    ...
    def _find_similar_attributes(self, attr_name: str, class_info: ClassInfo) -> List[str]:
        """
        ...
        similar_attrs.sort(key=lambda a: self._similarity_score(a, attr_name), reverse=True)
...
"""
    ...
    test_file = Path("/tmp/test_ast_patcher.py")
