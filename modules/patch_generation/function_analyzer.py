    ...
    def generate_parameter_checks(self, function_name: str) -> List[Dict[str, Any]]:
        ...
        for param in function_info.parameters:
            ...
            if annotation in ("str", "String", "str", "Text"):
                check_code = (
                    f"if {param_name} is None or not isinstance({param_name}, str):"
                )
                default_value = "''"
                ...
                check_code = (
                    f"if {param_name} is None or not hasattr({param_name}, '__iter__'):"
                )
                default_value = "[]"
            elif annotation in ("dict", "Dict", "mapping", "Mapping"):
                check_code = (
                    f"if {param_name} is None or not hasattr({param_name}, 'items'):"
                )
                default_value = "{}"
...
"""
    ...
    if analyzer.analyze_code(test_code):
        ...
        for issue in issues:
            print(
                f"- {issue.function_name}, {issue.parameter_name}: {issue.issue_type} (line {issue.line_number})"
            )
            if issue.suggestion:
