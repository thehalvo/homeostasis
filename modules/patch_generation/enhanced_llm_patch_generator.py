...
class EnhancedLLMPatchGenerator(LLMPatchGenerator):
    ...
    def generate_enhanced_patch(
        self,
        error_context: Dict[str, Any],
        source_code: Optional[str] = None,
        additional_context: Optional[Dict[str, Any]] = None,
        mode: Optional[CodeGenerationMode] = None,
    ) -> EnhancedPatchResult:
        """
        ...
        try:
            ...
            if mode is None:
                mode = self._determine_generation_mode(
            ...
            if (
                self.enable_iterative_refinement and
            ...
            return EnhancedPatchResult(
                ...
                metadata={
                    ...
                    "generation_time": generation_result.__dict__.get(
                        "generation_time", 0
                    ),
                    "refinement_time": (
                        refinement_history.total_time if refinement_history else 0
                    ),
                },
            )
    ...
    def _create_patch_from_generation(
        self,
        generation_result: GenerationResult,
        error_context: Dict[str, Any],
    ...
    def _perform_iterative_refinement(
        ...
        initial_validation = self._run_test_validation(
            target_file, str(Path(target_file).parent) if target_file else None
        )
        ...
        initial_feedback = ValidationFeedback(
            test_results={
                ...
                "failed_tests": [],
                "errors": (
                    [initial_validation.error_output]
        ...
        test_command = self._get_test_command(
            target_file, str(Path(target_file).parent) if target_file else "."
        )
    ...
    def _create_multi_file_patches(
        self,
    ...
    def _calculate_overall_confidence(
        ...
        if refinement_history:
            if refinement_history.convergence_achieved:
                ...
                best_iter = refinement_history.iterations[
                    refinement_history.best_iteration
                ]
        ...
        if impact_analysis:
            risk_confidence = {"low": 0.9, "medium": 0.7, "high": 0.5}
            confidence_factors.append(
    ...
    def _collect_all_warnings(
        ...
        if impact_analysis:
            for breaking_change in impact_analysis.breaking_changes:
                warnings.append(f"Breaking change: {breaking_change['description']}")
    ...
    def learn_from_success(
        self, patch_result: EnhancedPatchResult, success_metrics: Dict[str, Any]
    ):
        """
        ...
        task_type = self._determine_task_type(
            patch_result.patch.get("error_context", {})
        )

        # Create prompt example from successful patch
        example = PromptExample(
            context=patch_result.patch.get("error_context", {}).get(
                "language", "unknown"
            ),
            problem=patch_result.patch.get("error_context", {}).get(
                "error_message", ""
            ),
...
def create_enhanced_llm_patch_generator(
    config: Optional[Dict[str, Any]] = None,
) -> EnhancedLLMPatchGenerator:
    """
