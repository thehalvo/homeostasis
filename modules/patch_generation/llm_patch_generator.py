...
class LLMPatchGenerator:
    ...
    def __init__(
        ...
        self.validation_config = PatchValidationConfig(
            enable_test_validation=validation_config.get(
            ...
            require_all_tests_pass=validation_config.get(
                "require_all_tests_pass", False
            ),
            test_command=validation_config.get("test_command"),
            pre_validation_commands=validation_config.get(
                "pre_validation_commands", []
            ),
            post_validation_commands=validation_config.get(
    ...
    def generate_patch_from_error_context(
        self,
        ...
        try:
            ...
            prompt_data = self.context_manager.prepare_llm_prompt(
                context_id, template_type="patch_generation"
            )
            ...
            if patch_response:
                ...
                processed_patch = self._process_llm_response(
                    patch_response, error_context, source_code
                )
    ...
    def generate_patch_from_analysis(
        ...
        return self.generate_patch_from_error_context(
            error_context, source_code, {"analysis_method": "rule_based"}
        )

    def _enhance_prompt_with_source_code(
        self,
        ...
        relevant_code = self._extract_relevant_code_section(
            source_code, error_context.get("line_number", 0), context_lines=10
        )
        ...
        if language_info:
            ...
            llm_context = self.language_detector.get_llm_context_for_language(
                language_info
            )
        else:
        ...
        if source_code:
            style_conventions = self.style_analyzer.analyze_file_style(
                error_context.get("file_path", ""), language, source_code
            )
...
"""
    ...
    def _extract_relevant_code_section(
    ...
    def _generate_llm_patch(
        self, prompt_data: Dict[str, Any], error_context: Dict[str, Any]
    ) -> Optional[str]:
        """
        ...
        try:
            ...
            logger.info(
                f"Generated LLM patch response: {len(response.content)} characters"
            )
    ...
    def _get_framework_specific_guidance(
        self, frameworks: List[str], language: str
    ) -> str:
        """
        ...
        if not frameworks:
            return (
                "No specific frameworks detected. Follow standard language conventions."
            )
    ...
    def _format_style_conventions(self, style_conventions) -> str:
        ...
        if style_conventions.architectural_patterns:
            lines.append(
                f"- Architectural patterns: {', '.join(style_conventions.architectural_patterns)}"
            )
    ...
    def _format_changes_with_style(
        self,
    ...
    def apply_llm_patch(self, patch: Dict[str, Any], target_file: str) -> bool:
        """
    ...
    def _apply_changes_to_content(
        self, content: str, changes: List[Dict[str, Any]]
    ) -> str:
        """
        ...
        sorted_changes = sorted(
            changes, key=lambda x: x.get("line_start", 0), reverse=True
        )
    ...
    def _run_test_validation(
        self, target_file: str, working_dir: Optional[str] = None
    ) -> TestResult:
        """
        ...
        try:
            ...
            result = self._run_command(
                test_command, working_dir, timeout=self.validation_config.test_timeout
            )

            # Parse test results
            test_count, passed_count, failed_count = self._parse_test_output(
                result.stdout
            )
            ...
            if self.validation_config.require_all_tests_pass:
                ...
                success = result.returncode == 0 or (
                    test_count > 0 and passed_count > failed_count
                )
            ...
            return TestResult(
                ...
                validation_result=(
                    ValidationResult.SUCCESS if success else ValidationResult.FAILURE
                ),
            )
    ...
    def _run_command(
        self, command: str, working_dir: str, timeout: Optional[float] = None
    ...
    def _get_test_command(self, target_file: str, working_dir: str) -> Optional[str]:
        ...
        # Python projects
        if (working_path / "pytest.ini").exists() or (
            working_path / "pyproject.toml"
        ).exists():
            return "pytest -v"
        elif (working_path / "setup.py").exists():
            return "python -m pytest"
        elif (working_path / "requirements.txt").exists() and Path(
            target_file
        ).suffix == ".py":
    ...
    def generate_patch_with_validation(
        self,
        error_context: Dict[str, Any],
        source_code: Optional[str] = None,
        additional_context: Optional[Dict[str, Any]] = None,
        ...
        for attempt in range(self.validation_config.max_validation_attempts):
            logger.info(
                f"Patch generation attempt {attempt + 1}/{self.validation_config.max_validation_attempts}"
            )
            ...
            try:
                ...
                backup_file = target_path.with_suffix(
                    target_path.suffix + ".validation_backup"
                )
                ...
                if self.apply_llm_patch(patch, target_file):
                    ...
                    if test_result.success:
                        logger.info(
                            f"Patch validation successful on attempt {attempt + 1}"
                        )
                        return patch
                    else:
                        logger.warning(
                            f"Patch validation failed on attempt {attempt + 1}: {test_result.error_output}"
                        )

                        # If retry is enabled and we have more attempts, update context with failure info
                        if (self.validation_config.retry_on_test_failure and
                            attempt <
                            self.validation_config.max_validation_attempts - 1):
                            # Update error context with validation failure information
                            error_context = (
                                self._update_context_with_validation_failure(
        ...
        logger.error(
            f"Failed to generate valid patch after {self.validation_config.max_validation_attempts} attempts"
        )
        return None

    def _update_context_with_validation_failure(
        self,
        error_context: Dict[str, Any],
        ...
        updated_context["validation_failures"] = updated_context.get(
            "validation_failures", []
        )
...
def create_llm_patch_generator(
    config: Optional[Dict[str, Any]] = None,
) -> LLMPatchGenerator:
    """
    ...
    return LLMPatchGenerator(
        api_key_manager=api_key_manager, context_manager=context_manager, config=config
    )
...
"""
    ...
    # Generate patch
    patch = generator.generate_patch_from_error_context(test_error, test_source)
