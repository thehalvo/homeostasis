...
class ConfidenceThreshold:
    ...
    adjustment_history: List[Tuple[float, float, str]]  # (timestamp, adjustment, reason)
    context_hash: str
    last_updated: float
...
class ConfidenceCalculator:
    ...
    def _calculate_adjustments(self, context: ConfidenceContext) -> Dict[str, float]:
        ...
        if context.historical_success_rate is not None:
            ...
            if context.historical_success_rate < 0.5:
                adjustments['historical_success'] = -0.3 * (0.5 - context.historical_success_rate)
            elif context.historical_success_rate > 0.8:
                adjustments['historical_success'] = 0.1 * (context.historical_success_rate - 0.8)
    ...
    def _generate_reasoning(
        ...
        for factor, adjustment in adjustments.items():
            ...
            if factor == 'historical_success':
                ...
                if adjustment < 0:
                    reasons.append(
                        f"Fix is {context.fix_complexity.value} complexity"
                    )
                elif adjustment > 0:
                    reasons.append(
                        f"Fix is {context.fix_complexity.value} (low risk)"
                    )
                ...
                if adjustment < 0:
                    reasons.append(
                        f"Deployment during {context.time_of_day}"
                    )
    ...
    def _save_threshold(self, context_hash: str, threshold: ConfidenceThreshold) -> None:
...
class ContextualThresholds:
    ...
    def get_threshold_for_context(
        ...
        result = self.confidence_calculator.calculate_confidence(
            deployment_context.get('model_confidence', 0.5),
            context
        )
...
class ReviewTrigger:
    ...
    def _build_context(
        ...
        for component in components:
            comp_criticality = self.contextual_thresholds._determine_criticality(component)
    ...
    def _check_additional_triggers(
        ...
        always_review_errors = ['security_vulnerability', 'data_corruption', 'authentication_failure']
        if error_data.get('error_type') in always_review_errors:
            reasons.append(f"Error type '{error_data['error_type']}' always requires review")
        
        # Trigger for fixes affecting multiple critical files
        critical_file_patterns = ['config', 'security', 'auth', 'database']
        critical_files = [
            f for f in fix_data.get('affected_files', [])
