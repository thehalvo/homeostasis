    ...
    def to_training_feedback(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'metrics': {
                ...
                'stability': np.mean(self.stability_trend) if self.stability_trend else 1.0
            }
        }
...
class DeploymentMonitor:
    ...
    def track_deployment(
        ...
        logger.info(f"Started monitoring deployment {fix_id} for error type {error_type}")
    ...
    def check_deployments(self, health_checker: HealthChecker) -> List[OutcomeReport]:
        ...
        for fix_id, deployment in list(self.active_deployments.items()):
            ...
            if current_time - deployment.last_checked < self.check_interval.total_seconds():
                continue
    ...
    def get_deployment_stats(self) -> Dict[str, Any]:
        ...
        if self.outcome_history:
            successes = sum(1 for r in self.outcome_history 
                          if r.outcome == FixOutcome.SUCCESS)
            stats['success_rate'] = successes / len(self.outcome_history)
            
            total_duration = sum(r.duration_hours for r in self.outcome_history)
            stats['avg_monitoring_duration'] = total_duration / len(self.outcome_history)
    ...
    def _calculate_performance_impact(
        ...
        return {
            ...
            'response_time_impact': health_metrics.get('avg_response_time', 100) - 100  # Baseline 100ms
        }
    ...
    def _generate_outcome_report(
        ...
        recurrence_rate = deployment.error_recurrence_count / max(1, duration_hours / 24)
    ...
    def _extract_lessons(
        ...
        if outcome == FixOutcome.FAILURE:
            lessons.append(f"Fix for {deployment.error_type} failed after {deployment.error_recurrence_count} recurrences")
            if deployment.confidence > 0.8:
                lessons.append("High confidence prediction failed - need to recalibrate")
        
        elif outcome == FixOutcome.REGRESSION:
            lessons.append("Fix caused performance regression")
            worst_metric = max(deployment.performance_metrics.items(), key=lambda x: abs(x[1]))
            ...
            if deployment.fix_type == 'ml_generated':
                lessons.append(f"ML model {deployment.model_version} successful for {deployment.error_type}")
...
class OutcomeTracker:
    ...
    def predict_fix_success(
        ...
        if error_patterns:
            error_success_rate = sum(1 for p in error_patterns if p['success']) / len(error_patterns)
        ...
        for bin_idx, data in confidence_bins.items():
            if data['actual']:
                accuracy[f"{bin_idx:.1f}"] = {
                    ...
                    'calibration_error': abs(np.mean(data['predicted']) - np.mean(data['actual'])),
    ...
    def _identify_regression_indicators(self) -> List[str]:
        ...
        indicators = []
        regression_reports = [r for r in self.deployment_monitor.outcome_history 
    ...
    def _get_deployment_for_report(self, report: OutcomeReport) -> Optional[DeployedFix]:
...
class LearningPipeline:
    ...
    def run_learning_cycle(self, health_checker: HealthChecker) -> Dict[str, Any]:
        ...
        logger.info(f"Completed learning cycle {results['cycle']} with {results['outcomes_processed']} outcomes")
    ...
    def _extract_rules_from_success(self, report: OutcomeReport) -> None:
        ...
        patch_data = type('PatchData', (), {
            'file_path': deployment.affected_files[0] if deployment.affected_files else '',
        ...
        self.rule_extractor.analyze_successful_fix(
            error_data,
            patch_data,
            fix_metadata
        )
    ...
    def _get_deployment_for_outcome(self, report: OutcomeReport) -> Optional[DeployedFix]:
    ...
    def get_learning_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'last_cycle_time': datetime.fromtimestamp(self.last_learning_time).isoformat(),
