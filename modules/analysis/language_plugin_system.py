...
class LanguagePluginRegistry:
    ...
    def register_plugin(self, plugin: LanguagePlugin):
        ...
        if language_id in self.plugins:
            raise LanguagePluginError(
                f"A plugin for language '{language_id}' is already registered"
            )
        ...
        logger.info(
            f"Registered plugin for language: {plugin.get_language_name()} ({language_id})"
        )
    ...
    def register_plugin_class(self, plugin_class: Type[LanguagePlugin]):
        ...
        if not issubclass(plugin_class, LanguagePlugin):
            raise LanguagePluginError(
                f"Class {plugin_class.__name__} is not a subclass of LanguagePlugin"
            )
    ...
    def unregister_plugin(self, language_id: str):
        ...
        if language_id not in self.plugins:
            raise LanguagePluginError(
                f"No plugin registered for language '{language_id}'"
            )
                    ...
                    for name, obj in inspect.getmembers(module):
                        if (
                            ...
                            try:
                                ...
                                logger.warning(
                                    f"Failed to register plugin class {obj.__name__}: {e}"
                                )
                finally:
                    # Clean up sys.path
...
class PythonLanguagePlugin(LanguagePlugin):
    ...
    def __init__(self):
        ...
        from .analyzer import AnalysisStrategy, Analyzer
...
if __name__ == "__main__":
    ...
    for lang_id in languages:
        ...
        logger.info(
            f"Author: {metadata['author'] if 'author' in metadata else metadata.get('plugin_author', 'Unknown')}"
        )
        logger.info(f"Description: {metadata['plugin_description']}")
        logger.info(
            f"Supported frameworks: {', '.join(metadata['supported_frameworks'])}"
        )
        ...
        if lang_id == "python":
            ...
            logger.info(
                f"Generated fix: {fix.get('suggestion') if 'suggestion' in fix else fix.get('patch_type', 'unknown')}"
            )
