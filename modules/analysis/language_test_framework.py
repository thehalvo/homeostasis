        ...
        for source_lang, target_lang in language_pairs:
            ...
            for test_case in source_cases:
                try:
                    ...
                    standard_error = convert_to_standard_format(test_case.error_data, source_lang)
                    ...
                    roundtrip_error = source_adapter.from_standard_format(standard_error)
                    
                    # Check that key information is preserved in roundtrip
                    preserved = True
                    error_message = None
                    
                    if "error_type" in test_case.error_data and "error_type" in roundtrip_error:
                        if test_case.error_data["error_type"] != roundtrip_error["error_type"]:
                            preserved = False
        ...
        for test_case in test_suite.test_cases:
            try:
                ...
                results["pattern_detection_tests"].append({
                    ...
                    "similar_languages": [e.get("language", "unknown") for e in similar_errors]
                })
            except Exception as e:
                results["pattern_detection_tests"].append({
                    "test_case": test_case.name,
            ...
            if test_case.context:  # Only test with cases that have context
                try:
                    ...
                    results["cross_suggestion_tests"].append({
                        ...
                        "suggestion_languages": [s.get("language", "unknown") for s in suggestions]
                    })
                except Exception as e:
                    results["cross_suggestion_tests"].append({
                        "test_case": test_case.name,
...
def discover_test_generators() -> Dict[str, TestCaseGeneratorBase]:
    ...
    for lang_id, plugin in plugins.items():
        ...
        if hasattr(plugin, "get_test_generator") and callable(getattr(plugin, "get_test_generator")):
            try:
...
if __name__ == "__main__":
    ...
    # Example usage
    suite = LanguageTestSuite("Example Test Suite", "An example test suite for demonstration")
    ...
    go_test = LanguageTestCase(
        ...
        context={
            "code_snippet": "result := value.GetData()"
        },
