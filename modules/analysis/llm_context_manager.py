    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'LLMContext':
        ...
        if isinstance(data.get('error_context'), dict):
            ...
            if 'language' in error_context_data:
                error_context_data['language'] = LanguageType(error_context_data['language'])
        ...
        if isinstance(data.get('error_classification'), dict):
            ...
            if 'category' in classification_data:
                classification_data['category'] = ErrorCategory(classification_data['category'])
            if 'severity' in classification_data:
                classification_data['severity'] = ErrorSeverity(classification_data['severity'])
...
class LLMContextManager:
    ...
    def __init__(self, storage_dir: Optional[Path] = None, max_contexts: int = 1000):
        ...
        logger.info(f"Initialized LLM Context Manager with storage at {self.storage_dir}")
    
    def store_error_context(self, 
                          error_context: ErrorContext,
                          additional_analysis: Optional[Dict[str, Any]] = None,
                          project_context: Optional[Dict[str, Any]] = None,
                          monitoring_data: Optional[Dict[str, Any]] = None) -> str:
        """
        ...
        if additional_analysis:
            ...
            llm_context.language_specific_analysis = additional_analysis.get("language_specific")
            llm_context.compiler_diagnostics = additional_analysis.get("compiler_diagnostics")
    ...
    def search_similar_contexts(self, 
                               error_context: ErrorContext,
                               max_results: int = 5) -> List[LLMContext]:
        """
        ...
        for context in self.contexts.values():
            similarity_score = self._calculate_similarity(error_context, context.error_context)
    ...
    def export_contexts_for_training(self, 
                                    output_file: Path,
                                    include_successful_fixes: bool = True) -> int:
        """
        ...
        for context in self.contexts.values():
            ...
            if hasattr(context, 'llm_responses') and context.llm_responses:
                successful_fixes = [
                    response for response in context.llm_responses
                    if response.get("test_result", {}).get("success", False)
                ]
                if successful_fixes:
                    sample["successful_fix"] = successful_fixes[-1]  # Latest successful fix
        ...
        logger.info(f"Exported {len(training_data)} contexts for training to {output_file}")
    ...
    def _enrich_with_historical_data(self, context: LLMContext):
        ...
        similar_contexts = self.search_similar_contexts(context.error_context, max_results=3)
        ...
        for similar_ctx in similar_contexts:
            if hasattr(similar_ctx, 'llm_responses') and similar_ctx.llm_responses:
                successful_fixes = [
                    response for response in similar_ctx.llm_responses
                    if response.get("test_result", {}).get("success", False)
                ]
                if successful_fixes:
    ...
    def _calculate_similarity(self, context1: ErrorContext, context2: ErrorContext) -> float:
        ...
        if context1.error_message and context2.error_message:
            ...
            if words1 and words2:
                ...
                similarity_score += 0.4 * (len(common_words) / len(words1.union(words2)))
...
def store_error_for_llm(context_manager: LLMContextManager,
