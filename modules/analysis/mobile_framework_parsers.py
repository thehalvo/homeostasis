...
class FrameworkDetector:
    ...
    def detect_framework(
        self, error_context: ErrorContext, project_root: Optional[str] = None
    ) -> str:
        """
        ...
        if error_context.file_path:
            ...
            if any(
                ...
                for pattern in ["assets/", "library/", "projectsettings/"]
            ):
    ...
    def _detect_from_project_structure(self, project_root: str) -> str:
        ...
        try:
            ...
            # Flutter detection
            if (project_path / "pubspec.yaml").exists() or (
                ...
                try:
                    ...
                    with open(package_json) as f:
                        ...
                        if "react-native" in deps:
                            return FrameworkType.REACT_NATIVE
                        elif ("@capacitor/core" in deps or
                        ...
                        if any(
                            "cap " in script or "capacitor" in script
                            ...
                            "vue-cli-service" in script for script in scripts.values()
                        ):
                            return FrameworkType.VUE
                            ...
                            return FrameworkType.EMBER
                        elif any("tailwind" in script or
                                 "postcss" in script or
                                 "sass" in script
                                 for script in scripts.values()):
                            return FrameworkType.CSS
                        elif any("webpack" in script or
                                 "rollup" in script or
                                 "vite" in script or
                                 "build" in script
                                 for script in scripts.values()):
            ...
            if (project_path / "next.config.js").exists() or (
                ...
                return FrameworkType.NEXTJS
            elif (project_path / "vue.config.js").exists() or (
                project_path / "src" / "main.js"
            ).exists():
                return FrameworkType.VUE
            elif (project_path / "angular.json").exists() or (
                project_path / "src" / "app"
            ).exists():
                return FrameworkType.ANGULAR
            elif (project_path / "svelte.config.js").exists() or (
                project_path / "src" / "App.svelte"
            ).exists():
                return FrameworkType.SVELTE
            elif (project_path / ".ember-cli").exists() or (
                project_path / "ember-cli-build.js"
            ).exists():
                return FrameworkType.EMBER
            ...
            # Xamarin detection
            xamarin_files = ["*.sln", "*.csproj", "*.vbproj", "*.fsproj"]
...
class UnifiedFrameworkParserFactory:
    ...
    def create_parser(
        self, error_context: ErrorContext, project_root: Optional[str] = None
    ) -> Optional[LanguageSpecificParser]:
        """
        ...
        logger.warning(
            f"No suitable parser found for {error_context.language.value} / {framework}"
        )
        return None

    def analyze_error_with_framework_context(
        self, error_context: ErrorContext, project_root: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            analysis_result["runtime_analysis"] = parser.detect_runtime_issues(
                error_context
            )

            # Add framework-specific analysis if available
            if hasattr(parser, "get_framework_specific_analysis"):
                analysis_result["framework_specific_analysis"] = (
                    parser.get_framework_specific_analysis(error_context)
                )
...
def create_mobile_framework_parser(
    error_context: ErrorContext, project_root: Optional[str] = None
) -> Optional[LanguageSpecificParser]:
    """
...
if __name__ == "__main__":
    ...
    if ADDITIONAL_MOBILE_FRAMEWORKS_AVAILABLE:
        ...
        capacitor_analysis = factory.analyze_error_with_framework_context(
            capacitor_context
        )
        ...
        capacitor_recommendations = factory.get_framework_recommendations(
            capacitor_context
        )
        print(
