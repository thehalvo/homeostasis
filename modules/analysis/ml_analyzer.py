...
class MLAnalyzer:
    ...
    def _initialize_ml_model(self, model_path: Optional[str] = None):
        ...
        if not model_path:
            model_path = os.path.join(
                os.path.dirname(__file__), "models", "error_classifier.pkl"
            )
    ...
    def _rule_analysis(self, error_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "confidence_score" not in result and "confidence" in result:
            result["confidence_score"] = self._convert_rule_confidence(
                result["confidence"]
            )
    ...
    def _ml_analysis(self, error_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "error": str(e), "confidence_score": 0.0}

    def _combine_results(
        self, rule_result: Dict[str, Any], ml_result: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        if (ml_confidence >= self.ml_confidence_threshold and
    ...
    def analyze_error(self, error_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.mode == MLAnalysisMode.FALLBACK:
            ...
            MLAnalysisMode.PARALLEL,
            MLAnalysisMode.PRIMARY,
            MLAnalysisMode.ENSEMBLE,
        ]:
...
class HybridAnalyzer:
    ...
    def _initialize_llm_analyzer(
        ...
        try:
            ...
            self.llm_analyzer = AIAnalyzer(
                model_type=AIModelType.LLM, api_key=api_key, endpoint=endpoint
            )
    ...
    def analyze_error(self, error_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if (max(rule_confidence, ml_confidence) >= 0.7 or
                not self.use_llm or
        ...
        try:
            ...
            if llm_confidence > 0.7 and llm_confidence > max(
                rule_confidence, ml_confidence
            ):
                combined_result["error_type"] = llm_result.get(
                    "root_cause", combined_result.get("error_type", "unknown")
                )
...
def analyze_with_hybrid_system(
    error_data: Dict[str, Any], ml_mode: str = "parallel", use_llm: bool = False
) -> Dict[str, Any]:
    """
...
if __name__ == "__main__":
    ...
    for mode in ["disabled", "fallback", "parallel", "primary"]:
        ...
        print(
            f"Error type: {result.get('error_type', result.get('root_cause', 'Unknown'))}"
        )
        print(f"Confidence: {result.get('confidence_score', 0.0):.2f}")

        if "rule_analysis" in result:
            print(
                f"Rule confidence: {result['rule_analysis'].get('confidence_score', 0.0):.2f}"
            )

        if "ml_analysis" in result:
            print(
                f"ML confidence: {result['ml_analysis'].get('confidence_score', 0.0):.2f}"
            )
    ...
    print(
        f"Error type: {hybrid_result.get('error_type', hybrid_result.get('root_cause', 'Unknown'))}"
    )
