...
class APMConfig:
    ...
    additional_params: Dict[str, Any] = field(default_factory=dict)  # Additional parameters
...
class APMIntegration(ABC):
    ...
    def _initialize_session(self):
        ...
        if self.config.provider == APMProvider.DATADOG:
            self.session.headers.update({
                ...
                "DD-APPLICATION-KEY": self.config.additional_params.get("app_key", "")
            })
    ...
    @abstractmethod
    def get_metrics(self, metric_names: List[str], 
    ...
    @abstractmethod
    def get_traces(self, filter_params: Optional[Dict[str, Any]] = None,
                  limit: int = 100,
                  start_time: Optional[int] = None,
    ...
    @abstractmethod
    def get_errors(self, filter_params: Optional[Dict[str, Any]] = None,
                  limit: int = 100,
                  start_time: Optional[int] = None,
                  end_time: Optional[int] = None) -> Dict[str, Any]:
        """
...
class NewRelicIntegration(APMIntegration):
    ...
    def get_metrics(self, metric_names: List[str], 
        ...
        metrics_clause = ", ".join([f"average({name})" for name in metric_names])
        
        # Build NRQL query
        query = f"SELECT {metrics_clause} FROM Metric"
        
        if self.config.app_id:
            query += f" WHERE appId = {self.config.app_id}"
    ...
    def get_errors(self, filter_params: Optional[Dict[str, Any]] = None,
        ...
        if filter_params:
            ...
            if "error_message" in filter_params:
                conditions.append(f"errorMessage LIKE '%{filter_params['error_message']}%'")
    ...
    def get_traces(self, filter_params: Optional[Dict[str, Any]] = None,
        ...
        if filter_params:
            if "service" in filter_params:
                query["query"]["bool"]["must"].append({
                    "term": {
                        "service.name": filter_params["service"]
                    }
                })
            if "transaction_name" in filter_params:
                query["query"]["bool"]["must"].append({
                    "term": {
                        "transaction.name": filter_params["transaction_name"]
                    }
                })
            if "duration_min" in filter_params:
                query["query"]["bool"]["must"].append({
                    "range": {
                        "transaction.duration.us": {
                            "gte": filter_params["duration_min"] * 1000  # Convert to microseconds
                        }
                    }
                })
        elif self.config.service_name:
        ...
        return self._make_request(
            method="POST",
            endpoint="apm-*-transaction*/_search",
            data=data
        )
    ...
    def get_errors(self, filter_params: Optional[Dict[str, Any]] = None,
        ...
        if filter_params:
            ...
            if "error_type" in filter_params:
                query["query"]["bool"]["must"].append({
                    "term": {
                        "error.type": filter_params["error_type"]
                    }
                })
            if "error_message" in filter_params:
                query["query"]["bool"]["must"].append({
                    "match_phrase": {
                        "error.message": filter_params["error_message"]
                    }
                })
        elif self.config.service_name:
            query["query"]["bool"]["must"].append({
...
class APMDataCollector:
    ...
    def convert_to_standard_format(self, apm_data: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
    ...
    def _convert_datadog_format(self, apm_data: Dict[str, Any], 
    ...
    def _convert_newrelic_format(self, apm_data: Dict[str, Any], 
                                result: List[Dict[str, Any]]) -> None:
        """
    ...
    def _convert_dynatrace_format(self, apm_data: Dict[str, Any], 
                                 result: List[Dict[str, Any]]) -> None:
        """
    ...
    def _convert_elastic_format(self, apm_data: Dict[str, Any], 
                               result: List[Dict[str, Any]]) -> None:
        """
        ...
        if "hits" in apm_data and "hits" in apm_data["hits"]:
            for hit in apm_data["hits"]["hits"]:
                ...
                # Extract stack trace
                stack_trace = []
                if "exception" in error_info and "stacktrace" in error_info["exception"]:
    ...
    def _convert_generic_format(self, apm_data: Dict[str, Any], 
                               result: List[Dict[str, Any]]) -> None:
        """
...
def create_apm_integration(provider_name: str, api_key: str, 
                          api_endpoint: Optional[str] = None,
                          service_name: Optional[str] = None,
                          environment: Optional[str] = None,
                          additional_params: Optional[Dict[str, Any]] = None) -> APMIntegration:
    """
...
def collect_apm_errors(provider_name: str, api_key: str, 
                      api_endpoint: Optional[str] = None,
                      service_name: Optional[str] = None,
                      start_time: Optional[int] = None,
                      end_time: Optional[int] = None,
