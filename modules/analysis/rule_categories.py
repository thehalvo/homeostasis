...
class RuleType(Enum):
    ...
    SECURITY_VULNERABILITY = "security_vulnerability"  # Detects security vulnerabilities
...
class RuleDependency:
    ...
    def _dfs_check_circular(self, rule_id: str, visited: Set[str], 
                           path: List[str], circular_chains: List[List[str]]):
        """
...
def detect_rule_conflicts(rules: List[Union[Rule, EnhancedRule]]) -> List[Dict[str, Any]]:
    """
    ...
    for i, rule1 in enumerate(rules):
        for j, rule2 in enumerate(rules[i+1:], i+1):
            ...
            if pattern1_simplified in pattern2_simplified or pattern2_simplified in pattern1_simplified:
                conflicts.append({
                    "rule1": rule1.id,
                    "rule2": rule2.id,
...
if __name__ == "__main__":
    ...
    for conflict in conflicts:
        print(f"{conflict['rule1']} conflicts with {conflict['rule2']}: {conflict['reason']}")