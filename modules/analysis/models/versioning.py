...
class ModelVersionControl:
    ...
    def commit_model(self, model: Any, model_name: str, model_type: str,
                    training_config: Dict[str, Any], metrics: Dict[str, float],
                    parent_version: Optional[str] = None,
                    tags: Optional[List[str]] = None,
                    artifacts: Optional[Dict[str, str]] = None) -> ModelVersion:
        ...
        for key in all_keys:
            ...
            if val1 != val2:
                diff[key] = {
                    ...
                    "new": val2,
                    "change": val2 - val1,
                    "change_pct": ((val2 - val1) / val1 * 100) if val1 != 0 else float('inf')
                }
...
class ModelEvaluator:
    ...
    def evaluate_model(self, version_id: str, X_test: np.ndarray, y_test: np.ndarray,
                      dataset_name: str = "test", task_type: Optional[str] = None,
                      additional_metrics: Optional[Dict[str, Callable]] = None) -> EvaluationResult:
    ...
    def _evaluate_classifier(self, y_true: np.ndarray, y_pred: np.ndarray,
                           ...
                           additional_metrics: Optional[Dict[str, Callable]]) -> Tuple[Dict, str, float]:
    ...
    def _evaluate_regressor(self, y_true: np.ndarray, y_pred: np.ndarray,
                          additional_metrics: Optional[Dict[str, Callable]]) -> Tuple[Dict, str, float]:
        ...
        metrics = {
            ...
            "mape": np.mean(np.abs((y_true - y_pred) / y_true)) * 100 if not any(y_true == 0) else float('inf'),
    ...
    def _extract_feature_importance(self, model: Any) -> Optional[Dict[str, float]]:
        ...
        if hasattr(model, 'feature_importances_'):
            ...
            if 'model' in model.named_steps and hasattr(model.named_steps['model'], 'feature_importances_'):
    ...
    def _generate_evaluation_plots(self, y_true: np.ndarray, y_pred: np.ndarray,
                                 y_proba: Optional[np.ndarray], version_id: str,
                                 dataset_name: str) -> Dict[str, str]:
        """Generate evaluation plots."""
        plots_dir = self.version_control.evaluations_dir / f"{version_id}_{dataset_name}_plots"
    ...
    def compare_models(self, version_ids: List[str], X_test: np.ndarray, 
                      y_test: np.ndarray, dataset_name: str = "test") -> pd.DataFrame:
        ...
        for version_id in version_ids:
            ...
            # Extract key metrics
            metrics = eval_result.classification_metrics or eval_result.regression_metrics or {}
    ...
    def perform_ab_test(self, version_a: str, version_b: str,
        ...
        result = {
            ...
            "improvement_pct": float((eval_b.primary_score - eval_a.primary_score) / eval_a.primary_score * 100)
        }
...
class AutoModelSelector:
    ...
    def select_best_model(self, model_name: str, X_val: np.ndarray, y_val: np.ndarray,
                         selection_criteria: Optional[Dict[str, Any]] = None) -> str:
        ...
        candidates = self.version_control.list_versions(
            model_name=model_name,
            status="staging"  # Only consider staging models
        )
    ...
    def recommend_deployment(self, model_name: str, X_val: np.ndarray, y_val: np.ndarray,
                           current_production: Optional[str] = None) -> Dict[str, Any]:
        ...
        if current_production:
            ...
            if ab_result["winner"] == current_production:
                ...
                recommendation["confidence"] = "high" if ab_result["significant"] else "medium"
...
if __name__ == "__main__":
    ...
    comparison = evaluator.compare_models(
        [version.version_id, version2.version_id],
        X_test,
        y_test
    )
