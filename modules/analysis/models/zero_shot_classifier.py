...
class ZeroShotErrorClassifier:
    ...
    def classify_zero_shot(self, error_data: Dict[str, Any], 
    ...
    def _classify_by_similarity(self, error_data: Dict[str, Any], 
                               k: int) -> Dict[str, Any]:
    ...
    def _combine_results(self, semantic: Dict[str, Any], 
                        nli: Dict[str, Any], 
        ...
        # Collect scores
        for method_result, method_name in [(semantic, 'semantic_similarity'),
        ...
        return {
            ...
            'methods_used': {
                'semantic': semantic['predictions'][:1] if semantic['predictions'] else [],
    ...
    def _generate_explanation(self, error_data: Dict[str, Any], 
    ...
    def learn_from_feedback(self, error_data: Dict[str, Any], 
                          correct_class: str, 
                          predicted_class: str):
        """
        ...
        logger.info(f"Learning from feedback: predicted {predicted_class}, "
...
def demonstrate_zero_shot_classification():
    ...
    test_errors = [
        {
            ...
            'error_details': {
                'detailed_frames': [{
                    'code': 'db = Database(config["database_url"])'
                }]
            }
        },
        {
        ...
        },
        {
            ...
            'error_details': {
                'detailed_frames': [{
                    'code': 'validate_email(user_input["email"])'
                }]
            }
        },
        {
    ...
    for i, error in enumerate(test_errors):
        ...
        for j, pred in enumerate(results['predictions']):
            logger.info(f"{j+1}. {pred['class']} (score: {pred['score']:.3f}, "
        ...
        for method, preds in results['methods_used'].items():
            if preds:
                logger.info(f"  {method}: {preds[0]['class']} ({preds[0]['score']:.3f})")
