...
class ErrorTaxonomy:
    ...
    fine_patterns = {
        ...
        "encoding_error": {
            "parent": "value_errors",
            "patterns": [r"UnicodeDecodeError", r"codec can't decode", r"invalid.*encoding"],
...
class HierarchicalAttention(nn.Module):
    ...
    def forward(self, features: torch.Tensor, level_mask: Optional[torch.Tensor] = None):
        """
...
class HierarchicalErrorClassifier(nn.Module):
    ...
    def __init__(self, taxonomy: ErrorTaxonomy, hidden_dim: int = 768, 
...
class HierarchicalClassificationPipeline:
    ...
    def _process_outputs(self, outputs: Dict[str, torch.Tensor], 
        ...
        if level1_label in outputs['level2']:
            ...
            level2_label = self.level2_encoders[level1_label].inverse_transform([level2_idx])[0]
            ...
            results['level2'] = {
                ...
                'all_probs': {
                    ...
                    for label, prob in zip(
                        self.level2_encoders[level1_label].classes_, 
                        level2_probs
                    )
                }
            }
    ...
    def visualize_classification(self, results: Dict[str, Any]) -> str:
        ...
        # Level details
        if 'level1' in results:
            lines.append(f"Level 1 (Coarse): {results['level1']['prediction']} "
                        f"(confidence: {results['level1']['confidence']:.2f})")
        
        if 'level2' in results:
            lines.append(f"Level 2 (Medium): {results['level2']['prediction']} "
    ...
    def train(self, training_data: List[Tuple[Dict[str, Any], Dict[str, str]]],
              epochs: int = 10, batch_size: int = 16) -> Dict[str, Any]:
        """
        ...
        logger.info(f"Training hierarchical classifier with {len(training_data)} samples...")
