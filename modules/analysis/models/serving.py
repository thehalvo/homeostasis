        ...
        if self.backend == "memory":
            ...
            try:
                self.redis_client.setex(
                    key, self.config.cache_ttl_seconds, json.dumps(prediction)
                )
            except Exception as e:
    ...
    def _batch_processing_loop(self):
        ...
        while self.processing:
            ...
            while len(batch) < self.config.batch_size:
                ...
                ) * 1000 >= self.config.max_batch_wait_ms:
                    break
...
class ModelServer:
        ...
        if self.config.enable_ab_testing and self.config.ab_test_versions:
            ...
            if self.config.model_version:
                ...
                versions = self.version_control.list_versions(
                    model_name=self.config.model_name, status="production"
                )
                if not versions:
                    versions = self.version_control.list_versions(
    ...
    def _init_metrics(self):
        ...
        self.active_requests_gauge = Gauge(
            "model_active_requests", "Number of active requests", ["model_name"]
        )
    ...
    def _select_model_version(self, request: InferenceRequest) -> str:
        ...
        if self.config.enable_ab_testing and len(self.config.ab_test_versions) > 1:
            ...
            if self.config.ab_test_weights:
                ...
                # Equal weights
                weights = [1.0 / len(self.config.ab_test_versions)] * len(
                    self.config.ab_test_versions
                )
    ...
    async def predict(self, request: InferenceRequest) -> InferenceResponse:
        ...
        if self.config.enable_metrics:
            self.active_requests_gauge.labels(model_name=self.config.model_name).set(
        ...
        try:
            ...
            if cached_result is not None:
                if self.config.enable_metrics:
                    self.cache_hits.labels(
                        model_name=self.config.model_name, model_version=model_version
                    ).inc()
            ...
            if self.config.enable_metrics:
                self.model_errors.labels(
                    model_name=self.config.model_name,
                    model_version=(
                        model_version if "model_version" in locals() else "unknown"
                    ),
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        if self.request_history:
            recent_requests = [
                r for r in self.request_history if time.time() - r["timestamp"] < 60
            ]
...
class ModelServingOrchestrator:
    ...
    def run(self, host: str = "127.0.0.1", port: int = 8080):
        """Run the unified API server with secure defaults."""
        if not self.app:
...
class AutoScaler:
    ...
    def _make_scaling_decision(self, model_name: str, policy: Dict[str, Any]):
        ...
        recent_metrics = [
            m for m in policy["metrics_history"] if time.time() - m["timestamp"] < 30
        ]
        ...
        if recent_metrics:
            ...
            desired_replicas = max(
                config.min_replicas, min(config.max_replicas, desired_replicas)
            )

            # Scale if needed
            if desired_replicas != current_replicas:
                logger.info(
                    f"Scaling {model_name} from {current_replicas} to {desired_replicas} replicas"
                )
...
if __name__ == "__main__":
    ...
    config2 = ServingConfig(
        model_name="complexity_analyzer", enable_cache=True, enable_metrics=True
    )
