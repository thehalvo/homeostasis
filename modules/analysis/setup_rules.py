...
def verify_circular_dependencies() -> List[List[str]]:
    ...
    if circular_deps:
        ...
        if len(circular_deps) > 5:
            print(
                f"  ... and {len(circular_deps) - 5} more circular dependency chains."
            )
    else:
                            ...
                            json.dump(
                                {
                                    ...
                                    "rules": [
                                        rule.to_dict() for rule in enhanced_rules
                                    ],
                                },
                                f,
                                indent=2,
...
def verify_analyzer() -> bool:
    ...
    try:
        ...
        if result.get("matched", False):
            ...
            print(
                f"  Confidence: {result.get('confidence')} ({result.get('confidence_score', 0.0):.2f})"
            )
            return True
        else:
...
def main() -> None:
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Setup and verify the rule system")
    parser.add_argument(
        "--setup-dirs", action="store_true", help="Set up rule directories"
    )
    parser.add_argument("--verify", action="store_true", help="Verify rule integrity")
    parser.add_argument(
        "--upgrade", action="store_true", help="Upgrade rules to enhanced format"
    )
    parser.add_argument(
        "--report", action="store_true", help="Generate a status report"
    )
    parser.add_argument(
        "--output", "-o", type=str, help="Output file for the status report"
    )
    parser.add_argument("--all", "-a", action="store_true", help="Perform all actions")
