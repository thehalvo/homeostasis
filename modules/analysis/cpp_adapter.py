...
class CPPErrorAdapter(LanguageAdapter):
    ...
    def _extract_stack_trace(self, cpp_error: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        if "Segmentation fault" in message or "core dumped" in message:
            ...
            return [
                {"frame": 0, "signal": "SIGSEGV", "description": "Segmentation fault"}
            ]

        return []

    def _parse_gdb_stack_trace(
        self, stack_trace: Union[str, List[str]]
    ) -> List[Dict[str, Any]]:
        """
        ...
        for line in lines:
            ...
            if match:
                ...
                frame = {"frame": frame_num, "function": function}
    ...
    def _categorize_error(self, error_type: str, message: str) -> str:
        ...
        # Linker errors
        if any(
            x in message.lower() for x in ["undefined reference", "linker error", "ld:"]
        ):
            return "linker"

        # Memory errors
        if any(
            x in error_type.lower() + message.lower()
            for x in ["segmentation", "sigsegv", "memory", "heap", "stack overflow"]
        ):
            return "memory"

        # Runtime errors
        if any(
            x in error_type.lower() for x in ["runtime", "sigabrt", "sigfpe", "sigill"]
        ):
            return "runtime"
        ...
        if any(
            x in message
            for x in ["std::", "boost::", "<algorithm>", "<vector>", "<map>"]
        ):
            return "stl"
