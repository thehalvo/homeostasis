...
class SimilarityModel(AIModel):
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            print(
                f"Would load similarity model and error corpus from: {self.config.model_path}"
            )
            self.is_ready = True
            return True
        except Exception as e:
...
class LLMModel(AIModel):
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            if not api_key:
                print(
                    "No API key provided for LLM. Set config.api_key or LLM_API_KEY environment variable."
                )
                return False
...
class EnsembleModel(AIModel):
    ...
    def analyze(self, error_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        selected_result["ensemble_models"] = [
            model.__class__.__name__ for model in self.models
        ]
...
class AIModelFactory:
    ...
    @staticmethod
    def create_ensemble(
        config: AIModelConfig, model_configs: List[AIModelConfig]
    ) -> EnsembleModel:
        """
...
class AIAnalyzer:
    ...
    def __init__(
        self,
        model_type: Union[str, AIModelType] = "stub",
        model_path: Optional[str] = None,
        api_key: Optional[str] = None,
        endpoint: Optional[str] = None,
        parameters: Optional[Dict[str, Any]] = None,
...
AVAILABLE_MODELS = {
    ...
    "classifier": AIModelConfig(
        model_type=AIModelType.CLASSIFIER, model_path="models/error_classifier.pkl"
    ),
    "similarity": AIModelConfig(
        model_type=AIModelType.SIMILARITY, model_path="models/error_embeddings.pkl"
    ),
    "llm": AIModelConfig(
