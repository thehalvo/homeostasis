...
class Rule:
    ...
    def __init__(self, 
        ...
        # Other fields
        self.id = id or f"{self.category.value}_{self.type}_{hash(pattern) & 0xFFFFFFFF}"
        self.tags = tags or []
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'Rule':
        ...
        if missing_fields:
            raise ValueError(f"Missing required fields for rule: {', '.join(missing_fields)}")
...
class RuleLoader:
    ...
    def load_from_file(file_path: Union[str, Path], 
                       format: Optional[Union[str, RuleFormat]] = None) -> RuleSet:
        """
        ...
        if format == RuleFormat.JSON:
            ...
            try:
                ...
                raise ImportError("PyYAML is required to load YAML rule files. Install it with pip install pyyaml.")
    ...
    @staticmethod
    def load_from_directory(directory_path: Union[str, Path], 
                           recursive: bool = False) -> List[RuleSet]:
        """
...
if __name__ == "__main__":
    ...
    fastapi_rules = convert_legacy_patterns(FASTAPI_ERROR_PATTERNS, RuleCategory.FASTAPI)
    ...
    # Export as JSON
    python_rules_file = DEFAULT_RULES_DIR / RuleCategory.PYTHON.value / "common_errors.json"
    with open(python_rules_file, "w") as f:
        json.dump(python_rule_set.to_dict(), f, indent=2)
    
    fastapi_rules_file = DEFAULT_RULES_DIR / RuleCategory.FASTAPI.value / "common_errors.json"
