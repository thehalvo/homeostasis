    ...
    def print_summary(self) -> None:
        ...
        if self.stats['circular_dependencies']:
            ...
            if len(self.stats['circular_dependencies']) > 3:
                print(f"  ... and {len(self.stats['circular_dependencies']) - 3} more circular dependency chains.")
...
class RuleTestResult:
    ...
    def __init__(self, rule: Union[Rule, EnhancedRule], test_string: str, expected_match: bool):
        """
...
class RuleTester:
    ...
    def load_rules(self, rule_ids: List[str] = None, categories: List[str] = None) -> None:
        """
    ...
    def test_rule(self, rule: Union[Rule, EnhancedRule], test_string: str, 
                 expected_match: bool = True) -> RuleTestResult:
        """
    ...
    def test_with_examples(self, rules: List[Union[Rule, EnhancedRule]] = None) -> List[RuleTestResult]:
        """
    ...
    def export_results(self, results: List[RuleTestResult], output_path: Path) -> None:
        ...
        for result in results:
            ...
            if result.match:
                serialized["match_groups"] = [g for g in result.match.groups()] if result.match.groups() else []
...
class RuleManager:
    ...
    def create_rule(self, rule_data: Dict[str, Any], rule_set_name: str, 
                   category: str) -> Union[Rule, EnhancedRule]:
        """
    ...
    def update_rule(self, rule_id: str, rule_data: Dict[str, Any]) -> Union[Rule, EnhancedRule, None]:
        """
        ...
        for rule_set in all_rule_sets:
            for i, rule in enumerate(rule_set.rules):
                if rule.id == rule_id:
                    # Update rule data
                    if isinstance(rule, EnhancedRule):
                        updated_rule = EnhancedRule.from_dict({**rule.to_dict(), **rule_data})
                    else:
                    ...
                    if rule_set_file:
                        ...
                        raise ValueError(f"Could not find rule set file for rule ID '{rule_id}'")
...
class RuleAnalyzer:
    ...
    def print_analysis(self, analysis: Dict[str, Any]) -> None:
        ...
        print(f"Confidence: {analysis.get('confidence')} ({analysis.get('confidence_score', 0.0):.2f})")
        ...
        if 'alternative_matches' in analysis and analysis['alternative_matches']:
            ...
            for i, alt in enumerate(analysis['alternative_matches'], 1):
                print(f"  {i}. {alt.get('rule_id')} - {alt.get('description')} (Confidence: {alt.get('confidence')})")
...
def create_parser() -> argparse.ArgumentParser:
    ...
    create_parser.add_argument("--suggestion", "-s", help="Suggestion for fixing the error")
    create_parser.add_argument("--category", "-c", default="custom", 
                             help="Category for the rule")
    ...
    create_parser.add_argument("--examples", help="Comma-separated list of example errors")
    create_parser.add_argument("--rule-set", default="custom_rules",
    ...
    update_parser.add_argument("--suggestion", "-s", help="Suggestion for fixing the error")
    ...
    update_parser.add_argument("--examples", help="Comma-separated list of example errors")
    ...
    test_parser.add_argument("--error", "-e", help="Error string to test against all rules")
    test_parser.add_argument("--use-examples", "-x", action="store_true", 
    ...
    try:
        if args.command == "list":
            ...
            if args.input:
                ...
                if not args.pattern or not args.type or not args.description or not args.root_cause or not args.suggestion:
                    print("Error: Missing required fields. Use --input or provide --pattern, --type, --description, --root-cause, and --suggestion.")
                    return
                ...
                if args.examples:
                    rule_data["examples"] = [example.strip() for example in args.examples.split(",")]
            ...
            if args.test_file:
                ...
                print("Error: No test method specified. Use --test-file, --error, or --use-examples.")
                return
