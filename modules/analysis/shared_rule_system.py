...
class SharedRule:
    ...
    def matches(
        ...
        if isinstance(pattern, list):
            compiled_patterns = [
                self._get_compiled_pattern(p, language) for p in pattern
            ]
        else:
    ...
    def _compile_patterns(self):
        ...
        if isinstance(self.pattern, list):
            ...
            try:
                self.compiled_patterns["generic"] = re.compile(
                    self.pattern, re.IGNORECASE | re.DOTALL
                )
            except Exception as e:
                logger.warning(f"Invalid pattern in rule {self.rule_id}: {e}")
            ...
            if isinstance(pattern, list):
                for i, p in enumerate(pattern):
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Invalid pattern for {lang} in rule {self.rule_id}: {e}"
                        )
            else:
                try:
...
class SharedRuleRegistry:
    ...
    def get_rules_for_language(self, language: str) -> List[SharedRule]:
        ...
        """
        return [
            rule for rule in self.rules.values() if rule.applies_to_language(language)
        ]
    ...
    def match_rules(
        ...
        for rule in applicable_rules:
            ...
            if is_match:
                matches.append({"rule": rule, "match_groups": match_groups})
                    ...
                    if isinstance(data, dict) and "rules" in data:
                        ...
                        for rule_data in data["rules"]:
                            try:
                                ...
                                logger.warning(
                                    f"Error loading rule from {file_path}: {e}"
                                )
                    else:
        ...
        for category, rules in rules_by_category.items():
            ...
            try:
                with open(file_path, "w") as f:
                    json.dump({"category": category, "rules": rules}, f, indent=2)
...
def create_default_shared_rules() -> List[SharedRule]:
    ...
    rules.append(
        SharedRule(
            ...
            pattern=["(?:key not found|KeyError|no such element|key doesn't exist)"],
    ...
    rules.append(
        SharedRule(
            ...
            pattern=["(?:TypeError|ClassCastException|type mismatch|type error)"],
...
def match_shared_rules(
    error_data: Dict[str, Any], language: str
) -> List[Dict[str, Any]]:
    """
