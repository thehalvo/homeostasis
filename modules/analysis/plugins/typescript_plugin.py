...
class TypeScriptExceptionHandler:
        ...
        try:
            ...
            if type_rules_path.exists():
                with open(type_rules_path, 'r') as f:
                    ...
                    logger.info(f"Loaded {len(rules['type_errors'])} TypeScript type error rules")
            ...
            if comp_rules_path.exists():
                with open(comp_rules_path, 'r') as f:
                    ...
                    logger.info(f"Loaded {len(rules['compilation'])} TypeScript compilation rules")
            ...
            if advanced_rules_path.exists():
                with open(advanced_rules_path, 'r') as f:
                    ...
                    logger.info(f"Loaded {len(rules['advanced_features'])} TypeScript advanced features rules")
        ...
        for category, rule_list in self.rules.items():
            ...
            for rule in rule_list:
                try:
                    ...
                    logger.warning(f"Invalid regex pattern in TypeScript rule {rule.get('id', 'unknown')}: {e}")
    ...
    def _calculate_confidence(self, match: re.Match, rule: Dict[str, Any], 
        ...
        if "jsx" in error_data.get("message", "").lower() or "tsx" in str(error_data.get("stack_trace", "")):
    ...
    def analyze_type_error(self, error_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "root_cause": f"typescript_type_error_{error_code.lower()}" if error_code else "typescript_type_error",
            "severity": "error",
            "error_code": error_code,
...
class TypeScriptPatchGenerator:
    ...
    def _load_templates(self) -> Dict[str, str]:
        ...
        if not self.ts_template_dir.exists():
            logger.warning(f"TypeScript templates directory not found: {self.ts_template_dir}")
            return templates
    ...
    def _fix_cannot_find_name(self, error_data: Dict[str, Any], analysis: Dict[str, Any], 
                            source_code: str) -> Optional[Dict[str, Any]]:
    ...
    def _fix_type_assignment(self, error_data: Dict[str, Any], analysis: Dict[str, Any], 
                           source_code: str) -> Optional[Dict[str, Any]]:
    ...
    def _fix_property_not_exist(self, error_data: Dict[str, Any], analysis: Dict[str, Any], 
                              source_code: str) -> Optional[Dict[str, Any]]:
    ...
    def _fix_config_error(self, error_data: Dict[str, Any], analysis: Dict[str, Any], 
                        source_code: str) -> Optional[Dict[str, Any]]:
    ...
    def _fix_unused_variable(self, error_data: Dict[str, Any], analysis: Dict[str, Any], 
                           source_code: str) -> Optional[Dict[str, Any]]:
        """Fix unused variable warnings."""
        message = error_data.get("message", "")
        
        # Extract variable name
        var_match = re.search(r"'([^']+)' is declared but its value is never read", message)
        if var_match:
...
class TypeScriptLanguagePlugin(LanguagePlugin):
    ...
    def __init__(self):
        ...
        self.supported_frameworks = [
            ...
            "express", "nestjs", "fastify", "koa", "node", "deno"
        ]
    ...
    def _is_type_error(self, error_data: Dict[str, Any]) -> bool:
        ...
        type_error_codes = ["TS2304", "TS2322", "TS2339", "TS2307", "TS2345", "TS2540", "TS2571", "TS2564"]
