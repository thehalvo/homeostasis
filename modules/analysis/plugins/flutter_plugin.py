...
class DartErrorAdapter:
    ...
    def to_standard_format(self, dart_error: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "widget_properties": (
                widget_info.get("properties", {}) if widget_info else {}
            ),
            "context": {
...
class FlutterExceptionHandler:
        ...
        try:
                ...
                if file_path.exists():
                    try:
                        with open(file_path, "r") as f:
                            ...
                            logger.info(
                                f"Loaded {len(rules[category])} Flutter {category} rules"
                            )
                    except Exception as e:
                        logger.error(
                            f"Error loading Flutter {category} rules from {filename}: {e}"
                        )
                        rules[category] = []
                else:
                    rules[category] = []
                    logger.warning(
                        f"Flutter {category} rules file not found: {filename}"
                    )
            ...
            if not rules.get("common"):
                ...
                self._save_default_rules(
                    rules_dir / rule_files["common"], rules["common"]
                )
        ...
        for category, rule_list in self.rules.items():
            ...
            for rule in rule_list:
                try:
                    ...
                    logger.warning(
                        f"Invalid regex pattern in Flutter rule {rule.get('id', 'unknown')}: {e}"
                    )
    ...
    def analyze_dart_error(self, error_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Async/Future errors
        if "future" in message and (
            "completed with an error" in message or "unhandled" in message
        ):
            return {
                "category": "dart",
                "subcategory": "async",
...
class FlutterPatchGenerator:
    ...
    def _load_templates(self) -> Dict[str, str]:
        ...
        if not self.flutter_template_dir.exists():
            logger.warning(
                f"Flutter templates directory not found: {self.flutter_template_dir}"
            )
            return templates
        ...
        for template_name, template_content in default_templates.items():
            ...
            if not template_path.exists():
                try:
                    ...
                    logger.error(
                        f"Error creating default Flutter template {template_name}: {e}"
                    )
...
class FlutterLanguagePlugin(LanguagePlugin):
    ...
    def denormalize_error(self, standard_error: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "widget": (
                ...
                if standard_error.get("widget_type")
                else None
            ),
    ...
    def can_handle(self, error_data: Dict[str, Any]) -> bool:
        ...
        if any(
            indicator in project_files_str for indicator in flutter_project_indicators
        ):
            # Check dependencies for Flutter
            dependencies = context.get("dependencies", [])
            if any(
                "flutter" in dep.lower() or "dart" in dep.lower()
    ...
    def _is_dart_error(self, error_data: Dict[str, Any]) -> bool:
        ...
        return any(
            pattern in message or pattern in error_type for pattern in dart_patterns
        )
