...
class BashExceptionHandler:
            ...
            for shell_type in ["bash", "zsh", "fish", "sh"]:
                ...
                if shell_rules_path.exists():
                    with open(shell_rules_path, "r") as f:
                        ...
                        logger.info(
                            f"Loaded {len(rules[shell_type])} {shell_type} rules"
                        )
    ...
    def _analyze_by_patterns(
        self, message: str, exit_code: int, command: str
    ) -> Dict[str, Any]:
...
class BashPatchGenerator:
    ...
    def _load_templates(self) -> Dict[str, str]:
        ...
        if not self.bash_template_dir.exists():
            logger.warning(
                f"Bash templates directory not found: {self.bash_template_dir}"
            )
            return templates
    ...
    def _fix_syntax_error(
        self, error_data: Dict[str, Any], analysis: Dict[str, Any], script_content: str
    ) -> Optional[Dict[str, Any]]:
    ...
    def _fix_command_not_found(
        ...
        if command:
            ...
            if command in common_packages:
                ...
                fixes.insert(
                    1,
                    f"Install {command}: sudo apt-get install {package} (Ubuntu/Debian) or brew install {package} (macOS)",
                )
