...
class IoTPlugin(LanguagePlugin):
    ...
    def _load_rules(self):
        ...
        rules_path = os.path.join(
            os.path.dirname(__file__), "../rules/iot/iot_errors.json"
        )
    ...
    def _detect_generic_iot_issues(self, code: str) -> List[Dict[str, Any]]:
        ...
        if re.search(
            r'(password|passwd|pwd|key|token)\s*=\s*["\'][^"\']+["\']',
            code,
            re.IGNORECASE,
        ):
            issues.append(
    ...
    def analyze_error_detailed(
        ...
        result = {
            ...
            "resource_usage": (
                iot_error.resource_usage
                if iot_error.resource_usage
                else {"cpu": 0, "memory": 0}
            ),
            "severity": iot_error.severity,
        }
    ...
    def analyze_device_health(
        ...
        if historical_data:
            historical_metrics = [DeviceMetrics(**data) for data in historical_data]
    ...
    def _get_platform_constraints(self, platform: IoTPlatform) -> Dict[str, Any]:
        ...
        if platform == IoTPlatform.ARDUINO and "memory_limits" not in constraints:
            constraints["memory_limits"] = {"default": {"ram": 2048, "flash": 32768}}

        return constraints

    def generate_device_config(
        self, platform: str, requirements: Dict[str, Any]
    ) -> Dict[str, Any]:
