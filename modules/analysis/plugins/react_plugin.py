...
class ReactExceptionHandler:
        ...
        try:
            ...
            if state_rules_path.exists():
                with open(state_rules_path, "r") as f:
                    ...
                    logger.info(
                        f"Loaded {len(rules['state'])} React state management rules"
                    )
        ...
        for category, rule_list in self.rules.items():
            ...
            for rule in rule_list:
                try:
                    ...
                    logger.warning(
                        f"Invalid regex pattern in React rule {rule.get('id', 'unknown')}: {e}"
                    )
        ...
        if matches:
            ...
            return {
                ...
                "subcategory": best_match.get(
                    "category", best_match.get("subcategory", "unknown")
                ),
    ...
    def analyze_state_management_error(
    ...
    def _analyze_redux_error(
        self, message: str, error_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Analyze Redux-specific errors."""
        if ("store" in message or "getstate" in message) and (
            "undefined" in message or "not found" in message
        ):
            return {
                "category": "react",
                "subcategory": "redux",
    ...
    def _analyze_context_error(
        self, message: str, error_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Analyze React Context-specific errors."""
        if "provider" in message and (
            "outside" in message or "must be used within" in message
        ):
            return {
                "category": "react",
                "subcategory": "context",
...
class ReactPatchGenerator:
    ...
    def _load_templates(self) -> Dict[str, str]:
        ...
        if not self.react_template_dir.exists():
            logger.warning(
                f"React templates directory not found: {self.react_template_dir}"
            )
            return templates
    ...
    def _fix_missing_dependency(
        ...
        hook_match = re.search(
            r"React Hook (\w+) has a missing dependency.*'([^']+)'", message
        )
        if hook_match:
    ...
    def _fix_state_update_unmounted(
        self, error_data: Dict[str, Any], analysis: Dict[str, Any], source_code: str
    ) -> Optional[Dict[str, Any]]:
    ...
    def _fix_conditional_hook_call(
        self, error_data: Dict[str, Any], analysis: Dict[str, Any], source_code: str
    ) -> Optional[Dict[str, Any]]:
    ...
    def _fix_redux_store_connection(
        self, error_data: Dict[str, Any], analysis: Dict[str, Any], source_code: str
    ) -> Optional[Dict[str, Any]]:
...
""",
    ...
    def _fix_context_provider_missing(
        self, error_data: Dict[str, Any], analysis: Dict[str, Any], source_code: str
    ) -> Optional[Dict[str, Any]]:
        ...
        context_match = re.search(
            r"useContext.*must be used within.*(\w+)Provider", message
        )
...
class ReactLanguagePlugin(LanguagePlugin):
    ...
    def analyze_error(self, error_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Check if it's a state management error
                ...
                analysis = self.exception_handler.analyze_state_management_error(
                    standard_error
                )
    ...
    def _is_state_management_error(self, error_data: Dict[str, Any]) -> bool:
        ...
        return any(
            pattern in message or pattern in stack_trace for pattern in state_patterns
        )
