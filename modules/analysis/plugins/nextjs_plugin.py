        ...
        try:
            ...
            if data_fetching_rules_path.exists():
                with open(data_fetching_rules_path, "r") as f:
                    ...
                    logger.info(
                        f"Loaded {len(rules['data_fetching'])} Next.js data fetching rules"
                    )
            ...
            if api_routes_rules_path.exists():
                with open(api_routes_rules_path, "r") as f:
                    ...
                    logger.info(
                        f"Loaded {len(rules['api_routes'])} Next.js API routes rules"
                    )
            ...
            if app_router_rules_path.exists():
                with open(app_router_rules_path, "r") as f:
                    ...
                    logger.info(
                        f"Loaded {len(rules['app_router'])} Next.js App Router rules"
                    )
        ...
        for category, rule_list in self.rules.items():
            ...
            for rule in rule_list:
                try:
                    ...
                    logger.warning(
                        f"Invalid regex pattern in Next.js rule {rule.get('id', 'unknown')}: {e}"
                    )
...
class NextjsPatchGenerator:
    ...
    def _load_templates(self) -> Dict[str, str]:
        ...
        if not self.nextjs_template_dir.exists():
            logger.warning(
                f"Next.js templates directory not found: {self.nextjs_template_dir}"
            )
            return templates
    ...
    def _fix_getserversideprops_return(
        self, error_data: Dict[str, Any], analysis: Dict[str, Any], source_code: str
    ) -> Optional[Dict[str, Any]]:
  ...
  try {
    ...
    def _fix_getstaticprops_return(
        self, error_data: Dict[str, Any], analysis: Dict[str, Any], source_code: str
    ) -> Optional[Dict[str, Any]]:
  ...
  try {
    ...
    def _fix_getstaticpaths_return(
        self, error_data: Dict[str, Any], analysis: Dict[str, Any], source_code: str
    ) -> Optional[Dict[str, Any]]:
...
export default async function handler(req, res) {
  ...
  return res.status(500).json({ error: 'Unknown error occurred' });
    ...
    def _fix_api_method_not_allowed(
        self, error_data: Dict[str, Any], analysis: Dict[str, Any], source_code: str
    ) -> Optional[Dict[str, Any]]:
...
class NextjsLanguagePlugin(LanguagePlugin):
    ...
    def analyze_error(self, error_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Check if it's an API route error
                ...
                analysis = self.exception_handler.analyze_api_route_error(
                    standard_error
                )

            # Check if it's an App Router error
            elif self._is_app_router_error(standard_error):
                analysis = self.exception_handler.analyze_app_router_error(
                    standard_error
                )
    ...
    def _is_api_route_error(self, error_data: Dict[str, Any]) -> bool:
        ...
        return any(
            pattern in message or pattern in stack_trace for pattern in api_patterns
        )
    ...
    def _is_app_router_error(self, error_data: Dict[str, Any]) -> bool:
        ...
        return any(
            ...
            for pattern in app_router_patterns
        )
