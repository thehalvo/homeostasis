...
class ReactNativeExceptionHandler:
        ...
        try:
            ...
            if common_rules_path.exists():
                with open(common_rules_path, 'r') as f:
                    ...
                    logger.info(f"Loaded {len(rules['common'])} common React Native rules")
            else:
            ...
            rules = {"common": self._create_default_rules(), "native": [], "navigation": []}
        ...
        for category, rule_list in self.rules.items():
            ...
            for rule in rule_list:
                try:
                    ...
                    logger.warning(f"Invalid regex pattern in React Native rule {rule.get('id', 'unknown')}: {e}")
    ...
    def analyze_native_module_error(self, error_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "native module" in message and ("cannot be null" in message or "not found" in message):
            return {
                "category": "react_native",
                "subcategory": "native_modules",
                "confidence": "high",
...
class ReactNativePatchGenerator:
    ...
    def _load_templates(self) -> Dict[str, str]:
        ...
        if not self.rn_template_dir.exists():
            logger.warning(f"React Native templates directory not found: {self.rn_template_dir}")
            return templates
        ...
        for template_file in self.rn_template_dir.glob("*.template"):
            try:
                ...
                logger.error(f"Error loading React Native template {template_file}: {e}")
...
"""
    ...
    def generate_patch(self, error_data: Dict[str, Any], analysis: Dict[str, Any], 
        ...
        if strategy:
            try:
                ...
                logger.error(f"Error generating React Native patch for {root_cause}: {e}")
        
        # Try to use templates if no specific strategy matches
        return self._template_based_patch(error_data, analysis, source_code)
    
    def _fix_native_module_missing(self, error_data: Dict[str, Any], analysis: Dict[str, Any], 
                                  source_code: str) -> Optional[Dict[str, Any]]:
        """Fix missing native module errors."""
        message = error_data.get("message", "")
        
        # Extract module name if possible
        module_match = re.search(r"'([^']+)'|\"([^\"]+)\"|(\w+)\s*native module", message, re.IGNORECASE)
        module_name = None
        if module_match:
    ...
    def _fix_metro_bundler_error(self, error_data: Dict[str, Any], analysis: Dict[str, Any], 
                                source_code: str) -> Optional[Dict[str, Any]]:
    ...
    def _fix_module_not_found(self, error_data: Dict[str, Any], analysis: Dict[str, Any], 
                             source_code: str) -> Optional[Dict[str, Any]]:
        ...
        module_name = module_match.group(1) or module_match.group(2) if module_match else "unknown"
    ...
    def _fix_bridge_error(self, error_data: Dict[str, Any], analysis: Dict[str, Any], 
                         source_code: str) -> Optional[Dict[str, Any]]:
    ...
    def _fix_ios_pods_error(self, error_data: Dict[str, Any], analysis: Dict[str, Any], 
                           source_code: str) -> Optional[Dict[str, Any]]:
    ...
    def _fix_android_gradle_error(self, error_data: Dict[str, Any], analysis: Dict[str, Any], 
                                 source_code: str) -> Optional[Dict[str, Any]]:
    ...
    def _fix_styling_error(self, error_data: Dict[str, Any], analysis: Dict[str, Any], 
                          source_code: str) -> Optional[Dict[str, Any]]:
...
class ReactNativeLanguagePlugin(LanguagePlugin):
    ...
    def __init__(self):
        ...
        self.supported_frameworks = [
            "react-native", "expo", "@react-native-community",
    ...
    def can_handle(self, error_data: Dict[str, Any]) -> bool:
        ...
        framework = error_data.get("framework", "").lower()
        if "react-native" in framework or "react native" in framework or "expo" in framework:
            return True
        
        # Check runtime environment
        runtime = error_data.get("runtime", "").lower()
        if any(platform in runtime for platform in ["react-native", "react native", "expo", "android", "ios"]):
            # Additional check to ensure it's React Native, not just Android/iOS
            message = error_data.get("message", "").lower()
            stack_trace = str(error_data.get("stack_trace", "")).lower()
            if any(rn_indicator in message + stack_trace for rn_indicator in ["react-native", "react native", "metro", "rctbridge"]):
                return True
        ...
        if any(indicator in project_files_str for indicator in rn_project_indicators):
            ...
            if any(any(rn_dep in dep.lower() for rn_dep in rn_dependencies) for dep in dependencies):
                return True
