...
class ZigExceptionHandler:
    ...
    def __init__(self):
        ...
        self.zig_concepts = {
            "optionals": ["unwrapping null", "optional type mismatch", "missing null check"],
            "error_unions": ["error not handled", "error set mismatch", "missing try/catch"],
            ...
            "structs": ["field not found", "struct initialization", "field type mismatch"],
            "enums": ["enum value not found", "enum switch not exhaustive", "invalid enum"],
            "unions": ["union field access", "union tag mismatch", "tagged union error"],
            "pointers": ["null pointer", "pointer arithmetic", "alignment error"],
            "generics": ["generic parameter", "type parameter", "generic instantiation"]
        }
    ...
    def _analyze_zig_concepts(self, message: str) -> Dict[str, Any]:
        ...
        if any(keyword in message_lower for keyword in ["optional", "null", "?", "unwrap"]):
        ...
        if any(keyword in message_lower for keyword in ["error", "try", "catch", "!"]):
            if "error" in message_lower and ("not handled" in message_lower or "try" in message_lower):
                return {
                    "category": "zig",
        ...
        if any(keyword in message_lower for keyword in ["allocator", "allocation", "alloc", "free"]):
            return {
                "category": "zig",
        ...
        if any(keyword in message_lower for keyword in ["bounds", "index", "slice", "array"]):
            return {
                "category": "zig",
                "subcategory": "bounds",
                "confidence": "high",
...
class ZigPatchGenerator:
    ...
    def _load_templates(self) -> Dict[str, str]:
        ...
        if not self.zig_template_dir.exists():
            logger.warning(f"Zig templates directory not found: {self.zig_template_dir}")
            return templates
    ...
    def _fix_comptime_error(self, error_data: Dict[str, Any], analysis: Dict[str, Any], 
                           source_code: str) -> Optional[Dict[str, Any]]:
    ...
    def _fix_optional_error(self, error_data: Dict[str, Any], analysis: Dict[str, Any], 
                           source_code: str) -> Optional[Dict[str, Any]]:
    ...
    def _fix_error_union_error(self, error_data: Dict[str, Any], analysis: Dict[str, Any], 
                              source_code: str) -> Optional[Dict[str, Any]]:
    ...
    def _fix_bounds_error(self, error_data: Dict[str, Any], analysis: Dict[str, Any], 
                         source_code: str) -> Optional[Dict[str, Any]]:
