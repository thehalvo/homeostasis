...
class SwiftDependencyAnalyzer:
    ...
    def analyze_project_dependencies(self, project_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "error": str(e),
                "suggestions": [
                    "Check Package.swift syntax and Swift Package Manager setup"
                ],
            }
    ...
    def _extract_dependencies(self, content: str) -> List[Dict[str, Any]]:
        ...
        for pattern in dep_patterns:
            ...
            for match in matches:
                if "path:" in pattern:
                    dependencies.append({"type": "local", "path": match.group(1)})
                else:
                    dep = {"type": "remote", "url": match.group(1)}
    ...
    def _extract_targets(self, content: str) -> List[Dict[str, Any]]:
        ...
        target_pattern = (
            r'\.target\(\s*name:\s*"([^"]+)"(?:,\s*dependencies:\s*\[([^\]]*)\])?\)'
        )
    ...
    def _analyze_dependencies(
        self, package_info: Dict[str, Any], project_path: Path
    ) -> Dict[str, Any]:
        ...
        analysis = {
            ...
            "local_dependencies": len(
                [d for d in dependencies if d.get("type") == "local"]
            ),
            "potential_issues": [],
        }
            ...
            if dep.get("type") == "remote":
                ...
                if self._is_deprecated_package(url):
                    analysis["potential_issues"].append(
                        {"type": "deprecated_package", "url": url, "severity": "medium"}
                    )
                ...
                if not version_req:
                    analysis["potential_issues"].append(
                        {"type": "missing_version", "url": url, "severity": "low"}
                    )

        return analysis

    def _check_dependency_issues(
        self, package_info: Dict[str, Any], project_path: Path
    ) -> List[Dict[str, Any]]:
    ...
    def _generate_suggestions(self, issues: List[Dict[str, Any]]) -> List[str]:
        ...
        for issue in issues:
            ...
            if issue_type == "missing_package_resolved":
                suggestions.append(
                    "Run 'swift package resolve' to create Package.resolved"
                )
            elif issue_type == "missing_build_artifacts":
                suggestions.append(
                    "Run 'swift build' to download and build dependencies"
                )
            elif issue_type == "duplicate_dependency":
                package = issue.get("package", "unknown")
                suggestions.append(
                    f"Remove duplicate dependency declarations for {package}"
                )
    ...
    def analyze_dependency_error(
        ...
        if "No such module" in message:
            ...
            "could not build" in message.lower() and "swift package" in message.lower()
        ):
            return self._analyze_build_error(message, project_path)
        else:
            return self._analyze_generic_dependency_error(message, project_path)

    def _analyze_missing_module_error(
        self, message: str, project_path: str
    ) -> Dict[str, Any]:
    ...
    def _analyze_package_swift_error(
        self, message: str, project_path: str
    ) -> Dict[str, Any]:
    ...
    def _analyze_version_conflict_error(
        self, message: str, project_path: str
    ) -> Dict[str, Any]:
