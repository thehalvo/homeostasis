...
class SQLExceptionHandler:
    ...
    def analyze_exception(self, error_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if database_type in self.db_error_patterns:
            analysis = self._analyze_database_specific_error(message, error_code, database_type)
        else:
    ...
    def _detect_database_type(self, message: str, error_code: str) -> str:
        ...
        if any(indicator in message_lower for indicator in ["psql", "postgresql", "postgres"]):
            return "postgresql"
        ...
        if any(indicator in message_lower for indicator in ["sql server", "sqlserver", "mssql"]):
            return "sqlserver"
        
        # Oracle indicators
        if any(indicator in message_lower for indicator in ["oracle", "ora-"]) or error_code.startswith("ORA-"):
            return "oracle"
    ...
    def _analyze_database_specific_error(self, message: str, error_code: str, database_type: str) -> Dict[str, Any]:
        ...
        for pattern_type, pattern in patterns.items():
            if re.search(pattern, message, re.IGNORECASE):
                return self._get_error_analysis(pattern_type, database_type, message, error_code)
    ...
    def _analyze_generic_sql_error(self, message: str, error_code: str) -> Dict[str, Any]:
        ...
        if "table" in message_lower and ("not exist" in message_lower or "doesn't exist" in message_lower):
            return {
                "category": "sql",
                "subcategory": "table",
                "confidence": "high",
        ...
        if "column" in message_lower and ("not exist" in message_lower or "unknown" in message_lower):
            return {
                "category": "sql",
                "subcategory": "column",
                "confidence": "high",
    ...
    def _get_error_analysis(self, pattern_type: str, database_type: str, message: str, error_code: str) -> Dict[str, Any]:
        ...
        return error_analyses.get(pattern_type, {
            "category": "sql",
            "subcategory": "unknown",
            "confidence": "medium",
...
class SQLPatchGenerator:
    ...
    def _load_templates(self) -> Dict[str, str]:
        ...
        if not self.sql_template_dir.exists():
            logger.warning(f"SQL templates directory not found: {self.sql_template_dir}")
            return templates
    ...
    def generate_patch(self, error_data: Dict[str, Any], analysis: Dict[str, Any], 
        ...
        strategy = patch_strategies.get(specific_cause) or patch_strategies.get(root_cause)
    ...
    def _fix_table_not_exist(self, error_data: Dict[str, Any], analysis: Dict[str, Any], 
                           query: str) -> Optional[Dict[str, Any]]:
        """Fix table not exist errors."""
        message = error_data.get("message", "")
        
        # Extract table name from error message
        table_match = re.search(r'(?:table|relation)\s+["\']?([^"\']+)["\']?\s+(?:does not exist|doesn\'t exist)', message, re.IGNORECASE)
        if table_match:
    ...
    def _fix_column_not_exist(self, error_data: Dict[str, Any], analysis: Dict[str, Any], 
                            query: str) -> Optional[Dict[str, Any]]:
        """Fix column not exist errors."""
        message = error_data.get("message", "")
        
        # Extract column name from error message
        column_match = re.search(r'(?:column|field)\s+["\']?([^"\']+)["\']?\s+(?:does not exist|doesn\'t exist|unknown)', message, re.IGNORECASE)
        if column_match:
    ...
    def _fix_unique_constraint(self, error_data: Dict[str, Any], analysis: Dict[str, Any], 
                             query: str) -> Optional[Dict[str, Any]]:
    ...
    def _fix_foreign_key_constraint(self, error_data: Dict[str, Any], analysis: Dict[str, Any], 
                                  query: str) -> Optional[Dict[str, Any]]:
