...
class CapacitorCordovaExceptionHandler:
        ...
        try:
            ...
            if common_rules_path.exists():
                with open(common_rules_path, "r") as f:
                    ...
                    logger.info(
                        f"Loaded {len(rules['common'])} common Capacitor/Cordova rules"
                    )
            else:
        ...
        for category, rule_list in self.rules.items():
            ...
            for rule in rule_list:
                try:
                    ...
                    logger.warning(
                        f"Invalid regex pattern in Capacitor/Cordova rule {rule.get('id', 'unknown')}: {e}"
                    )
    ...
    def _calculate_confidence(
        ...
        if any(
            term in message or term in framework
            for term in ["capacitor", "cordova", "ionic"]
        ):
            base_confidence += 0.3
    ...
    def analyze_plugin_error(self, error_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "plugin" in message and (
            "not found" in message or "not installed" in message
        ):
            return {
                "category": "capacitor_cordova",
                "subcategory": "plugin_integration",
        ...
        if "method" in message and (
            "not available" in message or "not supported" in message
        ):
            return {
                "category": "capacitor_cordova",
                "subcategory": "plugin_integration",
    ...
    def analyze_platform_build_error(
...
class CapacitorCordovaPatchGenerator:
    ...
    def _load_templates(self) -> Dict[str, str]:
        ...
        if not self.cap_cordova_template_dir.exists():
            logger.warning(
                f"Capacitor/Cordova templates directory not found: {self.cap_cordova_template_dir}"
            )
            return templates
        ...
        for template_file in self.cap_cordova_template_dir.glob("*.template"):
            try:
                ...
                logger.error(
                    f"Error loading Capacitor/Cordova template {template_file}: {e}"
                )
...
""",
        ...
        for template_name, template_content in default_templates.items():
            ...
            if not template_path.exists():
                try:
                    ...
                    logger.debug(
                        f"Created default Capacitor/Cordova template: {template_name}"
                    )
                except Exception as e:
                    logger.error(
                        f"Error creating default Capacitor/Cordova template {template_name}: {e}"
                    )
    ...
    def _fix_plugin_installation(
        ...
        plugin_match = re.search(
            r"plugin[:\s]+['\"]?([^'\"\\s]+)['\"]?", message, re.IGNORECASE
        )
...
class CapacitorCordovaLanguagePlugin(LanguagePlugin):
    ...
    def can_handle(self, error_data: Dict[str, Any]) -> bool:
        ...
        if any(
            indicator in project_files_str for indicator in hybrid_project_indicators
        ):
            # Additional check for hybrid dependencies
            dependencies = context.get("dependencies", [])
            hybrid_dependencies = ["@capacitor", "@ionic", "cordova", "phonegap"]
            if any(
                any(hybrid_dep in dep.lower() for hybrid_dep in hybrid_dependencies)
    ...
    def analyze_error(self, error_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Check if it's a platform build error
                ...
                analysis = self.exception_handler.analyze_platform_build_error(
                    standard_error
                )
