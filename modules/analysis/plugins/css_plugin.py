...
class CSSExceptionHandler:
        ...
        for category, rule_list in self.rules.items():
            ...
            for rule in rule_list:
                try:
                    ...
                    logger.warning(
                        f"Invalid regex pattern in CSS rule {rule.get('id', 'unknown')}: {e}"
                    )
    ...
    def _find_matching_rules(
        ...
        if "css syntax error" in message and not any(
            fw in message for fw in ["tailwind", "styled", "emotion", "sass", "less"]
        ):
            return []
    ...
    def _calculate_confidence(
        ...
        if any(
            term in message
            for term in ["css", "style", "tailwind", "emotion", "styled"]
        ):
            base_confidence += 0.3
    ...
    def _generic_analysis(self, error_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "tailwind" in message:
            ...
            category = "css_in_js"
            suggestion = (
                "Check CSS-in-JS library configuration and styled component syntax"
            )
        ...
        if category != "unknown":
            ...
            if any(
                fw in message
                for fw in ["tailwind", "styled-components", "emotion", "sass", "less"]
            ):
                confidence = "high"
    ...
    def _analyze_styled_components_error(
...
class CSSPatchGenerator:
    ...
    def _load_templates(self) -> Dict[str, str]:
        ...
        if not self.css_template_dir.exists():
            logger.warning(
                f"CSS templates directory not found: {self.css_template_dir}"
            )
            return templates
    ...
    def _fix_tailwind_unknown_class(
        self, error_data: Dict[str, Any], analysis: Dict[str, Any], source_code: str
    ) -> Optional[Dict[str, Any]]:
    ...
    def _fix_tailwind_config_error(
        self, error_data: Dict[str, Any], analysis: Dict[str, Any], source_code: str
    ) -> Optional[Dict[str, Any]]:
    ...
    def _fix_styled_components_babel(
        self, error_data: Dict[str, Any], analysis: Dict[str, Any], source_code: str
    ) -> Optional[Dict[str, Any]]:
...
""",
    ...
    def _fix_styled_components_ssr(
        self, error_data: Dict[str, Any], analysis: Dict[str, Any], source_code: str
    ) -> Optional[Dict[str, Any]]:
...
class CSSLanguagePlugin(LanguagePlugin):
    ...
    def _is_layout_error(self, error_data: Dict[str, Any]) -> bool:
        ...
        layout_patterns = ["grid", "flexbox", "flex", "layout", "align", "justify"]
