...
class ErlangExceptionHandler:
    ...
    def _analyze_erlang_concepts(self, message: str) -> Dict[str, Any]:
        ...
        if any(
            keyword in message_lower for keyword in self.erlang_concepts["supervision"]
        ):
            return {
                "category": "erlang",
                "subcategory": "supervision",
...
class ErlangPatchGenerator:
    ...
    def _load_templates(self) -> Dict[str, str]:
        ...
        if not self.erlang_template_dir.exists():
            logger.warning(
                f"Erlang templates directory not found: {self.erlang_template_dir}"
            )
            return templates
