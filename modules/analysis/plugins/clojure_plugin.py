...
class ClojureExceptionHandler:
    ...
    def analyze_exception(self, error_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for rule in self.rules:
            ...
            try:
                ...
                logger.warning(
                    f"Error matching pattern in rule {rule.get('id', 'unknown')}: {e}"
                )
                continue
                ...
                try:
                    with open(rule_path, "r") as f:
                        ...
                        if isinstance(file_rules, list):
                            ...
                            logger.warning(
                                f"Unexpected format in rule file: {rule_file}"
                            )
                except Exception as e:
    ...
    def _create_match_text(
        self, error_type: str, message: str, stack_trace: Union[List, str]
    ) -> str:
        """
    ...
    def _enhance_analysis(
        self, base_result: Dict[str, Any], error_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        base_result["fix_suggestions"] = self._enhance_fix_suggestions(
            base_result.get("fix_suggestions", []), error_data, framework_context
        )
    ...
    def _extract_namespace_context(
        self, error_data: Dict[str, Any]
    ) -> Optional[Dict[str, Any]]:
    ...
    def _enhance_fix_suggestions(
        self,
        base_suggestions: List[str],
        error_data: Dict[str, Any],
        framework_context: Optional[Dict[str, Any]],
    ) -> List[str]:
        """Enhance fix suggestions based on context."""
        ...
        if framework_context:
            ...
            if "ring" in frameworks:
                enhanced.append(
                    "Check Ring middleware configuration and handler return values"
                )

            if "core.async" in frameworks:
                enhanced.append(
                    "Verify channel operations and ensure proper blocking/non-blocking usage"
                )

            if "spec" in frameworks:
                enhanced.append(
                    "Validate data against spec definitions and check instrumented functions"
                )
...
class ClojurePatchGenerator:
    ...
    def __init__(self):
        ...
        self.templates_dir = (
            Path(__file__).parent.parent / "patch_generation" / "templates" / "clojure"
        )
    ...
    def _generate_from_template(
        self, template_path: Path, analysis_result: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            patch_content = self._apply_template_substitutions(
