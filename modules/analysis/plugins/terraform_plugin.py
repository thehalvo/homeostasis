...
class TerraformExceptionHandler:
    ...
    def analyze_exception(self, error_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not provider:
            provider = self._detect_provider(message, error_data.get("config_content", ""))
    ...
    def _detect_provider(self, message: str, config_content: str) -> str:
        ...
        if any(indicator in message_lower for indicator in ["aws", "amazon", "s3", "ec2", "iam"]):
            return "aws"
        elif any(indicator in message_lower for indicator in ["azure", "azurerm", "microsoft"]):
            return "azurerm"
        elif any(indicator in message_lower for indicator in ["google", "gcp", "googleapis"]):
            return "google"
    ...
    def _analyze_provider_specific_error(self, message: str, provider: str) -> Dict[str, Any]:
...
class TerraformPatchGenerator:
    ...
    def _load_templates(self) -> Dict[str, str]:
        ...
        if not self.terraform_template_dir.exists():
            logger.warning(f"Terraform templates directory not found: {self.terraform_template_dir}")
            return templates
        ...
        if provider != "unknown":
            ...
            if provider_strategy:
                try:
                    ...
                    logger.error(f"Error generating provider-specific patch for {root_cause}: {e}")
    ...
    def _fix_syntax_error(self, error_data: Dict[str, Any], analysis: Dict[str, Any], 
        ...
        if "missing required argument" in message.lower():
            ...
            arg_match = re.search(r'argument "([^"]+)" is required', message, re.IGNORECASE)
            if arg_match:
        ...
        if "unsupported argument" in message.lower():
            # Extract argument name
            arg_match = re.search(r'argument "([^"]+)" is not expected', message, re.IGNORECASE)
            if arg_match:
                arg_name = arg_match.group(1)
                fixes.append({
                    "type": "suggestion",
    ...
    def _fix_resource_error(self, error_data: Dict[str, Any], analysis: Dict[str, Any], 
                           config_content: str) -> Optional[Dict[str, Any]]:
        ...
        resource_match = re.search(r'Error (?:creating|reading|updating|deleting) ([^:]+):', message)
        ...
        if "does not exist" in message.lower():
            fixes.insert(0, f"Create the {resource_type} or update references to existing resources")
    ...
    def _fix_provider_error(self, error_data: Dict[str, Any], analysis: Dict[str, Any], 
                           config_content: str) -> Optional[Dict[str, Any]]:
    ...
    def _fix_state_error(self, error_data: Dict[str, Any], analysis: Dict[str, Any], 
                        config_content: str) -> Optional[Dict[str, Any]]:
    ...
    def _fix_dependency_error(self, error_data: Dict[str, Any], analysis: Dict[str, Any], 
                             config_content: str) -> Optional[Dict[str, Any]]:
    ...
    def _fix_backend_error(self, error_data: Dict[str, Any], analysis: Dict[str, Any], 
                          config_content: str) -> Optional[Dict[str, Any]]:
    ...
    def _fix_state_lock_error(self, error_data: Dict[str, Any], analysis: Dict[str, Any], 
                             config_content: str) -> Optional[Dict[str, Any]]:
    ...
    def _template_based_patch(self, error_data: Dict[str, Any], analysis: Dict[str, Any], 
        ...
        template_map = {
            ...
            "terraform_provider_error": f"{provider}_provider_fix" if provider != "unknown" else "provider_fix",
