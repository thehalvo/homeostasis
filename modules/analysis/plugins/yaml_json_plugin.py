...
class YAMLJSONExceptionHandler:
    ...
    def analyze_exception(self, error_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if error_type == "SchemaError":
            ...
            "expected" in message.lower() or "got" in message.lower()
        ):
            return {
                "category": "yaml_json",
                "subcategory": "type",
    ...
    def _detect_framework(self, content: str, file_path: str) -> str:
        ...
        if content:
            ...
            for framework, config in self.config_frameworks.items():
                pattern_matches = sum(
                    1 for pattern in config["patterns"] if pattern in content_lower
                )
    ...
    def _analyze_yaml_error(
        self, message: str, content: str, framework: str
    ) -> Dict[str, Any]:
        ...
        if framework in self.config_frameworks:
            return self._analyze_framework_specific_error(
                message, content, framework, "yaml"
            )
    ...
    def _analyze_json_error(
        ...
        if framework in self.config_frameworks:
            return self._analyze_framework_specific_error(
                message, content, framework, "json"
            )
    ...
    def _analyze_framework_specific_error(
        self, message: str, content: str, framework: str, file_type: str
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "subcategory": framework,
            "confidence": "medium",
            "suggested_fix": framework_fixes.get(
                framework, f"Check {framework} configuration syntax and structure"
            ),
    ...
    def _analyze_generic_config_error(
        self, message: str, content: str, framework: str
    ) -> Dict[str, Any]:
...
class YAMLJSONPatchGenerator:
    ...
    def _load_templates(self) -> Dict[str, str]:
        ...
        if not self.yaml_json_template_dir.exists():
            logger.warning(
                f"YAML/JSON templates directory not found: {self.yaml_json_template_dir}"
            )
            return templates
    ...
    def generate_patch(
        self, error_data: Dict[str, Any], analysis: Dict[str, Any], content: str = ""
    ...
    def _fix_json_unquoted_property(
        self, error_data: Dict[str, Any], analysis: Dict[str, Any], content: str
    ) -> Optional[Dict[str, Any]]:
    ...
    def _fix_json_unterminated_string(
        self, error_data: Dict[str, Any], analysis: Dict[str, Any], content: str
    ) -> Optional[Dict[str, Any]]:
    ...
    def normalize_error(self, error_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        normalized = {
            ...
            "file_path": error_data.get(
                "file_path", error_data.get("filename", error_data.get("file", ""))
            ),
