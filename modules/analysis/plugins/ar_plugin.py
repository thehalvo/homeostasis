...
from ...emerging_tech.augmented_reality import (ARError, ARPerformanceMetrics,
                                                ARPlatform,
                                                ARResilienceManager)
...
class ARPlugin(LanguagePlugin):
    ...
    def _load_rules(self):
        ...
        rules_path = os.path.join(
            os.path.dirname(__file__), "../rules/ar/ar_errors.json"
        )
    ...
    def _check_platform_specific_issues(
        self, code: str, platform: ARPlatform
    ) -> List[Dict[str, Any]]:
    ...
    def _check_unity_ar_issues(self, code: str) -> List[Dict[str, Any]]:
        ...
        if re.search(r"ARRaycastManager.*Raycast.*\(.*\)", code) and not re.search(
            r"if\s*\(\s*\w+\s*!=\s*null", code
        ):
            issues.append(
                {
    ...
    def _check_arkit_issues(self, code: str) -> List[Dict[str, Any]]:
        ...
        if "ARWorldTrackingConfiguration" in code and not re.search(
            r"isSupported|checkAvailability", code
        ):
            issues.append(
                {
    ...
    def _check_webxr_issues(self, code: str) -> List[Dict[str, Any]]:
        ...
        if "navigator.xr.requestSession" in code and "isSessionSupported" not in code:
            issues.append(
                {
    ...
    def analyze_error_detailed(
        ...
        if performance_metrics:
            metrics = ARPerformanceMetrics(
                ...
                motion_to_photon_latency_ms=performance_metrics.get(
    ...
    def get_platform_info(self, code: str, file_path: str = None) -> Dict[str, Any]:
        ...
        if platform.value in self.rules.get("platform_specific", {}):
            platform_info["requirements"] = self.rules["platform_specific"][
                platform.value
            ]
    ...
    def _detect_tracking_capabilities(
        self, code: str, platform: ARPlatform
    ) -> List[str]:
    ...
    def generate_platform_config(
        self, platform: str, device_capabilities: Dict[str, Any]
    ) -> Dict[str, Any]:
