...
class JavaScriptDependencyAnalyzer:
    ...
    def __init__(self):
        ...
        self.package_files = ["package.json", "package-lock.json", "yarn.lock", "pnpm-lock.yaml"]
        ...
        self.known_issues = {
            ...
            "common_conflicts": [
                ...
                {"packages": ["webpack", "webpack-dev-server"], "reason": "version_compatibility"}
            ],
            "security_vulnerable": [
    ...
    def analyze_dependency_error(self, error_data: Dict[str, Any], project_path: str) -> Dict[str, Any]:
        ...
        if "version" in error_message.lower() and ("conflict" in error_message.lower() or "mismatch" in error_message.lower()):
    ...
    def _analyze_peer_dependencies(self, package_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _find_dependency_issues(self, dependencies: Dict[str, str]) -> List[Dict[str, Any]]:
    ...
    def _detect_version_conflicts(self, package_data: Dict[str, Any], project_path: Path) -> List[Dict[str, Any]]:
    ...
    def _detect_missing_dependencies(self, project_path: Path) -> List[Dict[str, Any]]:
        ...
        if package_json_path.exists():
            ...
            for module in imported_modules:
                ...
                if module.startswith('.') or module in ['fs', 'path', 'http', 'https', 'url', 'crypto']:
                    continue
                
                # Extract package name (handle scoped packages)
                package_name = module.split('/')[0] if not module.startswith('@') else '/'.join(module.split('/')[:2])
    ...
    def _detect_security_issues(self, package_data: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    def _detect_outdated_packages(self, package_data: Dict[str, Any]) -> List[Dict[str, Any]]:
        """Detect outdated packages."""
        # This would typically run npm outdated or yarn outdated
        # For now, return empty list as this requires external calls
        return []
    
    def _analyze_module_not_found(self, error_message: str, project_path: str) -> Dict[str, Any]:
        ...
        package_name = module_name.split('/')[0] if not module_name.startswith('@') else '/'.join(module_name.split('/')[:2])
    ...
    def _analyze_version_conflict(self, error_message: str, project_path: str) -> Dict[str, Any]:
    ...
    def _analyze_peer_dependency_issue(self, error_message: str, project_path: str) -> Dict[str, Any]:
    ...
    def _analyze_engine_compatibility(self, error_message: str, project_path: str) -> Dict[str, Any]:
    ...
    def _find_missing_peer_dependencies(self, package_data: Dict[str, Any]) -> List[str]:
    ...
    def _generate_suggestions(self, analysis: Dict[str, Any]) -> List[str]:
        ...
        if analysis["conflicts"]:
            suggestions.append("Resolve version conflicts by updating packages to compatible versions")
        
        # Suggestions for missing dependencies
        if analysis["missing_dependencies"]:
            package_manager = analysis["package_manager"]
            missing_packages = [dep["package"] for dep in analysis["missing_dependencies"]]
            suggestions.append(f"Install missing dependencies: {package_manager} install {' '.join(missing_packages)}")
        
        # Suggestions for security issues
        if analysis["security_issues"]:
            suggestions.append("Update packages with security vulnerabilities")
            suggestions.append("Run 'npm audit fix' or 'yarn audit --fix' to automatically fix issues")
        
        # Suggestions for peer dependencies
        missing_peers = analysis["peer_dependencies"]["missing"]
        if missing_peers:
            package_manager = analysis["package_manager"]
            suggestions.append(f"Install missing peer dependencies: {package_manager} install {' '.join(missing_peers)}")
