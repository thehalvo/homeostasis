...
class RubyPatchGenerator:
    ...
    def __init__(self):
        ...
        self.templates_dir = (
            Path(__file__).parent.parent / "patch_generation" / "templates" / "ruby"
        )
    ...
    def generate_patch(
        ...
        if "patch_code" not in patch_result:
            ...
            if root_cause == "ruby_nil_reference":
                patch_result["suggestion_code"] = self._generate_nil_check_suggestion(
                    analysis, context
                )
            elif root_cause == "ruby_record_not_found":
                patch_result["suggestion_code"] = (
                    self._generate_record_not_found_suggestion(analysis, context)
                )
            elif root_cause == "ruby_load_error" or root_cause == "ruby_missing_gem":
                patch_result["suggestion_code"] = self._generate_load_error_suggestion(
                    analysis, context
                )
    ...
    def _generate_instructions(self, analysis: Dict[str, Any], patch_code: str) -> str:
        ...
        if "nil_reference" in root_cause:
            ...
            return (
                "Use find_by instead of find to avoid exceptions for missing records."
            )
...
"""
    ...
    def _generate_record_not_found_suggestion(
        self, analysis: Dict[str, Any], context: Dict[str, Any]
    ) -> str:
...
"""
    ...
    def _generate_load_error_suggestion(
        self, analysis: Dict[str, Any], context: Dict[str, Any]
    ) -> str:
