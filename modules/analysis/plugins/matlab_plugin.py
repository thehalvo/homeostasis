...
class MATLABExceptionHandler:
    ...
    def _analyze_by_patterns(self, message: str, file_path: str) -> Dict[str, Any]:
        ...
        if ("undefined function or variable" in message_lower or
        ...
        if "matrix dimensions" in message_lower and (
            "must agree" in message_lower or "not agree" in message_lower
        ):
            return {
                "category": "matlab",
    ...
    def _analyze_matlab_concepts(self, message: str) -> Dict[str, Any]:
        ...
        if any(
            ...
            for keyword in ["undefined", "variable", "function"]
        ):
            return {
                "category": "matlab",
                "subcategory": "undefined",
        ...
        # Check for index errors
        if any(
            keyword in message_lower for keyword in ["index", "subscript", "bounds"]
        ):
            return {
                "category": "matlab",
                "subcategory": "index",
        ...
        if any(
            keyword in message_lower for keyword in ["toolbox", "license", "feature"]
        ):
            return {
                "category": "matlab",
                "subcategory": "toolbox",
        ...
        if any(
            keyword in message_lower for keyword in ["file", "load", "save", "read"]
        ):
            return {
                "category": "matlab",
                "subcategory": "file",
...
class MATLABPatchGenerator:
    ...
    def _load_templates(self) -> Dict[str, str]:
        ...
        if not self.matlab_template_dir.exists():
            logger.warning(
                f"MATLAB templates directory not found: {self.matlab_template_dir}"
            )
            return templates
    ...
    def _fix_matrix_error(
        ...
        if ("array indices" in message.lower() and
    ...
    def _fix_toolbox_error(
        ...
        if "requires" in message.lower() and "toolbox" in message.lower():
            ...
            toolbox_match = re.search(
                r"requires.*?([A-Za-z\s]+).*?toolbox", message, re.IGNORECASE
            )
            toolbox_name = (
                toolbox_match.group(1).strip() if toolbox_match else "toolbox"
            )
