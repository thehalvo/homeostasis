...
try:
    ...
    from rule_config import DEFAULT_RULES_DIR, get_all_rule_sets
except ImportError:
    try:
        # Try relative import
...
class ASCIIBar:
    ...
    @staticmethod
    def generate_horizontal_bar_with_label(
        label: str,
        value: int,
        max_value: int,
        width: int = 40,
        ...
        """
        bar = ASCIIBar.generate_horizontal_bar(
            value, max_value, width, fill_char, empty_char
        )
    ...
    @staticmethod
    def generate_stacked_bar_with_legend(
        values: Dict[str, int],
        max_value: int,
        width: int = 60,
        title: str = "",
...
class RuleVisualizer:
    ...
    def _collect_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "example_coverage": {
                ...
                "coverage_percentage": (
                    (rules_with_examples / len(self.rules) * 100) if self.rules else 0
                ),
            },
        }
    ...
    def visualize_severity_distribution(self, width: int = 60) -> str:
        ...
        fills = {"critical": "█", "high": "▓", "medium": "▒", "low": "░", "info": " "}

        filtered_severities = {
            k: v for k, v in by_severity.items() if k in ordered_severities
        }
    ...
    def visualize_dependency_graph(self) -> str:
        ...
        rules_with_deps = [
            r for r in self.rules if hasattr(r, "dependencies") and r.dependencies
        ]
    ...
    def export_html(self, output_path: Path) -> None:
        ...
        html_content = html_content.replace(
            "{{total_rules}}", str(self.stats["total_rules"])
        )

        example_coverage = self.stats["example_coverage"]
        html_content = html_content.replace(
            "{{rules_with_examples}}", str(example_coverage["rules_with_examples"])
        )
        html_content = html_content.replace(
            "{{example_coverage_percent}}",
            f"{example_coverage['coverage_percentage']:.1f}",
        )
        html_content = html_content.replace(
            "{{total_examples}}", str(example_coverage["total_examples"])
        )
        html_content = html_content.replace(
            "{{examples_per_rule}}",
            (
        ...
        html_content = html_content.replace(
            "{{rule_type_segments}}", rule_type_segments
        )
...
def main() -> None:
    ...
    parser.add_argument(
        "--html", "-o", type=str, help="Generate HTML visualization to specified file"
    )
    parser.add_argument(
        "--width",
        "-w",
    ...
    parser.add_argument(
        "--category", "-c", action="store_true", help="Show category distribution only"
    )
    parser.add_argument(
        "--severity", "-s", action="store_true", help="Show severity distribution only"
    )
    parser.add_argument(
        "--tags", "-g", action="store_true", help="Show tag distribution only"
    )
    parser.add_argument(
        "--examples", "-e", action="store_true", help="Show example coverage only"
    )
    parser.add_argument(
        "--dependencies", "-d", action="store_true", help="Show dependency graph only"
    )
    ...
    if (args.text or
        ...
        if args.text and not any(
            [args.category, args.severity, args.tags, args.examples, args.dependencies]
        ):
