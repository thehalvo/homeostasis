...
class EnhancedCrossLanguageOrchestrator(CrossLanguageOrchestrator):
    ...
    def generate_cross_language_fix(
        self,
        ...
        cross_language_suggestions = self.find_cross_language_fixes(
            error_data, language
        )

        # Enhance context with cross-language information
        enhanced_context = self._enhance_context_with_cross_language(
            context, cross_language_suggestions
        )
        ...
        for error in similar_errors:
            ...
            if error_id in self.error_history:
                ...
                try:
                    ...
                    logger.warning(
                        f"Error analyzing similar error in {error_lang}: {e}"
                    )
                    continue
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        if metrics["language_detection_times"]:
            metrics["avg_language_detection_time"] = sum(
                metrics["language_detection_times"]
            ) / len(metrics["language_detection_times"])
    ...
    def _store_enhanced_error_analysis(
    ...
    def _get_cache_key(
        ...
        if context:
            ...
            context_hash = hashlib.md5(context_str.encode(), usedforsecurity=False).hexdigest()[:10]
    ...
    def _enhance_context_with_cross_language(
        self, context: Dict[str, Any], suggestions: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
        ...
        if suggestions:
            ...
            if good_suggestions:
                ...
                enhanced_context["source_languages"] = [
                    s["language"] for s in good_suggestions
                ]
...
if __name__ == "__main__":
    ...
    for lang, error_data in examples.items():
        if lang in languages:
            ...
            try:
                ...
                fix = orchestrator.generate_cross_language_fix(
                    error_data, lang, context
                )
                logger.info(
                    f"{lang} fix: {fix.get('suggestion') or fix.get('patch_code', '(no suggestion)')}"
                )
                ...
                for i, cross_fix in enumerate(cross_fixes):
                    if i < 2:  # Show only the first 2
                        logger.info(
                            f"  Suggestion from {cross_fix['language']} (similarity: {cross_fix['similarity']:.2f}):"
                        )
    ...
    if "avg_language_detection_time" in metrics:
        logger.info(
            f"  Avg language detection time: {metrics['avg_language_detection_time']:.3f}s"
        )
