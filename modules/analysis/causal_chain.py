...
class ErrorNode:
    ...
    causes: List[str] = field(default_factory=list)  # IDs of errors that caused this one
    effects: List[str] = field(default_factory=list)  # IDs of errors caused by this one
    variables: Dict[str, Any] = field(default_factory=dict)  # Variables involved in the error
    ...
    is_terminal: bool = False  # Whether this is a terminal effect with no further consequences
...
class CausalChainAnalyzer:
    ...
    def _detect_causality(self, node1: ErrorNode, node2: ErrorNode) -> Tuple[float, str, Dict[str, Any]]:
        """
        ...
        same_service = (
            node1.context.get("service", "") == node2.context.get("service", "") and 
            node1.context.get("service", "")
        )
        
        # Special case: Check if node2 is caused by None/null access to an object
        if node2.error_type == "TypeError" and "NoneType" in str(node2.error_data.get("message", "")):
        ...
        for pattern in error_chain_patterns:
            if node1.error_type == pattern["cause"] and node2.error_type == pattern["effect"]:
    ...
    def analyze_error_sequence(self, error_sequence: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        for id1, node1 in self.error_graph.items():
            for id2, node2 in self.error_graph.items():
                if id1 != id2:
                    ...
                    causality_confidence, reason, evidence = self._detect_causality(node1, node2)
    ...
    def detect_error_patterns(self, error_sequence: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        return {
            ...
            "error_type_patterns": [
                ...
                for pattern, count in sorted(type_patterns.items(), key=lambda x: x[1], reverse=True)
            ],
            "root_cause_patterns": [
                {"pattern": pattern, "count": count}
                for pattern, count in sorted(cause_patterns.items(), key=lambda x: x[1], reverse=True)
            ],
