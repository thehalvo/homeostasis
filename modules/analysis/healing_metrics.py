...
class HealingMetricsCollector:
    ...
    def track_healing_attempt(self, error_data: Dict[str, Any],
                            language: str,
                            successful: bool,
        ...
        self.metrics["global"]["healing_success_rate"] = (
            ...
            self.metrics["global"]["healing_attempts"] * 100
        )
        ...
        lang_metrics["avg_healing_time"] = (
            (lang_metrics["avg_healing_time"] * (lang_metrics["attempts"] - 1) + total_time) /
        ...
        if is_cross_language and source_language:
            ...
            self.metrics["cross_language"]["success_rate"] = (
                ...
                self.metrics["cross_language"]["attempts"] * 100
            )
    ...
    def track_regression_prevention(self, error_data: Dict[str, Any],
                                  language: str,
                                  prevented: bool) -> None:
        """
    ...
    def get_healing_efficiency(self, language: Optional[str] = None,
        ...
        if error_type:
            if error_type in self.metrics["error_types"]:
                ...
                result["error_type"] = {"warning": f"No metrics for error type: {error_type}"}
    ...
    def get_trends(self, period: str = "weekly", 
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        report = {
            ...
            "summary": {
                ...
                "cross_language_success_rate": self.metrics["cross_language"]["success_rate"],
                "regression_prevention_rate": self.metrics["regression"]["prevention_rate"]
            },
            "performance": {
                "avg_total_healing_time": self.metrics["global"]["avg_total_healing_time"],
                "avg_detection_time": self.metrics["global"]["avg_detection_time"],
                "avg_analysis_time": self.metrics["global"]["avg_analysis_time"],
                "avg_fix_generation_time": self.metrics["global"]["avg_fix_generation_time"],
                "avg_fix_application_time": self.metrics["global"]["avg_fix_application_time"]
            },
    ...
    def export_metrics(self, format: str = "json", 
        ...
        if output_file is None:
            ...
            output_file = METRICS_DIR / f"healing_metrics_export_{timestamp}.{format.lower()}"
        else:
    ...
    def _update_timing_metrics(self, timings: Dict[str, float]) -> None:
            ...
            if phase in timings:
                ...
                if attempts == 1:
                    ...
                    global_metrics[default_metric] = (
                        (current_avg * (attempts - 1) + time_value) / attempts
                    )
        ...
        if attempts == 1:
            ...
            global_metrics["avg_total_healing_time"] = (
                (global_metrics["avg_total_healing_time"] * (attempts - 1) + total_time) / attempts
            )
    ...
    def _get_top_performers(self, category: str, limit: int = 5) -> Dict[str, Any]:
        ...
        significant_items = [
            (name, metrics) for name, metrics in items
...
def track_healing_event(orchestrator, error_data: Dict[str, Any],
                      language: str,
                      fix_data: Optional[Dict[str, Any]] = None,
                      successful: bool = False,
                      timings: Optional[Dict[str, float]] = None,
                      metrics_collector: Optional[HealingMetricsCollector] = None) -> None:
    """
    ...
    if fix_data and fix_data.get("used_cross_language") and "source_languages" in fix_data:
...
if __name__ == "__main__":
    ...
    error_types = {
        ...
        "java": ["NullPointerException", "ClassCastException", "ArrayIndexOutOfBoundsException"],
    ...
    logger.info(f"Total healing attempts: {report['summary']['total_healing_attempts']}")
    ...
    for lang, metrics in report["top_languages"].items():
        logger.info(f"  {lang}: {metrics['success_rate']:.1f}% success rate from {metrics['attempts']} attempts")
