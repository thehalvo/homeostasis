...
class JavaParser(LanguageSpecificParser):
    ...
    def __init__(self):
        ...
        self.framework_patterns = [
            (
                r"org\.springframework\.beans\.factory\.NoSuchBeanDefinitionException",
                "spring_bean_not_found",
            ),
        ...
        try:
            with tempfile.NamedTemporaryFile(
                ...
                result = subprocess.run(
                    ["javac", f.name], capture_output=True, text=True, timeout=10
                )
...
class GoParser(LanguageSpecificParser):
    ...
    def __init__(self):
        ...
        self.runtime_patterns = [
            ...
            (r"panic: (.+)", "panic"),
            (
                r"fatal error: concurrent map read and map write",
    ...
    def _test_compile_go(self, source_code: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            with tempfile.NamedTemporaryFile(mode="w", suffix=".go", delete=False) as f:
                ...
                # Try to compile
                result = subprocess.run(
                    ["go", "build", f.name], capture_output=True, text=True, timeout=10
                )
...
class CSharpParser(LanguageSpecificParser):
    ...
    def __init__(self):
        ...
        self.compilation_patterns = [
            ...
            (
                r"error CS0246: The type or namespace name '(.+)' could not be found",
                "type_not_found",
            ),
...
class DartParser(LanguageSpecificParser):
    ...
    def __init__(self):
        ...
        self.flutter_patterns = [
            ...
            (
                r"MediaQuery\.of\(\) called with a context that does not contain a MediaQuery",
                "media_query_error",
            ),
            (
                r"Provider\.of<(.+)>\(\) called with a context that does not contain a (.+)",
                "provider_error",
            ),
        ]
...
class XamarinParser(LanguageSpecificParser):
    ...
    def __init__(self):
        ...
        self.xamarin_compilation_patterns = [
            ...
            (
                r"The type or namespace name '(.+)' does not exist in the namespace '(.+)'",
                "namespace_error",
            ),
            (
                r"No resource found that matches the given name: '(.+)'",
                "resource_not_found",
            ),
        ]
    ...
    def __init__(self):
        ...
        self.unity_runtime_patterns = [
            (
                ...
                "unity_null_reference",
            ),
            (
                r"MissingReferenceException: The object of type '(.+)' has been destroyed",
...
class CompilerIntegration:
    ...
    def get_detailed_diagnostics(
        self, source_code: str, language: LanguageType, file_path: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            elif language == LanguageType.RUST and self.available_compilers.get(
                "rustc"
            ):
                diagnostics.update(self._analyze_rust_code(source_code))
            elif language == LanguageType.CSHARP and self.available_compilers.get(
                "dotnet"
            ):
                diagnostics.update(self._analyze_csharp_code(source_code))
            elif language == LanguageType.TYPESCRIPT and self.available_compilers.get(
                "tsc"
            ):
        ...
        try:
            with tempfile.NamedTemporaryFile(mode="w", suffix=".go", delete=False) as f:
                ...
                build_result = subprocess.run(
                    ["go", "build", f.name], capture_output=True, text=True, timeout=15
                )
    ...
    def _analyze_dart_code(self, source_code: str) -> Dict[str, Any]:
        ...
        try:
            with tempfile.NamedTemporaryFile(
                ...
                if not result["syntax_valid"] and self.available_compilers.get(
                    "flutter"
                ):
                    ...
                    if flutter_result.stdout:
                        result["compilation_errors"].extend(
                            flutter_result.stdout.strip().split("\n")
                        )
...
def create_language_parser(language: LanguageType) -> Optional[LanguageSpecificParser]:
    ...
    """
    parser_map = {
        LanguageType.PYTHON: lambda: __import__(
            "comprehensive_error_detector", fromlist=["PythonParser"]
        ).PythonParser(),
        LanguageType.JAVASCRIPT: lambda: __import__(
            "comprehensive_error_detector", fromlist=["JavaScriptParser"]
        ).JavaScriptParser(),
...
if __name__ == "__main__":
    ...
    unity_context = ErrorContext(
        error_message=unity_error, language=LanguageType.CSHARP
    )
    ...
    python_diagnostics = integration.get_detailed_diagnostics(
        python_code, LanguageType.PYTHON
    )
    ...
    if integration.available_compilers.get("dart"):
        ...
        dart_diagnostics = integration.get_detailed_diagnostics(
            dart_code, LanguageType.DART
        )
