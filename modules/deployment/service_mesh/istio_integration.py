...
class IstioIntegration:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.template_dir = Path(self.config.get("template_dir", 
                                           "modules/deployment/service_mesh/templates"))
...
"""
    ...
    def _run_istioctl(self, args: List[str], input_data: Optional[str] = None) -> Dict[str, Any]:
    ...
    def create_virtual_service(self, service_name: str, fix_id: str, 
                              host: str, gateway: str = "mesh",
                              canary_percentage: int = 0) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Virtual service template not found at {self.template_dir / 'virtual_service.yaml'}")
    ...
    def create_destination_rule(self, service_name: str, fix_id: str,
                               primary_version: str = "v1",
                               canary_version: str = "canary") -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Destination rule template not found at {self.template_dir / 'destination_rule.yaml'}")
            return {"success": False, "error": "Template not found"}
    ...
    def update_canary_percentage(self, service_name: str, 
        ...
        try:
            ...
            if result.get("simulated", False):
                ...
                logger.info(f"Simulating update of canary percentage to {percentage}% for {service_name}")
                return {"success": True, "simulated": True}
            ...
            if len(routes) >= 2:
                ...
                logger.error(f"VirtualService {vs_name} does not have expected route structure")
    ...
    def delete_istio_resources(self, service_name: str) -> Dict[str, Any]:
        ...
        vs_result = self._run_kubectl([
            "delete", "virtualservice", vs_name,
            "--namespace", self.namespace
        ])
        
        # Delete DestinationRule
        dr_result = self._run_kubectl([
            "delete", "destinationrule", dr_name,
            "--namespace", self.namespace
        ])
    ...
    def setup_canary_deployment(self, service_name: str, fix_id: str,
                              host: str, initial_percentage: int = 0) -> Dict[str, Any]:
        ...
        logger.info(f"Successfully setup Istio canary deployment for {service_name} with {initial_percentage}% traffic to fix {fix_id}")
