...
class AWSProvider(BaseCloudProvider):
    ...
    def _run_aws_cli(self, args: List[str], 
        ...
        try:
            ...
            try:
                if stdout and stdout.strip().startswith("{") or stdout.strip().startswith("["):
    ...
    def _deploy_lambda(self, service_name: str, fix_id: str, 
        ...
        try:
            ...
            if get_function_result.get("success", False) and not get_function_result.get("simulated", False):
                ...
                if not role_arn:
                    ...
                    if get_role_result.get("success", False) and not get_role_result.get("simulated", False):
                ...
                create_function_result = self._run_aws_cli([
                    ...
                    "--memory-size", str(memory_size),
                    "--timeout", str(timeout),
                    "--tags", f"Service={service_name},FixId={fix_id}"
                ])
    ...
    def _deploy_ecs(self, service_name: str, fix_id: str, 
                  source_path: str, **kwargs) -> Dict[str, Any]:
        """Deploy to AWS ECS.
        ...
        try:
            ...
            if get_cluster_result.get("success", False) and not get_cluster_result.get("simulated", False):
            ...
            task_def = {
                ...
                "networkMode": "awsvpc",
                "executionRoleArn": kwargs.get("execution_role_arn", "ecsTaskExecutionRole"),
            ...
            task_definition_arn = register_task_result.get("taskDefinition", {}).get("taskDefinitionArn")
            ...
            if services and services[0].get("status") != "INACTIVE" and not get_service_result.get("simulated", False):
    ...
    def _deploy_eks(self, service_name: str, fix_id: str, 
                  source_path: str, **kwargs) -> Dict[str, Any]:
        """Deploy to AWS EKS.
    ...
    def _undeploy_lambda(self, service_name: str, fix_id: str, **kwargs) -> Dict[str, Any]:
    ...
    def _undeploy_ecs(self, service_name: str, fix_id: str, **kwargs) -> Dict[str, Any]:
        ...
        list_result = self._run_aws_cli([
            ...
            "--family-prefix", service_identifier,
            "--sort", "DESC",
            "--max-items", "1"
        ])
        
        if not list_result.get("success", False) and not list_result.get("simulated", False):
            return list_result
        ...
        if task_definitions:
            ...
            deregister_result = self._run_aws_cli([
                ...
                "--task-definition", task_definitions[0].split("/")[1]
            ])
    ...
    def get_service_logs(self, service_name: str, fix_id: str, 
        ...
        if deployment_type == "lambda" and self.lambda_enabled:
            ...
            cmd = [
                "logs", "filter-log-events",
                "--log-group-name", f"/ecs/{service_identifier}"
            ]
