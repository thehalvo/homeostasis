...
class AzureProvider(BaseCloudProvider):
    ...
    def _deploy_function(self, service_name: str, fix_id: str, 
        ...
        try:
            ...
            rg_exists = check_rg_result in [True, "true", "True"] or check_rg_result.get("output") in [True, "true", "True"]
            ...
            if check_storage_result.get("nameAvailable", True) or check_storage_result.get("simulated", False):
                ...
                if not create_storage_result.get("success", False) and not create_storage_result.get("simulated", False):
            ...
            if check_function_result.get("success", False) and not check_function_result.get("simulated", False):
                ...
                if not create_plan_result.get("success", False) and not create_plan_result.get("simulated", False):
                ...
                if not create_function_result.get("success", False) and not create_function_result.get("simulated", False):
    ...
    def _deploy_container(self, service_name: str, fix_id: str, 
                        source_path: str, **kwargs) -> Dict[str, Any]:
        ...
        Returns:
            Dict[str, Any]: Deployment information
        """
        container_group_name = kwargs.get("container_group_name", f"{service_name}-{fix_id}")
        image = kwargs.get("image")
        
        if not image:
            logger.error("Image required for Container Instance deployment")
            return {"success": False, "error": "Image required for Container Instance deployment"}
            
        if not self.az_available:
            logger.info(f"Simulating Azure Container Instance deployment: {container_group_name}")
            return {
                "success": True, 
                "simulated": True,
        ...
        try:
            ...
            if check_container_result.get("success", False) and not check_container_result.get("simulated", False):
                # Container group exists, delete it first
                logger.info(f"Deleting existing Container Instance: {container_group_name}")
                ...
                if not delete_result.get("success", False) and not delete_result.get("simulated", False):
    ...
    def _deploy_aks(self, service_name: str, fix_id: str, 
                  source_path: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            if not get_credentials_result.get("success", False) and not get_credentials_result.get("simulated", False):
    ...
    def deploy_service(self, service_name: str, fix_id: str, 
                     source_path: str, **kwargs) -> Dict[str, Any]:
    ...
    def _undeploy_function(self, service_name: str, fix_id: str, **kwargs) -> Dict[str, Any]:
    ...
    def _undeploy_container(self, service_name: str, fix_id: str, **kwargs) -> Dict[str, Any]:
        ...
        if not self.az_available:
            logger.info(f"Simulating Azure Container Instance undeployment: {container_group_name}")
            return {
                "success": True, 
                "simulated": True,
                "container_group_name": container_group_name
    ...
    def undeploy_service(self, service_name: str, fix_id: str, 
                       **kwargs) -> Dict[str, Any]:
    ...
    def get_service_status(self, service_name: str, fix_id: str, 
                         **kwargs) -> Dict[str, Any]:
        ...
        if deployment_type == "function" and self.function_app_enabled:
            ...
            logger.info(f"Getting Azure Container Instance status: {container_group_name}")
    ...
    def get_service_logs(self, service_name: str, fix_id: str, 
        ...
        if deployment_type == "function" and self.function_app_enabled:
            ...
            logger.info(f"Getting Azure Container Instance logs: {container_group_name}")
            ...
            try:
                ...
                try:
                    ...
                    if process.returncode != 0:
                        logger.error(f"Error getting Kubernetes logs: {process.stderr.decode()}")
                        return []
