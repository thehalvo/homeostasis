...
class IaCProvider(ABC):
    ...
    @abstractmethod
    async def plan(self, working_dir: Path, variables: Dict[str, Any]) -> List[InfrastructureChange]:
        """Generate execution plan"""
        pass
    
    @abstractmethod
    async def apply(self, working_dir: Path, variables: Dict[str, Any], 
                   auto_approve: bool = False) -> Dict[str, Any]:
        """Apply infrastructure changes"""
        pass
    
    @abstractmethod
    async def destroy(self, working_dir: Path, variables: Dict[str, Any],
                     auto_approve: bool = False) -> Dict[str, Any]:
    ...
    @abstractmethod
    async def import_resource(self, working_dir: Path, resource_addr: str, 
...
class TerraformProvider(IaCProvider):
    ...
    async def plan(self, working_dir: Path, variables: Dict[str, Any]) -> List[InfrastructureChange]:
        ...
        try:
            ...
            await self._run_command(
                ["plan", "-out", str(plan_file), "-json"] + var_args,
                working_dir
            )
            
            # Parse plan JSON
            result = await self._run_command(
                ["show", "-json", str(plan_file)],
                working_dir
            )
    ...
    async def import_resource(self, working_dir: Path, resource_addr: str,
        ...
        try:
            await self._run_command(
                ["import", resource_addr, resource_id],
                working_dir
            )
            return True
        except Exception as e:
    ...
    def _parse_resource_change(self, resource_change: Dict[str, Any]) -> Optional[InfrastructureChange]:
    ...
    def _analyze_impact(self, resource_change: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            "dependencies": len(resource_change.get("change", {}).get("after_unknown", {})),
            "sensitive_changes": any(
                resource_change.get("change", {}).get("after_sensitive", {}).values()
            ),
            "force_replacement": resource_change.get("change", {}).get("replace_paths", []) != []
        }
    
    def _estimate_duration(self, resource_type: ResourceType, change_type: ChangeType) -> int:
        ...
        return int(base_duration.get(resource_type, 120) * multiplier.get(change_type, 1.0))
    
    def _calculate_risk(self, resource_type: ResourceType, change_type: ChangeType) -> float:
        ...
        risk = base_risk.get(resource_type, 0.5) * change_multiplier.get(change_type, 1.0)
        return min(1.0, risk)
...
class HelmProvider(IaCProvider):
    ...
    async def plan(self, working_dir: Path, variables: Dict[str, Any]) -> List[InfrastructureChange]:
        ...
        try:
            ...
            result = await self._run_command([
                "install", release_name, str(working_dir),
                "--dry-run", "--debug",
                "-n", namespace
            ])
...
class InfrastructureAsCodeIntegration:
    ...
    async def validate_infrastructure(self, repo_id: str, 
        ...
        try:
            ...
            if not provider:
                return {"valid": False, "errors": [f"Provider {repo.tool} not supported"]}
    ...
    async def plan_infrastructure_changes(self, repo_id: str,
        ...
        try:
            ...
            total_risk = sum(c.risk_score for c in changes) / len(changes) if changes else 0
    ...
    def _merge_variables(self, environment: Environment, 
    ...
    async def apply_infrastructure_changes(self, repo_id: str,
                                         environment: Environment,
                                         execution_plan: Dict[str, Any],
                                         auto_approve: bool = False) -> IaCExecution:
        """Apply infrastructure changes"""
        ...
        if not auto_approve and (repo.approval_required or 
                                execution_plan.get("approval_required", False)):
        ...
        # Log execution start
        await self.auditor.log_event("iac_execution_started", {
        ...
        try:
            ...
            # Apply changes
            variables = self._merge_variables(environment, execution_plan.get("variables", {}))
    ...
    async def _get_approval(self, repo: IaCRepository, environment: Environment,
    ...
    async def handle_infrastructure_healing(self, healing_context: HealingContext,
        ...
        for env in healing_context.affected_environments:
            ...
            try:
                ...
                if repo.auto_apply or plan.get("summary", {}).get("total_risk_score", 1.0) < 0.3:
    ...
    async def _determine_infrastructure_changes(self, healing_context: HealingContext,
        ...
        for step in healing_plan.steps:
            ...
            if step.action in ["scale_infrastructure", "add_resources", "modify_network"]:
            ...
            if healing_context.constraints.get("auto_scale", False):
                ...
                changes["min_size"] = healing_context.constraints.get("min_instances", 2)
                changes["max_size"] = healing_context.constraints.get("max_instances", 10)
        
        return changes
    
    async def get_infrastructure_state(self, repo_id: str,
                                     environment: Environment) -> Dict[str, Any]:
        """Get current infrastructure state"""
        # Sync repository
        success, work_dir_str = await self.sync_repository(repo_id)
    ...
    async def import_existing_infrastructure(self, repo_id: str,
        ...
        try:
            ...
            for resource in resources:
                ...
                success = await provider.import_resource(work_dir, resource_addr, resource_id)
    ...
    async def list_executions(self, environment: Optional[Environment] = None,
                            status: Optional[str] = None) -> List[IaCExecution]:
