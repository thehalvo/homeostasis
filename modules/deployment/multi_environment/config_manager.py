...
class ConfigProvider(ABC):
    ...
    @abstractmethod
    async def set(self, key: str, value: Any, environment: Optional[str] = None) -> bool:
...
class LocalConfigProvider(ConfigProvider):
    ...
    def _parse_config_data(self, data: Dict[str, Any], environment: str) -> Dict[str, ConfigValue]:
    ...
    def rotate_key(self, old_key: str, new_key: str, values: List[str]) -> List[str]:
        ...
        for encrypted_value in values:
            try:
                ...
                rotated_values.append(encrypted_value)  # Keep original if rotation fails
...
class ConfigValidator:
    ...
    async def validate(self, config_value: ConfigValue) -> Tuple[bool, List[str]]:
        ...
        for rule_name, rule_value in config_value.validation_rules.items():
            if rule_name in self.validators:
                is_valid, error = self.validators[rule_name](config_value.value, rule_value)
                if not is_valid:
    ...
    def _validate_range(self, value: Any, range_spec: Dict[str, Any]) -> Tuple[bool, str]:
    ...
    def _validate_length(self, value: Any, length_spec: Dict[str, int]) -> Tuple[bool, str]:
...
class MultiEnvironmentConfigManager:
    ...
    async def get_config(self, key: str, environment: Optional[str] = None,
                        scope: ConfigScope = ConfigScope.ENVIRONMENT,
                        decrypt_secrets: bool = True) -> Optional[Any]:
            ...
            for env in scopes_to_try:
                ...
                if value is not None:
                    ...
                    if isinstance(value, dict) and value.get("_type") == "reference":
                        ...
                        value = await self.get_config(ref_key, environment, scope, decrypt_secrets)
                    
                    # Handle secrets
                    if decrypt_secrets and isinstance(value, dict) and value.get("_type") == "secret":
    ...
    def _get_scope_hierarchy(self, scope: ConfigScope, environment: Optional[str]) -> List[str]:
        ...
        if environment:
            ...
            if scope == ConfigScope.SERVICE:
                ...
                pass
            if scope in [ConfigScope.ENVIRONMENT, ConfigScope.SERVICE, ConfigScope.INSTANCE]:
    ...
    async def set_config(self, key: str, value: Any, environment: Optional[str] = None,
                        scope: ConfigScope = ConfigScope.ENVIRONMENT,
                        encrypt_secret: bool = False,
                        validation_rules: Optional[Dict[str, Any]] = None,
                        reason: str = "") -> ConfigChange:
        """Set configuration value with change tracking"""
        # Get current value for change tracking
        old_value = await self.get_config(key, environment, scope, decrypt_secrets=False)
    ...
    def _requires_approval(self, change: ConfigChange) -> bool:
        ...
        if change.config_value.environment and "prod" in change.config_value.environment:
            return True
    ...
    async def _apply_change(self, change: ConfigChange):
        ...
        # Audit log
        await self.auditor.log_event("config_change", {
    ...
    async def apply_template(self, template_id: str, environment: str,
                           variables: Dict[str, Any]) -> List[ConfigChange]:
        ...
        # Validate variables
        missing_vars = [v['name'] for v in template.variables 
        ...
        for key, value in self._flatten_dict(configs).items():
            change = await self.set_config(
                ...
                reason=f"Applied from template {template_id}"
            )
    ...
    async def detect_drift(self, environment: str, 
                          service: Optional[str] = None) -> List[ConfigDrift]:
        ...
        for key in all_keys:
            ...
            if expected != actual:
                differences.append({
                    ...
                    "expected": expected,
                    "actual": actual,
                    "type": "missing" if actual is None else "extra" if expected is None else "mismatch"
                })
        ...
        if differences:
            ...
            drift = ConfigDrift(
                ...
                auto_remediate=severity in ["low", "medium"] and not any(
                    d["key"].endswith((".secret", ".password", ".key")) for d in differences
                )
            )
            drifts.append(drift)
    ...
    async def _get_expected_configs(self, environment: str,
        ...
        for template in self.templates.values():
            if environment in template.metadata.get("environments", []):
                ...
                default_vars = {v['name']: v.get('default', '') for v in template.variables}
    ...
    async def _get_actual_configs(self, environment: str,
                                service: Optional[str]) -> Dict[str, Any]:
    ...
    def _calculate_drift_severity(self, differences: List[Dict[str, Any]],
                                environment: str) -> str:
        """Calculate severity of configuration drift"""
        # Critical if security-related configs changed
        if any(d["key"].endswith((".secret", ".password", ".key", ".token")) for d in differences):
            return "critical"
    ...
    async def remediate_drift(self, drift: ConfigDrift) -> Dict[str, Any]:
        ...
        if not drift.auto_remediate:
            return {"status": "manual_intervention_required", "severity": drift.severity}
        ...
        for diff in drift.differences:
            ...
            try:
                if diff["type"] == "missing":
                    ...
                    await self._apply_change(change)
                    results["changes"].append({"key": key, "action": "removed", "success": True})
                ...
                results["changes"].append({"key": key, "action": "failed", "error": str(e)})
        
        results["status"] = "completed"
        return results
    
    async def promote_config(self, from_environment: str, to_environment: str,
        ...
        for key in keys:
            try:
                ...
                value = await self.get_config(key, from_environment, decrypt_secrets=False)
                if value is None:
                    continue
                
                # Check if value exists in target environment
                existing_value = await self.get_config(key, to_environment, decrypt_secrets=False)
    ...
    async def rollback_config(self, environment: str, timestamp: datetime) -> Dict[str, Any]:
            ...
            try:
                ...
                if historical_value != current_value:
                    ...
                    if historical_value is not None:
                        change = await self.set_config(
                            key, historical_value, environment,
    ...
    async def export_config(self, environment: str, format: ConfigFormat,
        ...
        configs = {}
        for key in keys:
            value = await self.get_config(key, environment, decrypt_secrets=include_secrets)
            if value is not None:
