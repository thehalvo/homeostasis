...
class KubernetesDeployment:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.health_probes = self.config.get(
            "health_probes",
            {
                "liveness": {
                    "path": "/health",
        ...
        self.template_dir = Path(
            self.config.get("template_dir", "modules/deployment/kubernetes/templates")
        )
...
""",
    ...
    def get_namespace(self) -> str:
        ...
        if not result["success"] and not result.get("simulated", False):
            ...
            create_result = self._run_kubectl(
                ["apply", "-f", "-"], input_data=namespace_yaml
            )
            if not create_result["success"] and not create_result.get(
                "simulated", False
            ):
    ...
    def delete_resource(self, resource_type: str, name: str) -> Dict[str, Any]:
        ...
        """
        logger.info(
            f"Deleting {resource_type} '{name}' from namespace {self.namespace}"
        )

        return self._run_kubectl(
            ["delete", resource_type, name, "--namespace", self.namespace]
        )
    ...
    def get_resource(self, resource_type: str, name: str) -> Dict[str, Any]:
        ...
        result = self._run_kubectl(
            ["get", resource_type, name, "--namespace", self.namespace, "-o", "json"]
        )
    ...
    def wait_for_deployment(self, name: str, timeout: int = 300) -> bool:
        ...
        """
        logger.info(
            f"Waiting for deployment '{name}' to be ready (timeout: {timeout}s)"
        )
    ...
    def create_deployment(
        ...
        try:
            ...
            logger.error(
                f"Deployment template not found at {self.template_dir / 'deployment.yaml'}"
            )
        ...
        yaml_content = template.format(
            ...
            liveness_delay=self.health_probes.get("liveness", {}).get(
                "initial_delay", 10
            ),
            liveness_period=self.health_probes.get("liveness", {}).get("period", 30),
            readiness_path=self.health_probes.get("readiness", {}).get(
                "path", "/health"
            ),
            readiness_port=self.health_probes.get("readiness", {}).get("port", port),
            readiness_delay=self.health_probes.get("readiness", {}).get(
                "initial_delay", 5
            ),
    ...
    def create_service(
        ...
        try:
            ...
            logger.error(
                f"Service template not found at {self.template_dir / 'service.yaml'}"
            )
    ...
    def create_ingress(
        ...
        try:
            ...
            logger.error(
                f"Ingress template not found at {self.template_dir / 'ingress.yaml'}"
            )
    ...
    def deploy_service(
        ...
        deployment_result = self.create_deployment(
            service_name=service_name, fix_id=fix_id, image=image
        )
        results["deployment"] = deployment_result

        if not deployment_result["success"] and not deployment_result.get(
            "simulated", False
        ):
        ...
        # Create service
        service_result = self.create_service(service_name=service_name, fix_id=fix_id)
        ...
        if create_ingress and host:
            ingress_result = self.create_ingress(
                service_name=service_name, fix_id=fix_id, host=host
            )
            results["ingress"] = ingress_result

            if not ingress_result["success"] and not ingress_result.get(
                "simulated", False
            ):
