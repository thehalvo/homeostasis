...
class AWSLambdaProvider(ServerlessProvider):
        ...
        **kwargs,
        ...
        try:
            ...
            exists_result = self._run_aws_cli(
                "lambda", "get-function", args=["--function-name", full_function_name]
            )

            if exists_result["success"] and not exists_result.get("simulated", False):
                # Function exists, update it
                logger.info(
                    f"Function {full_function_name} already exists, updating it"
                )
                return self.update_function(
                    function_name, fix_id, source_path, handler=handler
                )
            ...
            if not role_arn:
                return {
                    "success": False,
                    "error": "No IAM role ARN provided for Lambda function",
                }
            ...
            tags_args = (
                ["--tags"] + [f"{k}={v}" for k, v in tags.items()] if tags else []
            )
    ...
    def update_function(
        ...
        **kwargs,
        ...
        try:
            ...
            code_result = self._run_aws_cli(
                "lambda", "update-function-code", args=code_update_args
            )
            ...
            if handler:
                ...
                if "memory_size" in kwargs:
                    config_update_args.extend(
                        ["--memory-size", str(kwargs["memory_size"])]
                    )
                ...
                config_result = self._run_aws_cli(
                    "lambda", "update-function-configuration", args=config_update_args
                )

                if not config_result["success"] and not config_result.get(
                    "simulated", False
                ):
                    logger.error(
                        f"Failed to update Lambda function configuration: {config_result}"
                    )
        ...
        try:
            ...
            if since:
                ...
                try:
                    ...
                    logger.warning(
                        f"Invalid timestamp format for 'since': {since}. Using default."
                    )
    ...
    def invoke_function(
        ...
        try:
            ...
            result = self._run_aws_cli(
                "lambda",
                "invoke",
                args=invoke_args + ["--output", "json", response_file],
            )
    ...
    def setup_canary_deployment(
        ...
        try:
            ...
            publish_result = self._run_aws_cli(
                "lambda", "publish-version", args=publish_args
            )

            if not publish_result["success"] and not publish_result.get(
                "simulated", False
            ):
                logger.error(
                    f"Failed to publish Lambda function version: {publish_result}"
                )
                return publish_result

            # Get the version number
            version = (
                publish_result.get("Version", "1")
                if not publish_result.get("simulated", False)
                else "1"
            )
            ...
            routing_config = json.dumps(
                {"AdditionalVersionWeights": {version: traffic_percentage / 100.0}}
            )
            ...
            alias_args = [
                ...
                alias_name,
                "--function-version",
                "$LATEST",  # Primary traffic goes to LATEST
                "--routing-config",
                routing_config,
            ]
            ...
            if not alias_result["success"] and "ResourceConflictException" in str(
                alias_result.get("stderr", "")
            ):
                logger.info("Alias 'canary' already exists, updating it")
                return self.update_canary_percentage(
    ...
    def update_canary_percentage(
        ...
        try:
            ...
            if alias_result.get("simulated", False):
                ...
                version = (
                    next(iter(additional_weights.keys())) if additional_weights else "1"
                )
            ...
            update_result = self._run_aws_cli(
                "lambda", "update-alias", args=update_args
            )
